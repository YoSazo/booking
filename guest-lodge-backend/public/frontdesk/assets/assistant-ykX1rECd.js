import{e as M,c as n}from"./settings-BScQqWYt.js";let F=!1,g=null;const R=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function w(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function B(t,e,a){let i=null;const s=new Promise((d,r)=>{i=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(i))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function x(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(n.assistantData?.recipients||[]).filter(t=>t.active)}function A(){return E().filter(t=>t.verified)}function S(){return!!n.assistantData?.hotel?.subscribed||!!n.isMasterPin}function P(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function q(){if(n.assistantData?.latestResult)return n.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(n.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function L(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const i=Math.floor(a/60);if(i<24)return`${i} hr${i===1?"":"s"} ago`;const s=Math.floor(i/24);return s<7?`${s} day${s===1?"":"s"} ago`:$(t)}function O(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(n.nativeNotificationState||""),a=e==="registered",i=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(n.assistantData&&!t)return n.assistantData;if(g&&!t)return g;const e=n.activeHotelId;return n.assistantLoading=!0,n.assistantError="",h(),g=B(l("GET","/api/crm/frontdesk-assistant"),R,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return n.activeHotelId!==e?null:(n.assistantData=a.data,a.data)}).catch(a=>{throw n.activeHotelId===e&&(n.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{n.assistantLoading=!1,g=null,h(),document.getElementById("frontDeskAssistantOverlay")&&v()}),g}function h(){D();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=n.currentFilter==="bookings"&&n.bookingsSubview==="bookings"&&!n.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(P()){const r=q();if(!n.assistantData||!r){t.innerHTML="",t.style.display="none",!n.assistantData&&!n.assistantLoading&&!n.assistantError&&u().catch(()=>{});return}const f=r.type==="availability_warning"||r.status==="attention";t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result${f?" attention":""}" onclick="openFrontDeskAssistant()" aria-label="Open Front Desk Assistant activity">
    ...
    </button>`;return}if(!n.assistantData){const r=!!n.assistantError&&!n.assistantLoading;t.innerHTML=`<div class="fda-card is-off">
    ...
    </div>`,n.assistantLoading||u().catch(()=>{});return}const a=A(),i=n.assistantData.config||{},s=n.assistantData.bookingApproval||{};if(i.enabled){const r=i.nextCheckAt?` · next ${$(i.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
  ...
  </div>`}function _(t){const e=o((t.name||"?").charAt(0).toUpperCase()),a=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
  ...
  </div>`}function j(){const t=(n.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
  ...
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function V(){const t=n.assistantData;if(!t)return n.assistantError&&!n.assistantLoading?`<div class="fda-section">
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <input class="fda-toggle" id="assistant-enabled" type="checkbox" ${e.enabled?"checked":""} ${p} aria-label="Turn Front Desk Assistant on">
      </div>
    </div>
  ...
  <div class="fda-sheet-body">${V()}</div>`,T())}function U(){document.getElementById("frontDeskAssistantSheet")&&v()}function T(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,a=Number(document.getElementById("assistant-approval-window")?.value||20),i=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=i==="release"?`<strong>Your rule:</strong> no answer after ${a} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${a} minutes keeps the booking and emails the guest a confirmation.`}function Y(){w(!1),D();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&y()}),document.body.appendChild(t)),document.body.style.overflow="hidden",v(),requestAnimationFrame(()=>w(!1)),n.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function y(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",w(!0)}function W(){n.assistantError="",v(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(n.assistantData=t.data),h(),v(),e&&c(e,"success"),t}async function Z(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),a=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!a)return c("Enter a name and mobile number.","error");try{const i=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:a});b(i,i.verificationSent?"Verification code sent.":"Phone connected.")}catch(i){c(i.message||"Could not add that phone.","error")}}async function G(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(a){c(a.message||"That code could not be verified.","error")}}async function J(t){try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function Q(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function K(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await l("PUT","/api/crm/frontdesk-assistant",t)),await l("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(a){c(a.message||"Could not save the assistant.","error")}}async function X(){try{return b(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function tt(){try{b(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function et(){const t=n.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:Text this contact when a walk-in or outside booking changes your availability.","END:VCARD"].join(`\r
`),a=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),i=document.createElement("a");i.href=a,i.download="Marketel-Front-Desk.vcf",document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(a),2e3)}async function at(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){y(),c("Front Desk app access is managed with your Marketel account.","info");return}y();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const nt={activateFromAssistant:at,addAssistantRecipient:Z,closeFrontDeskAssistant:y,loadFrontDeskAssistant:u,openFrontDeskAssistant:Y,removeAssistantRecipient:Q,refreshFrontDeskAssistantSheet:U,renderFrontDeskAssistantCard:h,retryFrontDeskAssistant:W,resendAssistantCode:J,runAssistantCheckNow:tt,saveAssistantContact:et,saveAssistantSettings:K,sendAssistantTest:X,updateAssistantPolicySummary:T,verifyAssistantRecipient:G};function st(){F||(F=!0,D(),M(nt))}export{at as activateFromAssistant,Z as addAssistantRecipient,y as closeFrontDeskAssistant,st as install,u as loadFrontDeskAssistant,Y as openFrontDeskAssistant,U as refreshFrontDeskAssistantSheet,Q as removeAssistantRecipient,h as renderFrontDeskAssistantCard,J as resendAssistantCode,W as retryFrontDeskAssistant,tt as runAssistantCheckNow,et as saveAssistantContact,K as saveAssistantSettings,X as sendAssistantTest,T as updateAssistantPolicySummary,G as verifyAssistantRecipient};
