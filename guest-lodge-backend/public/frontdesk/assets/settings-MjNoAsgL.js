const d={token:"",bookings:[],guestMessages:[],currentFilter:"settings",manualAvailability:{rooms:[],overrides:{}},manualSelectedRoom:"",availabilityYear:new Date().getFullYear(),availabilityMonth:new Date().getMonth(),availabilityEditingDay:"",availabilityDaySaving:!1,editingRoomName:"",pendingDeleteRoomName:"",currentHotelPms:"",revenueEnabled:!1,hotelSubscribed:!1,revenuePeriod:"30d",revenueCache:{},revenueLoading:!1,revenueError:"",ALLOWED_REVENUE_PERIODS:new Set(["today","7d","30d","all"]),OTA_COMMISSION_RATE:.25,activeHotelId:"",activeHotelName:"",activeHotelAppIcon:"",appsViewPlatform:"ios",activeHotelDomain:"",activeHotelContext:null,bootInFlight:!1,CRM_HOTEL_BY_HOST:{"guestlodgeminot.clickinns.com":"guest-lodge-minot","booking-kappa-nine.vercel.app":"guest-lodge-minot","stcroix.clickinns.com":"st-croix-wisconsin","homeplacesuites.clickinns.com":"home-place-suites","myhomeplacesuites.com":"home-place-suites","www.myhomeplacesuites.com":"home-place-suites","suitestay.clickinns.com":"suite-stay","clickinns.com":"suite-stay","www.clickinns.com":"suite-stay"},CRM_HOTEL_LABELS:{"guest-lodge-minot":"Guest Lodge Minot","st-croix-wisconsin":"St. Croix Wisconsin","home-place-suites":"Home Place Suites","suite-stay":"Suite Stay"},deferredInstallPrompt:null,frontdeskInstalled:!1,_magicLoginPending:!1,editRooms:[],messageUnreadCount:0,messagesInboxOpen:!1,messagesThreadPickerOpen:!1,selectedMessageThread:"",bookingsVirtualList:[],bookingsVirtualRaf:0};let P=null;function $e(){return typeof lucide<"u"?Promise.resolve():P||(P=new Promise((e,t)=>{const o=document.createElement("script");o.src="https://unpkg.com/lucide@0.469.0/dist/umd/lucide.min.js",o.async=!0,o.onload=()=>e(),o.onerror=()=>t(new Error("lucide load failed")),document.head.appendChild(o)}),P)}async function K(e){if(!e||!e.type.startsWith("image/")||e.type==="image/webp"&&e.size<4e5)return e;try{const t=await createImageBitmap(e),o=1600,r=1200;let i=t.width,n=t.height;const a=Math.min(1,o/i,r/n);i=Math.round(i*a),n=Math.round(n*a);const l=document.createElement("canvas");l.width=i,l.height=n,l.getContext("2d").drawImage(t,0,0,i,n),t.close();const g=await new Promise((m,b)=>{l.toBlob(w=>w?m(w):b(new Error("encode failed")),"image/webp",.82)}),x=(e.name||"room-photo").replace(/\.[^.]+$/,"")||"room-photo";return new File([g],x+".webp",{type:"image/webp"})}catch{return e}}function Ae(){const e=()=>{d.currentFilter==="bookings"?loadMessages():loadMessageBadges()};"requestIdleCallback"in window?requestIdleCallback(e,{timeout:2500}):setTimeout(e,600)}function X(e){Object.assign(window,e)}async function H(){const e=document.getElementById("settingsList");if(e){e.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const t=await api("GET","/api/crm/verify"),r="https://"+(t?.domain||d.activeHotelId+".bookmarketel.com"),i="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(r),n=await api("GET","/api/crm/rooms");let a={nightly:69,weekly:299,monthly:999};n?.rates&&(a=n.rates);const l=n?.rooms||[];let g="";l.length?l.forEach(x=>{const m=x.images&&x.images.length>0;g+=`
    ...
    `,e.innerHTML=g}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load settings</div></div>'}}}function Z(){const e=document.getElementById("settings-booking-url");e&&navigator.clipboard.writeText(e.value).then(()=>{setLaunchChecklistFlag("link"),S(),toast("Link copied!","success")}).catch(()=>toast("Copy failed","error"))}function ee(){localStorage.setItem("settingsTourDone","1");const e=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",e);let t=0;const o=setInterval(()=>{t++;const r=document.getElementById("edit-rate-nightly");if(r||t>20){if(clearInterval(o),!r)return;const i=r.closest(".accordion-body");if(i&&i.style.display==="none"){i.style.display="block";const n=i.previousElementSibling?.querySelector(".accordion-arrow");n&&(n.style.transform="rotate(90deg)")}setTimeout(()=>{r.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const n=document.getElementById("checklistPointer");n&&n.remove();const a=r.getBoundingClientRect(),l=document.createElement("div");l.id="checklistPointer",l.style.cssText=`position:fixed;z-index:100000;left:50%;transform:translateX(-50%);top:${a.bottom+12}px;max-width:240px;width:calc(100% - 40px);`,l.innerHTML=`
          ...
          `,document.body.appendChild(l),setTimeout(()=>{const g=document.getElementById("checklistPointer");g&&g.remove()},6e3)},1e3)},100)}},200)}function I(e){updatePropertyStatusStrip(),applySetupStepFocus();const t=e==="photo"?document.getElementById("launchStepRates"):e==="rates"?document.querySelector('[data-launch-step="link"]'):null;t&&setTimeout(()=>t.scrollIntoView({behavior:"smooth",block:"start"}),400)}function te(){const t="https://"+(d.activeHotelDomain||d.activeHotelId+".bookmarketel.com");navigator.clipboard.writeText(t).then(()=>{setLaunchChecklistFlag("link"),S(),I("link"),toast("Link copied!","success"),loadBookings()}).catch(()=>toast("Copy failed","error"))}function oe(e,t){localStorage.setItem("settingsTourDone","1");const o=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",o);let r=0;const i=setInterval(()=>{r++;const n=document.querySelector(e);if(n||r>20){if(clearInterval(i),!n)return;n.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const a=document.getElementById("checklistPointer");a&&a.remove();const l=n.getBoundingClientRect(),g=document.createElement("div");g.id="checklistPointer",g.style.cssText=`
        ...
        `,document.body.appendChild(g),setTimeout(()=>{const x=document.getElementById("checklistPointer");x&&x.remove()},6e3)},1e3)}},200)}function D(){const e=String(d.token||localStorage.getItem("crmToken")||"").trim();return e&&(d.token=e),e}async function $(e,t){const o=D();if(!o)throw new Error("Not logged in");const r=await K(t),i=new FormData;i.append("image",r,r.name||"room.webp");const n=new URLSearchParams;d.activeHotelId&&n.set("hotelId",d.activeHotelId),n.set("token",o);const a=await fetch(`/api/crm/rooms/${e}/images?${n}`,{method:"POST",headers:{"x-crm-token":o},body:i}),l=await a.json().catch(()=>({}));if(!a.ok||!l.success)throw new Error(l.message||l.error||`Upload failed (${a.status})`);return l}async function ie(e,t){const o=e.target.files[0];if(o)try{await $(t,o),toast("Photo uploaded!","success"),H()}catch(r){toast(r.message||"Upload failed","error")}}async function ne(){const e=parseFloat(document.getElementById("settings-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("settings-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("settings-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),toast("Rates saved","success")}catch{toast("Failed to save rates","error")}}async function re(){const e=document.getElementById("settings-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),d.token=e;try{localStorage.setItem("crmToken",d.token)}catch{}toast("PIN updated!","success"),document.getElementById("settings-new-pin").value=""}catch{toast("Failed to change PIN","error")}}async function ae(){const e=document.getElementById("settings-support-msg")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),toast("Message sent!","success"),document.getElementById("settings-support-msg").value=""}catch{toast("Failed to send","error")}}function se(){const e=d.activeHotelDomain||d.activeHotelId+".bookmarketel.com",o=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:5173/?hotelId="+encodeURIComponent(d.activeHotelId)+"&preview=1":"https://"+e+"?preview=1";window.open(o,"_blank")}function de(){const e=document.getElementById("previewSiteBar");e&&(e.style.display=d.currentFilter==="settings"?"block":"none")}function S(){if(localStorage.getItem("settingsTourDone"))return;const e=parseInt(localStorage.getItem("settingsTourStep")||"0"),t=d.editRooms.some(a=>a.images&&a.images.length>0),o=hasLaunchChecklistFlag("rates"),r=hasLaunchChecklistFlag("link");e===1&&t&&localStorage.setItem("settingsTourStep","2"),e===2&&o&&localStorage.setItem("settingsTourStep","3"),e===3&&r&&localStorage.setItem("settingsTourStep","4");const i=document.getElementById("tourTooltip");i&&i.remove();const n=document.getElementById("tourBlurOverlay");n&&n.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(a=>{a.style.position=a.dataset.tourOrigPosition||"",a.style.zIndex="",a.style.boxShadow="",a.removeAttribute("data-tour-highlighted"),delete a.dataset.tourOrigPosition}),document.body.style.overflow=""}function le(){let e=0;const t={},o=[{title:"Why do you want a booking engine?",key:"why",type:"text",placeholder:"e.g. I want guests to book directly instead of calling me..."},{title:"How do guests currently book with you?",key:"currentBooking",type:"choice",options:[{label:"They call me or walk in",value:"phone_walkin"},{label:"Through Booking.com / Expedia",value:"ota"},{label:"I have a website but no booking system",value:"website_no_booking"},{label:"I don't take bookings online yet",value:"no_online"}]},{title:"How many rooms do you have?",key:"roomCount",type:"choice",options:[{label:"1–5 rooms",value:"1-5"},{label:"6–15 rooms",value:"6-15"},{label:"16–50 rooms",value:"16-50"},{label:"50+ rooms",value:"50+"}]},{title:"What's most important to you?",key:"priority",type:"choice",options:[{label:"Stop paying OTA commissions",value:"no_commission"},{label:"Get more direct bookings",value:"more_bookings"},{label:"Have a professional online presence",value:"professional"},{label:"Make it easier for guests to book",value:"easier_booking"}]}];function r(){let i=document.getElementById("onboardingOverlay");if(i&&i.remove(),e>=o.length){localStorage.setItem("onboardingDone","1");try{api("POST","/api/crm/onboarding-answers",t).catch(()=>{})}catch{}R();return}const n=o[e],a=document.createElement("div");a.id="onboardingOverlay",a.style.cssText="position:fixed;inset:0;z-index:100001;background:linear-gradient(135deg, #1a2b22 0%, #2E7D5B 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;",n.type==="text"?(a.innerHTML=`
      ...
      `,document.body.appendChild(a),document.getElementById("onboardingTextSubmit").onclick=()=>{const l=document.getElementById("onboardingTextInput").value.trim();l&&(t[n.key]=l,e++,r())}):(a.innerHTML=`
        <div style="max-width:360px;width:100%;text-align:center;">
          <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-bottom:8px;">Question ${e+1} of ${o.length}</div>
          <h2 style="font-size:20px;font-weight:700;color:white;margin:0 0 24px;line-height:1.3;">${n.title}</h2>
      ...
      `,document.body.appendChild(a),a.querySelectorAll(".onboarding-opt").forEach(l=>{l.addEventListener("click",()=>{t[n.key]=l.dataset.value,l.style.background="#1a1a2e",l.style.color="white",l.style.fontWeight="600",setTimeout(()=>{e++,r()},250)})}))}r()}function R(){const e=document.createElement("div");e.id="welcomeModalOverlay",e.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px;",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById("welcomeModalGotIt").onclick=()=>{e.remove(),d.hotelSubscribed?setTimeout(()=>{A()},200):setTimeout(()=>{maybeAutoGuideLaunchStep()},400)}}function pe(){if(document.getElementById("activatedModalOverlay"))return;const e=d.activeHotelDomain||(d.activeHotelId?d.activeHotelId+".bookmarketel.com":""),t=document.createElement("div");t.id="activatedModalOverlay",t.style.cssText="position:fixed;inset:0;z-index:100002;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px;",t.innerHTML=`
  ...
  `,document.body.appendChild(t),document.getElementById("activatedModalDone").onclick=()=>{t.remove();try{setFilter("bookings")}catch{}}}function O(){const e=isStandaloneApp()||d.frontdeskInstalled,t=typeof Notification<"u"&&Notification.permission==="granted";let o;return e&&pushSupported()?o=t?`<p style="font-size:12px;color:var(--text-muted);margin:0 0 14px;line-height:1.5;">You're all set — you'll get an alert on this phone every time a guest books.</p>
    ...
    <div id="iosInstallSheetCard" style="background:#fff;width:100%;max-width:440px;border-radius:20px 20px 0 0;padding:24px 22px 32px;box-shadow:0 -8px 40px rgba(0,0,0,0.2);">
      ...
      <button id="iosInstallSheetDone" style="width:100%;margin-top:22px;padding:14px;border-radius:12px;border:none;background:#2E7D5B;color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;">Got it</button>
    </div>`,document.body.appendChild(n),typeof lucide<"u"&&lucide.createIcons();const l=()=>n.remove();n.addEventListener("click",m=>{m.target===n&&l()});const g=document.getElementById("iosInstallSheetCard");g&&g.addEventListener("click",m=>m.stopPropagation());const x=document.getElementById("iosInstallSheetDone");x&&(x.onclick=l)}function A(){if(!d.hotelSubscribed){localStorage.setItem("settingsTourDone","1"),localStorage.removeItem("settingsTourStep");return}if(localStorage.getItem("settingsTourDone"))return;let e=parseInt(localStorage.getItem("settingsTourStep")||"0");const t=[{target:"",text:"",openAccordion:!1,tab:"settings",customModal:"homescreen"},{target:"#edit-hotel-phone",text:"Tap any field here to edit your hotel info — name, address, phone.",openAccordion:!1,tab:"settings"},{target:"#editRoomsCards .booking-card > div:first-child",text:"Add a photo of your room — guests book more when they see one.",openAccordion:!1,tab:"settings"},{target:"#launchStepRates",text:"You can change your rates here — nightly, weekly, and monthly.",openAccordion:!0,tab:"settings"},{target:'button[onclick*="copyBookingLink"]',text:"This is your booking URL — the link guests use to book with you directly.",openAccordion:!1,tab:"settings"},{target:'button[onclick*="openPreviewSite"]',text:"Preview your booking engine to see exactly what your guests see.",openAccordion:!1,tab:"settings"},{target:"#edit-new-pin",text:"This is your login PIN — you'll need it to access this dashboard next time. You can change it here anytime.",openAccordion:!0,expandAccountHelp:!0,tab:"settings"},{target:"#bookingsList",text:"",openAccordion:!1,tab:"bookings",customModal:!0},{target:"#availabilityCalendarWrap",text:"",openAccordion:!1,tab:"availability",customModal:"availability"},{target:".revenue-savings-pill",text:"See how much revenue your booking page generates and how much you're saving vs OTA commissions.",openAccordion:!1,tab:"revenue",waitForVisible:!0},{target:"#paymentsExplainer",text:"When guests book through your engine, their card is verified but never charged. This prevents no-shows — and you collect payment however you want when they physically check in.",openAccordion:!1,tab:"revenue",waitForVisible:!0,tooltipPosition:"above"},{target:"#bookingsList",text:"",openAccordion:!1,tab:"bookings",customModal:"finale"}];function o(){const s=document.getElementById("tourTooltip");s&&s.remove();const p=document.getElementById("tourBlurOverlay");p&&p.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(c=>{c.style.position=c.dataset.tourOrigPosition||"",c.style.zIndex="",c.style.boxShadow="",c.removeAttribute("data-tour-highlighted"),delete c.dataset.tourOrigPosition}),document.body.style.overflow=""}function r(){if(o(),!d.hotelSubscribed){localStorage.setItem("settingsTourDone","1"),localStorage.removeItem("settingsTourStep");return}x()}function i(){if(o(),e>=t.length){localStorage.setItem("settingsTourDone","1"),localStorage.removeItem("settingsTourStep");return}const s=t[e];if(s.tab==="revenue"&&!d.revenueEnabled){e++,localStorage.setItem("settingsTourStep",String(e)),i();return}if(s.tab&&s.tab!==d.currentFilter){const p=document.querySelector(`.tab[data-nav-filter="${s.tab}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${s.tab}"]`);p&&setFilter(s.tab,p),s.customModal?setTimeout(()=>{n(s)},150):setTimeout(()=>{n(s)},800);return}setTimeout(()=>{n(s)},100)}function n(s){if(s.customModal==="homescreen"){g();return}if(s.customModal===!0||s.customModal==="bookings"){w();return}if(s.customModal==="availability"){b();return}if(s.customModal==="finale"){x();return}if(s.waitForVisible){const u=s.target.split(",").map(h=>h.trim());let y=0;const v=20,f=()=>{y++;let h=null;for(const k of u){if(h=document.querySelector(k),h&&h.offsetParent!==null)break;h=null}h?a(h,s):y<v?setTimeout(f,200):(e++,localStorage.setItem("settingsTourStep",String(e)),i())};f();return}let p=null;const c=s.target.split(",").map(u=>u.trim());for(const u of c){if(p=document.querySelector(u),p&&!s.openAccordion&&p.offsetParent===null&&getComputedStyle(p).position!=="fixed"){p=null;continue}if(p)break}if(!p){e++,localStorage.setItem("settingsTourStep",String(e)),i();return}a(p,s)}function a(s,p){if(document.querySelectorAll("[data-setup-focus]").forEach(y=>y.classList.remove("setup-hidden")),p.expandAccountHelp){const y=document.querySelector('[data-setup-focus="extra"].booking-card'),v=y?.querySelector(".accordion-body"),f=y?.querySelector('[onclick*="toggleSection"]');v&&v.style.display==="none"&&f&&W(f)}if(p.openAccordion){let y=s.closest(".accordion-body");if(!y&&s.querySelector&&(y=s.querySelector(".accordion-body")),y&&y.style.display==="none"){y.style.display="block";const f=y.previousElementSibling?.querySelector(".accordion-arrow");f&&(f.style.transform="rotate(90deg)")}}p.targetParent&&(s=s.closest(".booking-card")||s.closest(".accordion-body")||s);const c=document.createElement("div");c.id="tourBlurOverlay",c.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.6);",document.body.appendChild(c),document.body.style.overflow="hidden",s.dataset.tourOrigPosition||(s.dataset.tourOrigPosition=s.style.position||""),s.style.position=s.style.position||"relative",s.style.zIndex="99999",s.style.isolation="isolate",s.style.boxShadow="0 0 0 4px #2E7D5B, 0 0 20px rgba(46,125,91,0.3)",s.setAttribute("data-tour-highlighted","1"),s.scrollIntoView({behavior:"smooth",block:"center"});const u=p.openAccordion||p.expandAccountHelp?750:500;setTimeout(()=>{l(s,p)},u)}function l(s,p){const c=s.getBoundingClientRect(),u=document.createElement("div");u.id="tourTooltip";const y=window.innerHeight-c.bottom,v=c.top,f=Math.min(280,window.innerWidth-32),h=c.left+c.width/2,k=Math.max(16,Math.min(h-f/2,window.innerWidth-f-16)),T=Math.max(20,Math.min(h-k,f-20)),E='<div style="margin-top:8px;"><button id="tourSkipBtn" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:11px;font-family:inherit;cursor:pointer;padding:4px 8px;">Skip tour</button></div>',z=p.tooltipPosition||(y>130?"below":v>130?"above":"center");z==="below"?(u.style.cssText=`position:fixed;z-index:100000;left:${k}px;top:${c.bottom+12}px;max-width:${f}px;width:${f}px;`,u.innerHTML=`
        ...
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          ...
          ${E}
        </div>`):z==="above"?(u.style.cssText=`position:fixed;z-index:100000;left:${k}px;bottom:${window.innerHeight-c.top+12}px;max-width:${f}px;width:${f}px;`,u.innerHTML=`
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          <p style="margin:0 0 12px;">${p.text}</p>
          <button id="tourNextBtn" style="padding:8px 20px;border-radius:6px;border:none;background:#2E7D5B;color:white;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;">${e<t.length-1?"Next →":"Got it!"}</button>
          ${E}
        </div>
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${T-8}px;"></div>`):(u.style.cssText=`position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:${f}px;width:${f}px;`,u.innerHTML=`
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          <p style="margin:0 0 12px;">${p.text}</p>
          <button id="tourNextBtn" style="padding:8px 20px;border-radius:6px;border:none;background:#2E7D5B;color:white;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;">${e<t.length-1?"Next →":"Got it!"}</button>
          ${E}
        </div>`),document.body.appendChild(u),document.getElementById("tourNextBtn").onclick=()=>{o(),e++,localStorage.setItem("settingsTourStep",String(e)),i()},document.getElementById("tourSkipBtn").onclick=()=>{r()}}function g(){const s=document.createElement("div");s.id="tourBlurOverlay",s.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(s),document.body.style.overflow="hidden";const p=d.activeHotelName||"Your Hotel",c=p.trim().charAt(0).toUpperCase(),u=p.length>10?p.slice(0,10):p,y="width:32px;display:flex;flex-direction:column;align-items:center;gap:5px;",v="width:32px;height:32px;border-radius:9px;box-sizing:border-box;",f="height:8px;max-width:46px;line-height:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;",h=`<div style="${y}"><div style="${v}background:rgba(255,255,255,0.22);"></div><div style="${f}"></div></div>`,k=`<div style="${y}"><div style="${v}background:#fff;color:#2E7D5B;font-weight:800;font-size:15px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);">${c}</div><div style="${f}font-size:7.5px;color:#fff;font-weight:700;">${u}</div></div>`,T=[h,h,h,h,k,h,h,h].join(""),E=document.createElement("div");if(E.id="tourTooltip",E.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:20px 16px;",E.innerHTML=`
      ...
      </div>`,document.body.appendChild(p),!document.getElementById("tourModalAnimStyle")){const c=document.createElement("style");c.id="tourModalAnimStyle",c.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(c)}document.getElementById("tourNextBtn").onclick=()=>{const u="https://"+(d.activeHotelDomain||d.activeHotelId+".bookmarketel.com");navigator.clipboard.writeText(u).catch(()=>{}),o(),localStorage.setItem("settingsTourDone","1"),setLaunchChecklistFlag("link"),localStorage.removeItem("settingsTourStep"),toast("Booking link copied!","success"),m()}}function m(s){const p=document.createElement("div");p.id="testDriveOverlay",p.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px 16px;",p.innerHTML=`
      <div style="background:white;border-radius:20px;max-width:340px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      <div style="padding:0 14px 14px;">
      ...
      </div>`],u=document.createElement("div");u.id="tourTooltip",u.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;";function y(){const f=p>=c.length-1?"Next — Revenue →":"Next →";u.innerHTML=`
        <div style="background:white;border-radius:20px;max-width:340px;width:100%;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
          ...
          <div style="padding:4px 18px 6px;text-align:center;">
            <div style="display:flex;justify-content:center;gap:6px;margin-bottom:10px;">
              ${c.map((h,k)=>`<div style="width:8px;height:8px;border-radius:50%;background:${k===p?"#2E7D5B":"#D8E4DC"};"></div>`).join("")}
            </div>
          </div>
        ...
        </div>`,document.getElementById("tourNextBtn").onclick=()=>{p<c.length-1?(p++,y()):(o(),e++,localStorage.setItem("settingsTourStep",String(e)),i())},document.getElementById("tourSkipBtn").onclick=()=>{r()}}if(document.body.appendChild(u),y(),!document.getElementById("tourModalAnimStyle")){const v=document.createElement("style");v.id="tourModalAnimStyle",v.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(v)}}function w(){const s=document.createElement("div");s.id="tourBlurOverlay",s.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(s),document.body.style.overflow="hidden";const p=document.createElement("div");if(p.id="tourTooltip",p.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;",p.innerHTML=`
      <div style="background:white;border-radius:20px;max-width:340px;width:100%;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      </div>`,document.body.appendChild(p),!document.getElementById("tourModalAnimStyle")){const c=document.createElement("style");c.id="tourModalAnimStyle",c.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(c)}document.getElementById("tourNextBtn").onclick=()=>{o(),e++,localStorage.setItem("settingsTourStep",String(e)),i()},document.getElementById("tourSkipBtn").onclick=()=>{r()}}i()}async function L(){const e=document.getElementById("editRoomsList");if(e){e.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const t=await api("GET","/api/crm/rooms");if(!t.rooms)throw new Error("No data");d.editRooms=t.rooms;const o=await api("GET","/api/crm/verify"),r=o?.hotelName||"";r&&(d.activeHotelName=r);const i=o?.hotelSubtitle||"",n=o?.hotelAddress||"",a=o?.hotelPhone||"",l=o?.appIconUrl||"";d.activeHotelAppIcon=l,updateFrontdeskManifestLink();let g={nightly:69,weekly:299,monthly:999,taxRate:.1};t.rates&&(g=t.rates);const m="https://"+(o?.domain||d.activeHotelId+".bookmarketel.com"),b="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(m),w=!hasLaunchChecklistFlag("rates");let s=`
      ...
      <div data-setup-focus="link" class="booking-card" data-launch-step="link" style="margin-bottom:14px;">
        <div style="padding:18px;">
          ...
          <div style="background:var(--bg);border-radius:10px;padding:14px;margin-bottom:12px;text-align:center;">
            ...
            <button onclick="copyBookingLink('${m.replace(/'/g,"\\'")}')" style="padding:10px 20px;border-radius:8px;border:none;background:var(--green);color:white;font-family:inherit;font-size:14px;font-weight:700;cursor:pointer;">📋 Copy link</button>
          </div>
          <div style="text-align:center;margin-bottom:10px;">
      ...
      <div data-setup-focus="extra" class="booking-card" style="margin-bottom:14px;">
        ...
        <div class="accordion-body" style="display:none;padding:0 18px 18px;">
          ...
          <div style="margin-bottom:12px;">
            <input type="text" id="edit-new-pin" value="${d.token}" placeholder="New PIN (4+ characters)" style="width:100%;font-size:16px;padding:8px 10px;border-radius:8px;border:1.5px solid var(--border);font-family:inherit;outline:none;text-align:center;letter-spacing:2px;">
          </div>
    ...
    `;e.innerHTML=s,B(),updatePropertyStatusStrip(),applySetupStepFocus(),typeof lucide<"u"&&lucide.createIcons(),setTimeout(()=>{d.hotelSubscribed&&d.currentFilter==="settings"&&localStorage.getItem("onboardingDone")&&!localStorage.getItem("settingsTourDone")&&!document.getElementById("welcomeModalOverlay")&&A()},300)}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>'}}}function F(){B()}function B(){const e=document.getElementById("editRoomsCards")||document.getElementById("editRoomsList");if(!d.editRooms.length){e.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>';return}e.innerHTML=d.editRooms.map(t=>{const o=(t.amenities||"").split("•").map(i=>i.trim()).filter(Boolean),r=t.images||[];return`
      ...
      <div style="padding:18px;">
        ...
        <div style="margin-bottom:12px;">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;" id="edit-amenity-pills-${t.id}">
            ${o.map(i=>`<span style="display:inline-flex;align-items:center;gap:4px;background:var(--green-pale);color:var(--green);padding:5px 10px;border-radius:8px;font-size:12px;font-weight:600;">${q(i)} ${i} <button onclick="removeAmenity('${t.id}','${i.replace(/'/g,"\\'")}')" style="background:none;border:none;color:var(--green);cursor:pointer;font-size:14px;margin-left:2px;">×</button></span>`).join("")}
          </div>
    ...
    </div>`}).join("")}function q(e){const t=e.toLowerCase();return t.includes("wifi")?'<i data-lucide="wifi" style="width:14px;height:14px;"></i>':t.includes("tv")||t.includes("television")?'<i data-lucide="tv" style="width:14px;height:14px;"></i>':t.includes("fridge")||t.includes("refrigerator")?'<i data-lucide="thermometer-snowflake" style="width:14px;height:14px;"></i>':t.includes("parking")?'<i data-lucide="car" style="width:14px;height:14px;"></i>':t.includes("housekeeping")||t.includes("cleaning")?'<i data-lucide="sparkles" style="width:14px;height:14px;"></i>':t.includes("bath")||t.includes("shower")?'<i data-lucide="bath" style="width:14px;height:14px;"></i>':t.includes("work")||t.includes("desk")?'<i data-lucide="laptop" style="width:14px;height:14px;"></i>':t.includes("pet")||t.includes("dog")?'<i data-lucide="paw-print" style="width:14px;height:14px;"></i>':t.includes("pool")?'<i data-lucide="waves" style="width:14px;height:14px;"></i>':t.includes("kitchen")||t.includes("microwave")?'<i data-lucide="cooking-pot" style="width:14px;height:14px;"></i>':'<i data-lucide="check" style="width:14px;height:14px;"></i>'}const G=[{key:"wifi",label:"Free WiFi",icon:"wifi"},{key:"tv",label:"Smart TV",icon:"tv"},{key:"fridge",label:"Fridge",icon:"thermometer-snowflake"},{key:"parking",label:"Free Parking",icon:"car"},{key:"housekeeping",label:"Weekly Housekeeping",icon:"sparkles"},{key:"bath",label:"Bath",icon:"bath"},{key:"workstation",label:"Workstation",icon:"laptop"},{key:"pet",label:"Pet Friendly",icon:"paw-print"},{key:"pool",label:"Pool",icon:"waves"},{key:"kitchen",label:"Kitchenette",icon:"cooking-pot"},{key:"ac",label:"Air Conditioning",icon:"wind"},{key:"laundry",label:"Laundry",icon:"shirt"}];let N=null;function Y(e){N=e;const o=(d.editRooms.find(n=>n.id===e)?.amenities||"").split("•").map(n=>n.trim().toLowerCase()).filter(Boolean);let r=document.getElementById("amenityPickerModal");r||(document.body.insertAdjacentHTML("beforeend",`
    ...
    `),document.getElementById("amenityPickerModal").addEventListener("click",M),r=document.getElementById("amenityPickerModal"));const i=document.getElementById("amenityPickerGrid");i.innerHTML=G.map(n=>{const a=o.some(l=>l.includes(n.key));return`<button onclick="toggleAmenityPreset(this,'${n.key}')" data-key="${n.key}" style="display:inline-flex;align-items:center;gap:5px;padding:7px 12px;border-radius:8px;border:1.5px solid ${a?"#2E7D5B":"#e5e7eb"};background:${a?"#E8F5EE":"white"};color:${a?"#2E7D5B":"#1a1a2e"};font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;"><i data-lucide="${n.icon}" style="width:14px;height:14px;"></i> ${n.label}</button>`}).join(""),document.getElementById("amenityCustomInput").value="",r.style.display="flex",typeof lucide<"u"&&lucide.createIcons()}function xe(e,t){const o=e.style.borderColor==="rgb(46, 125, 91)";e.style.borderColor=o?"#e5e7eb":"#2E7D5B",e.style.background=o?"white":"#E8F5EE",e.style.color=o?"#1a1a2e":"#2E7D5B"}function M(){document.getElementById("amenityPickerModal").style.display="none",N=null}function me(){const e=d.editRooms.find(i=>i.id===N);if(!e){M();return}const t=document.getElementById("amenityPickerGrid"),o=[];t.querySelectorAll("button").forEach(i=>{if(i.style.background==="rgb(232, 245, 238)"){const n=G.find(a=>a.key===i.dataset.key);n&&o.push(n.label)}});const r=document.getElementById("amenityCustomInput").value.trim();r&&o.push(r),e.amenities=o.join(" • "),M(),F(),typeof lucide<"u"&&lucide.createIcons()}function fe(e){Y(e)}function ye(e,t){const o=d.editRooms.find(i=>i.id===e);if(!o)return;const r=(o.amenities||"").split("•").map(i=>i.trim()).filter(Boolean);o.amenities=r.filter(i=>i!==t).join(" • "),F(),typeof lucide<"u"&&lucide.createIcons()}async function he(){const e=document.getElementById("edit-hotel-name")?.value.trim(),t=document.getElementById("edit-hotel-subtitle")?.value.trim(),o=document.getElementById("edit-hotel-address")?.value.trim(),r=document.getElementById("edit-hotel-phone")?.value.trim(),i=document.getElementById("edit-hotel-policy")?.value.trim();try{await api("POST","/api/crm/hotel-info",{name:e,subtitle:t,address:o,phone:r,cancellationPolicy:i}),toast("Hotel info saved!","success")}catch{toast("Failed to save","error")}}async function be(){const e=parseFloat(document.getElementById("edit-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("edit-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("edit-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),setLaunchChecklistFlag("rates"),S(),I("rates"),toast("Rates saved!","success")}catch{toast("Failed to save rates","error")}}async function ve(){const e=document.getElementById("edit-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),d.token=e;try{localStorage.setItem("crmToken",d.token)}catch{}toast("PIN updated!","success")}catch{toast("Failed to change PIN","error")}}function we(e){navigator.clipboard.writeText(e).then(()=>{setLaunchChecklistFlag("link"),S(),I("link"),toast("Link copied!","success")}).catch(()=>{toast("Failed to copy","error")})}function W(e){const t=e.nextElementSibling,o=e.querySelector(".accordion-arrow");t.style.display==="none"?(t.style.display="block",o&&(o.style.transform="rotate(90deg)")):(t.style.display="none",o&&(o.style.transform="rotate(0deg)"))}async function ke(){try{const e=await api("POST","/api/crm/go-live");e.success&&e.url?window.location.href=e.url:toast(e.message||"Failed to start checkout","error")}catch{toast("Failed to start checkout. Try again.","error")}}async function Ee(){try{const e=await api("GET","/api/crm/billing-portal");e.success&&e.url?window.location.href=e.url:toast(e.message||"Contact support@bookmarketel.com to manage your subscription.","error")}catch{toast("Contact support@bookmarketel.com to manage your subscription.","error")}}async function ze(){const e=document.getElementById("supportMessage")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),document.getElementById("supportMessage").value="",toast("Message sent! We'll reply to your email.","success")}catch{toast("Failed to send. Email support@bookmarketel.com directly.","error")}}async function Se(e){const t=d.editRooms.find(l=>l.id===e);if(!t){toast("Room not found — try refreshing","error");return}const o=document.getElementById("edit-name-"+e)?.value.trim(),r=document.getElementById("edit-desc-"+e)?.value.trim(),i=parseInt(document.getElementById("edit-occ-"+e)?.value)||4,n=parseInt(document.getElementById("edit-units-"+e)?.value)||1,a={id:e,name:o||t.name,description:r||"",amenities:t.amenities||"",maxOccupancy:i,totalUnits:n};try{const l=await api("POST","/api/crm/rooms",a);if(l&&l.success===!1){toast(l.message||"Failed to save","error");return}t.name=a.name,t.description=a.description,t.maxOccupancy=i,t.totalUnits=n,toast("Room saved!","success")}catch(l){toast("Failed to save: "+(l.message||""),"error")}}async function Ie(e,t){const o=Array.from(e.target.files);if(!o.length)return;e.target.value="";const i=document.getElementById("edit-card-"+t)?.querySelector("div:first-child");i&&(i.style.position="relative",i.insertAdjacentHTML("beforeend",'<div id="upload-spinner-'+t+'" style="position:absolute;inset:0;background:rgba(255,255,255,0.8);display:flex;align-items:center;justify-content:center;z-index:5;flex-direction:column;gap:6px;"><div style="width:24px;height:24px;border:3px solid var(--border);border-top-color:var(--green);border-radius:50%;animation:spin 0.7s linear infinite;"></div><div id="upload-progress-'+t+'" style="font-size:12px;color:var(--text-muted);font-weight:600;">0 / '+o.length+"</div></div>"));let n=0,a="";for(const g of o){try{const m=await $(t,g);if(m.image){const b=d.editRooms.find(w=>w.id===t);b&&(b.images||(b.images=[]),b.images.push(m.image),b.imageUrl||(b.imageUrl=m.image.url)),n++}}catch(m){a=m.message||"Upload failed"}const x=document.getElementById("upload-progress-"+t);x&&(x.textContent=n+" / "+o.length)}const l=document.getElementById("upload-spinner-"+t);l&&l.remove(),B(),S(),n>0?(I("photo"),toast(n+" photo"+(n!==1?"s":"")+" added!","success")):toast(a||"Upload failed — try JPG or PNG","error")}function _(e,t=512){return new Promise((o,r)=>{const i=new Image,n=URL.createObjectURL(e);i.onload=()=>{try{const a=Math.min(i.naturalWidth,i.naturalHeight),l=(i.naturalWidth-a)/2,g=(i.naturalHeight-a)/2,x=document.createElement("canvas");x.width=t,x.height=t;const m=x.getContext("2d");m.imageSmoothingQuality="high",m.drawImage(i,l,g,a,a,0,0,t,t),URL.revokeObjectURL(n),x.toBlob(b=>b?o(b):r(new Error("crop failed")),"image/png",.92)}catch(a){URL.revokeObjectURL(n),r(a)}},i.onerror=()=>{URL.revokeObjectURL(n),r(new Error("load failed"))},i.src=n})}function V(){const e=document.getElementById("appsAppIconPreview");e&&(e.innerHTML='<div style="width:24px;height:24px;border:3px solid rgba(255,255,255,0.5);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;"></div>')}function Q(e){const t=document.getElementById("appsAppIconPreview");t&&(t.style.background="#fff",t.innerHTML='<img src="'+e+'" alt="App icon" style="width:100%;height:100%;object-fit:cover;">')}async function Be(e){const t=e.files&&e.files[0];if(!t)return;V();const o=new FormData;try{const r=await _(t,512);o.append("icon",r,"app-icon.png")}catch{o.append("icon",t)}try{const r=D(),i=new URLSearchParams;d.activeHotelId&&i.set("hotelId",d.activeHotelId),r&&i.set("token",r);const a=await(await fetch(`/api/crm/hotel-app-icon?${i}`,{method:"POST",headers:{"x-crm-token":r},body:o})).json();if(a.success&&a.appIconUrl){d.activeHotelAppIcon=a.appIconUrl,Q(a.appIconUrl);const l=document.getElementById("appsView");l&&(l.dataset.appsKey=(d.activeHotelId||"")+"|"+a.appIconUrl+"|"+(d.activeHotelDomain||"")),typeof updateFrontdeskManifestLink=="function"&&updateFrontdeskManifestLink(),toast("Logo updated! Guests will see it on their phone.","success")}else{toast(a.message||"Failed to upload icon","error");const l=(d.activeHotelName||"H").trim().charAt(0).toUpperCase()||"🏨",g=document.getElementById("appsAppIconPreview");g&&(g.style.background="var(--green)",g.innerHTML=l)}}catch{toast("Failed to upload icon","error")}e.value=""}async function Te(e,t){if(confirm("Delete this photo?"))try{await api("DELETE",`/api/crm/rooms/${e}/images/${t}`);const o=d.editRooms.find(r=>r.id===e);o&&o.images&&(o.images=o.images.filter(r=>r.id!==t),o.imageUrl=o.images[0]?.url||null),B(),toast("Photo deleted","success")}catch{toast("Failed to delete","error")}}async function Pe(e){if(confirm("Delete this room type?"))try{await api("DELETE",`/api/crm/rooms/${e}`),toast("Room deleted","success"),L()}catch{toast("Failed to delete","error")}}function Me(){const e=document.getElementById("editRoomsList");document.getElementById("editAddForm")||(e.insertAdjacentHTML("beforeend",`
  ...
  `),document.getElementById("editNewRoomName").focus())}function Ce(){const e=document.getElementById("editNewRoomName").value.trim();e&&api("POST","/api/crm/rooms",{name:e,maxOccupancy:4,totalUnits:5}).then(()=>{toast("Room added","success"),L()}).catch(()=>toast("Failed to add","error"))}const J={addAmenityPrompt:fe,advanceSetupAfterAction:I,advanceTourIfNeeded:S,changePin:ve,checklistGoTo:oe,checklistGoToRates:ee,closeAmenityPicker:M,confirmAmenityPicker:me,confirmEditAddRoom:Ce,copyBookingLink:we,copyBookingLinkFromChecklist:te,deleteEditImage:Te,deleteEditRoom:Pe,frontdeskInstallCardHtml:O,getAmenityIcon:q,getCrmAuthToken:D,goLive:ke,handleInstallFrontdesk:ce,loadEditRooms:L,loadSettings:H,maybePromptInstalledNotifications:ue,openAmenityPicker:Y,openBillingPortal:Ee,openEditAddRoom:Me,openPreviewSite:se,postRoomImageUpload:$,refreshFrontdeskInstallCard:C,removeAmenity:ye,renderEditRooms:F,renderEditRoomsCards:B,saveEditRoom:Se,saveHotelInfo:he,saveRates:be,sendSupportMessage:ze,setAppIconPreviewImage:Q,setAppIconPreviewLoading:V,settingsChangePin:re,settingsCopyLink:Z,settingsSaveRates:ne,settingsSendSupport:ae,settingsUploadPhoto:ie,showActivatedModal:pe,showIosInstallSheet:U,showNotifPromptModal:j,showOnboardingQuestions:le,showWelcomeModal:R,squareCropImage:_,startSettingsTour:A,toggleAmenityPreset:xe,toggleAppNotifications:ge,toggleSection:W,updatePreviewSiteBar:de,uploadAppIcon:Be,uploadEditImages:Ie};function De(){X(J)}const Le=Object.freeze(Object.defineProperty({__proto__:null,default:J,install:De},Symbol.toStringTag,{value:"Module"}));export{$e as a,Le as b,d as c,X as e,Ae as s};
