import{e as q,c as i,f as H}from"./settings-Z62S4umK.js";let I=!1,m=null;const O=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function $(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function _(t,e,a){let n=null;const s=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function A(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function S(){return(i.assistantData?.recipients||[]).filter(t=>t.active)}function F(){return S().filter(t=>t.verified)}function N(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function g(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function j(){if(!g())return!1;try{const t=window.webkit?.messageHandlers?.marketelShell;return!t||typeof t.postMessage!="function"?!1:(t.postMessage({type:"openAssistant"}),!0)}catch{return!1}}function V(){const t=[i.editRooms,i.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const a=String(e[0]?.name||"").trim();if(a)return a}return"Queen Room"}function C(){if(i.assistantData?.latestResult)return i.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(i.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function U(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const n=Math.floor(a/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:T(t)}function Y(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(i.nativeNotificationState||""),a=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function T(t,e={}){if(!t)return"";const a=new Date(t);if(!Number.isFinite(a.getTime()))return"";const n=Date.now(),s=Math.round((a.getTime()-n)/6e4);if(e.relative&&s>0&&s<60)return`in ${s} min`;if(e.relative&&s>=60&&s<1440){const d=Math.round(s/60);return`in ${d} hr${d===1?"":"s"}`}return a.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function W(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function R(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const a=Number(e[1]);if(!Number.isInteger(a)||a<0||a>23)return"Not set";const n=a>=12?"PM":"AM";return`${a%12||12}:${e[2]} ${n}`}function D(t,e,a=""){const n=o(e||"");return`<div class="fda-time-control${a?" is-disabled":""}">
  ...
  `,document.head.appendChild(t)}async function p({force:t=!1}={}){if(i.assistantData&&!t)return i.assistantData;if(m&&!t)return m;const e=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",w(),m=_(l("GET","/api/crm/frontdesk-assistant"),O,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==e?null:(i.assistantData=a.data,a.data)}).catch(a=>{throw i.activeHotelId===e&&(i.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{i.assistantLoading=!1,m=null,w(),document.getElementById("frontDeskAssistantOverlay")&&v()}),m}const M="marketelAssistantConfigured";function G(){try{return localStorage.getItem(M+":"+(i.activeHotelId||""))==="1"}catch{return!1}}function Z(t){try{const e=M+":"+(i.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function J(){const t=i.assistantData,e=S(),a=t?!!t.config?.enabled&&e.length>0:G();t&&Z(a);const n=C(),s=!!n&&(n.type==="availability_warning"||n.status==="attention");window.marketelAssistantPillLabel=a?s?"Needs your review":"Front Desk":"Set up Front Desk",window.syncNativeShellState?.(),!t&&!i.assistantLoading&&!i.assistantError&&p().catch(()=>{})}function Q(){window.marketelAssistantPillLabel="Front Desk"}function w(){E();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!g()){t.innerHTML=`<div class="fda-card is-off">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${o(U(r.createdAt))} · View activity</span>
      </span>
    ...
    </div>`,i.assistantLoading||p().catch(()=>{});return}const a=F(),n=i.assistantData.config||{},s=i.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${T(n.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(W(n.checkFrequency))}${o(r)}${o(f)}</div>
        </div>
  ...
  </div>`}function K(t){const e=o((t.name||"?").charAt(0).toUpperCase()),a=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    ...
    <div class="fda-activity-copy">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},a=t.bookingApproval||{},n=S(),s=t.capabilities||{},d=N(),r=Number(s.maxRecipients||3),f=n.length<r,u=d?"":"disabled",y=d&&s.manualAvailability?"":"disabled",P=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",L=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),k=[];s.smsConfigured||k.push("Marketel's texting number still needs its server credentials."),s.manualAvailability||k.push("The Assistant works with properties whose Availability is managed in Marketel.");const B=k.length?`<div class="fda-section fda-lock">
        <div class="fda-section-title">Not ready on this property yet</div>
        <div class="fda-section-sub" style="margin:0;">${k.map(h=>o(h)).join(" ")}</div>
      </div>`:"";return`
    ${!!e.enabled&&n.length>0?"":`<div class="fda-section fda-story">
      ...
      <div class="fda-bubble assistant"><strong>Front Desk</strong><br>New booking: ${o(V())}, tonight. Is it still free?</div>
