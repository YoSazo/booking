import{c as r,e as L}from"./settings-BVJy4Woh.js";const w="marketelValueRevealPendingV1",b="marketelValueRevealStepV1";let a=0,l="guest",u=!1,c={rooms:[],rates:null},f=null,i={ready:!1,checking:!0,reason:"",attempts:0,domain:""},d=0;function I(){const e=window.location.hostname;return e==="localhost"||e==="127.0.0.1"||e==="0.0.0.0"||e==="::1"||e.endsWith(".local")||/^10\./.test(e)||/^192\.168\./.test(e)||/^172\.(1[6-9]|2\d|3[01])\./.test(e)}function o(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function E(e){const t=Number(e);return Number.isFinite(t)?new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:Number.isInteger(t)?0:2}).format(t):"$99"}function s(){return r.activeHotelName||"Your Property"}function p(){return c.rooms[0]||r.editRooms[0]||{name:"Your first room",totalUnits:1,images:[]}}function M(){const e=p();return e.images?.[0]?.url||e.imageUrl||""}function R(){return c.rates?.nightly||99}function y(){if(I()&&r.activeHotelId){const t=new URL(window.location.href);return t.port="5173",t.pathname="/",t.search="",t.hash="",t.searchParams.set("hotelId",r.activeHotelId),t.toString()}const e=i.domain||r.activeHotelDomain||"";return e?`https://${e}/`:""}function T(){const e=new URL(window.location.href);return e.search="",e.hash="",r.activeHotelId&&e.searchParams.set("hotelId",r.activeHotelId),e.searchParams.set("previewEditor","1"),e.toString()}function $(e=""){const t=r.activeHotelAppIcon||M(),n=s().trim().charAt(0).toUpperCase()||"M";return t?`<img class="${e}" src="${o(t)}" alt="">`:`<span class="${e}">${o(n)}</span>`}function x(){if(!r.hotelSubscribed)try{localStorage.setItem(w,"1"),localStorage.setItem(b,String(a))}catch{}}function g(e,t=""){typeof window.api=="function"&&window.api("POST","/api/crm/value-reveal-event",{eventName:e,contentName:t}).catch(()=>{})}function A(){try{const e=new URL(window.location.href);e.searchParams.delete("welcome"),e.searchParams.delete("reveal"),window.history.replaceState({},"",e.pathname+e.search+e.hash)}catch{}}function P(e){typeof window.setNativeShellVisible=="function"&&window.setNativeShellVisible(e)}function D(){return`<div class="mvr-progress" aria-label="Marketel overview progress">
    ${["Booking page","Guest app","Front Desk",r.hotelSubscribed?"Complete":"Activate"].map((t,n)=>`<div class="mvr-progress-item ${n===a?"is-active":""} ${n<a?"is-done":""}">
      <span></span><small>${o(t)}</small>
    </div>`).join("")}
  </div>`}function H(e=""){const t=M();return t?`<img class="${e}" src="${o(t)}" alt="${o(p().name||"Room")}">`:`<div class="${e} mvr-photo-placeholder"><span>${o((p().name||"R").trim().charAt(0).toUpperCase())}</span></div>`}function V(){return i.ready?`<div class="mvr-page-status is-ready"><span>✓</span>${i.reason==="local"?"Local guest preview connected":"Your live guest page is online"}</div>`:i.reason==="deployment-disabled"?'<div class="mvr-page-status is-attention"><span>!</span>Your live page deployment needs to be re-enabled. Your saved setup is safe.</div>':`<div class="mvr-page-status"><span class="mvr-status-pulse"></span>${i.checking?"Publishing your live guest page…":"Your personalized preview is ready while the live page finishes publishing."}</div>`}function N(){const e=p();return`<div class="mvr-booking-preview-card">
    ...
    <div class="mvr-preview-page">
      ...
      <div class="mvr-preview-room">
        ${H("mvr-preview-room-photo")}
        <div>
          <strong>${o(e.name||"Your room")}</strong>
          <small>${Math.max(1,Number(e.totalUnits)||1)} available</small>
        </div>
        <span><strong>${E(R())}</strong><small>/night</small></span>
      </div>
    </div>
    ...
    <div class="mvr-copy">
      ...
      <p>Guests can choose <strong>${o(p().name||"a room")}</strong> and book directly in under 60 seconds.</p>
  ...
  </section>`}function F(){return`<svg viewBox="0 0 64 64" aria-hidden="true">
  ...
  </svg>`}function G(){return`<section class="mvr-stage mvr-stage-app">
    <div class="mvr-copy">
    ...
    <div class="mvr-visual mvr-assistant-visual">
      <div class="mvr-booking-alert">
        ...
        <div><span>Front Desk</span><strong>New ${o(e)} booking</strong><small>Tomorrow · ${E(R())}</small></div>
        <b>now</b>
      </div>
      <div class="mvr-chat">
  ...
  </section>`}function _(){const e=r.hotelSubscribed;return`<section class="mvr-stage mvr-stage-finale">
  ...
  </section>`}function O(){return a===0?C():a===1?G():a===2?U():_()}function q(){if(a===3)return`<div class="mvr-footer mvr-footer-final">
    ...
    ${q()}
  </div>`,j())}function z(){const e=y();if(!e||document.getElementById("mvrLivePreview"))return;l="guest";const t=document.createElement("div");t.id="mvrLivePreview",t.className="mvr-live-preview",t.innerHTML=`<div class="mvr-live-toolbar">
