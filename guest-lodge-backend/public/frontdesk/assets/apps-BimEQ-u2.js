import{c as n,a as Ie,b as Te,e as ze}from"./settings-MEhrarZv.js";function y(e){return typeof window<"u"&&typeof window[e]=="function"?window[e]:null}function te(...e){return y("ensureAppsViewRendered")?.(...e)}function oe(...e){return y("showFinaleMockModal")?.(...e)}function ie(...e){return y("finishTourHydration")?.(...e)}function ae(...e){return y("goLive")?.(...e)}function re(...e){return y("toast")?.(...e)}function Ge(...e){return y("appsCloseLightbox")?.(...e)}let P=[],d=0,W=!1,O=null,F=null,D=null,T=null;function $e(){if(document.getElementById("frontdeskAppsTourStyle"))return;const e=document.createElement("style");e.id="frontdeskAppsTourStyle",e.textContent=`
    #appsTourLightbox {
  ...
  `,document.head.appendChild(e)}function C(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function ne(){O&&(document.removeEventListener("keydown",O),O=null)}function Be(e){ne(),O=t=>{if(t.defaultPrevented)return;const o=t.target&&t.target.tagName?t.target.tagName.toLowerCase():"";o==="input"||o==="textarea"||o==="select"||t.target?.isContentEditable||(t.key==="Escape"?(t.preventDefault(),e.onSkip?.()):t.key==="Enter"||t.key==="ArrowRight"?(t.preventDefault(),e.onNext?.()):t.key==="ArrowLeft"&&(t.preventDefault(),e.onBack?.()))},document.addEventListener("keydown",O)}function _e(e,t){return!e||!e.isConnected||t?.noHighlight?null:(e.dataset.appsTourOrigVisibility||(e.dataset.appsTourOrigVisibility=e.style.visibility||""),T?.destroy(),T=Te(e,{attribute:"data-apps-tour-spotlight-clone",zIndex:100002,hideSource:!0,prepareClone(o){o.style.boxShadow=t?.spotlightBoxShadow??"none",o.style.outline=t?.spotlightOutline??"none",o.style.outlineOffset=t?.spotlightOutlineOffset??"0",(e.classList.contains("apps-story-line")||t?.hideSpotlightBorder)&&(o.style.border="none",o.style.borderTop="none",o.style.borderTopWidth="0",o.style.paddingTop="0")}}),T?.element||null)}function se(e){const t=e||{};ne(),D?.destroy(),D=null,T?.destroy(),T=null,F&&(clearTimeout(F),F=null);const o=document.getElementById("appsTourLightbox");o&&!t.keepLightbox&&o.remove();const a=document.getElementById("appsTourTooltip");a&&a.remove(),document.querySelectorAll("[data-apps-tour-spotlight-clone]").forEach(i=>i.remove()),document.querySelectorAll("[data-apps-tour-highlighted]").forEach(i=>{i.style.position=i.dataset.appsTourOrigPosition||"",i.style.zIndex=i.dataset.appsTourOrigZIndex||"",i.style.isolation=i.dataset.appsTourOrigIsolation||"",i.style.boxShadow=i.dataset.appsTourOrigBoxShadow||"",i.style.outline=i.dataset.appsTourOrigOutline||"",i.style.outlineOffset=i.dataset.appsTourOrigOutlineOffset||"",i.style.transition=i.dataset.appsTourOrigTransition||"",i.style.visibility=i.dataset.appsTourOrigVisibility||"",i.dataset.appsTourOrigBorderTop!=null&&(i.style.borderTop=i.dataset.appsTourOrigBorderTop,i.style.paddingTop=i.dataset.appsTourOrigPaddingTop||"",delete i.dataset.appsTourOrigBorderTop,delete i.dataset.appsTourOrigPaddingTop),i.removeAttribute("data-apps-tour-highlighted"),delete i.dataset.appsTourOrigPosition,delete i.dataset.appsTourOrigZIndex,delete i.dataset.appsTourOrigIsolation,delete i.dataset.appsTourOrigBoxShadow,delete i.dataset.appsTourOrigOutline,delete i.dataset.appsTourOrigOutlineOffset,delete i.dataset.appsTourOrigTransition,delete i.dataset.appsTourOrigVisibility})}function h(e){se(),document.body.style.overflow="";const t=W;W=!1;try{const o=typeof te=="function"?te:window.ensureAppsViewRendered;typeof o=="function"&&o(!0)}catch{}if(e&&(localStorage.setItem("appsTourDone","1"),t||localStorage.getItem("settingsTourStep")==="handoff"||n.settingsTourActive)){const a=typeof oe=="function"?oe:window.showFinaleMockModal;if(typeof a=="function"){a();return}}}function Ee(e){const t=d+e;t<0||t>=P.length||(d=t,z())}function j(){if(localStorage.setItem("appsTourDone","1"),W||localStorage.getItem("settingsTourStep")==="handoff"||n.settingsTourActive){n.settingsTourActive=!1,localStorage.setItem("settingsTourDone","1"),localStorage.removeItem("settingsTourStep");const t=typeof ie=="function"?ie:window.finishTourHydration;typeof t=="function"&&t()}}function Ae(){j();const e=typeof ae=="function"?ae:window.goLive;if(h(!1),typeof e=="function"){e();return}const t=typeof re=="function"?re:window.toast;typeof t=="function"&&t("Open Go live to activate your booking page.","error")}function z(){$e();const e=P[d];if(!e){h(!0);return}const t=P.length,o=d>=t-1,a=`${d+1} / ${t}`,i=Math.max(8,Math.min(100,Math.round((d+1)/t*100))),r=document.querySelector(e.target);if(!r){d++,z();return}se({keepLightbox:!0});let s=document.getElementById("appsTourLightbox");s||(s=document.createElement("div"),s.id="appsTourLightbox",s.style.cssText="position:fixed;inset:0;z-index:100000;background:rgba(17,24,39,0.42);pointer-events:auto;",document.body.appendChild(s)),e.noHighlight||(r.dataset.appsTourOrigPosition=r.style.position||"",r.dataset.appsTourOrigZIndex=r.style.zIndex||"",r.dataset.appsTourOrigIsolation=r.style.isolation||"",r.dataset.appsTourOrigBoxShadow=r.style.boxShadow||"",r.dataset.appsTourOrigOutline=r.style.outline||"",r.dataset.appsTourOrigOutlineOffset=r.style.outlineOffset||"",r.dataset.appsTourOrigTransition=r.style.transition||"",r.style.position=r.style.position||"relative",r.style.zIndex="100002",r.style.isolation="isolate",r.style.transition="box-shadow 0.18s ease, outline 0.18s ease",r.style.boxShadow=e.spotlightBoxShadow??"none",r.style.outline=e.spotlightOutline??"none",r.style.outlineOffset=e.spotlightOutlineOffset??"0",(r.classList.contains("apps-story-line")||e.hideSpotlightBorder)&&(r.dataset.appsTourOrigBorderTop=r.style.borderTop||"",r.dataset.appsTourOrigPaddingTop=r.style.paddingTop||"",r.style.borderTop="none",r.style.paddingTop="0"),r.setAttribute("data-apps-tour-highlighted","1"));const l=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches,v=e.scrollBlock||"nearest",x=l?"auto":"smooth";r.scrollIntoView({behavior:x,block:v});const u=()=>{const p=document.getElementById("appsTourTooltip");p&&p.remove();const g=Math.min(370,window.innerWidth-28),b=e.primaryLabel||(o?"Done":"Next"),V=e.secondaryLabel||(o?"Not now":"Skip tour"),M=d<=0,J=e.kicker||"Guestel",f=document.createElement("div");f.id="appsTourTooltip",f.style.cssText=`position:fixed;z-index:100003;left:12px;top:14px;width:${g}px;max-width:${g}px;visibility:hidden;`,f.innerHTML=`
      <div class="apps-tour-panel" role="dialog" aria-live="polite" aria-label="${C(e.title)}">
        ...
        <div class="apps-tour-kicker">${C(J)}</div>
        <div class="apps-tour-title">${C(e.title)}</div>
        <p class="apps-tour-copy">${C(e.text)}</p>
        <div class="apps-tour-actions">
          <button type="button" id="appsTourBackBtn" class="apps-tour-btn" ${M?"disabled":""}>Back</button>
          <button type="button" id="appsTourSkipBtn" class="apps-tour-btn apps-tour-btn-ghost">${C(V)}</button>
          <button type="button" id="appsTourNextBtn" class="apps-tour-btn apps-tour-btn-primary">${C(b)}</button>
        </div>
      </div>`,document.body.appendChild(f),_e(r,e);const B=f.querySelector(".apps-tour-panel");D?.destroy(),D=Ie({tooltip:f,panel:B,target:r,anchor:r,spotlight:T,options:{preferredPlacement:e.tooltipPosition||"auto",maxWidth:g,gap:e.tooltipGap??10,autoScroll:!0,avoidBottomSelectors:[".mobile-bottom-nav","#previewSiteBar"]}}),f.style.visibility="visible";const k=()=>{if(e.activateOnNext){Ae();return}if(o){j(),h(!1),e.openGuestInstallCoachOnNext&&window.setTimeout(()=>y("appsOpenGuestInstallCoach")?.(),0);return}d++,z()},_=()=>{if(o){j(),h(!1);return}h(!0)},w=()=>{d<=0||(d--,z())};document.getElementById("appsTourNextBtn").onclick=k,document.getElementById("appsTourSkipBtn").onclick=_;const S=document.getElementById("appsTourBackBtn");S&&(S.onclick=w),Be({onNext:k,onBack:w,onSkip:_})};F=setTimeout(()=>{requestAnimationFrame(u)},l?40:320)}function Oe(e){const t=e&&e.replay,o=e&&e.chainFromSettingsTour;if(!t&&!o&&localStorage.getItem("appsTourDone")||document.getElementById("appsTourLightbox")||document.getElementById("appsTourTooltip"))return;Ge(),h(!1),W=!!o;const a=!!n.hotelSubscribed;P=!!y("isNativeFrontdeskApp")?.()||document.body.classList.contains("frontdesk-editor-preview")||new URLSearchParams(window.location.search).get("previewEditor")==="1"?[{target:"#tour-guest-reach",kicker:"Guestel updates",title:"Reach guests who choose to hear from you.",text:"Guests who keep your property in Guestel and allow property updates can receive a notification from you.",tooltipPosition:"above"},{target:"#tour-native-guest-share",kicker:"Invite guests",title:"Give them one Guestel link.",text:"Show the QR or copy the Guestel link. Guests can book immediately, then keep your property and stay in Guestel.",tooltipPosition:"above"},{target:"#tour-guest-icon-section",kicker:"Make it yours",title:"Choose how your property appears.",text:"Use your logo or a clear property photo for your Guestel card.",scrollBlock:"start",tooltipPosition:"below"}]:[{target:"#tour-apps-intro",kicker:"The loop",title:"One system with a clear side for you and for guests.",text:"You use Marketel Front Desk. Guests use Guestel and your direct booking experience."},{target:"#tour-apps-first",kicker:"Your side",title:"Download Marketel Front Desk.",text:"The owner app receives new-booking alerts even when the web dashboard is closed. Guests do not download this app.",scrollBlock:"center",tooltipPosition:"below",tooltipGap:8},{target:"#tour-apps-then",kicker:"Their side",title:"Guests keep your property in Guestel.",text:"They tap Add on your booking page. Guestel keeps your property, reservations, and messages together.",scrollBlock:"center",tooltipPosition:"below",tooltipGap:8},{target:"#tour-apps-after",kicker:"Direct reach",title:"Send a notification to their phone whenever you want.",text:"Anyone who keeps your property in Guestel and opts into property updates becomes reachable from Marketel Front Desk.",scrollBlock:"center",tooltipPosition:"below",tooltipGap:8},{target:"#tour-guest-icon-section",kicker:"One setup item",title:"Make the Guestel card feel like your property.",text:"Use a real logo or a clear property photo.",scrollBlock:"start",tooltipPosition:"auto",tooltipGap:10},{target:"#tour-apps-loop",kicker:a?"Live loop":"Activation",title:a?"This loop is on.":"Everything is ready to turn on.",text:a?"Guests book, keep your property in Guestel, receive opted-in updates, and message you. Front Desk gets the alerts.":"For $199/month, guests can book direct, keep your property in Guestel, receive opted-in updates, and message you — while Front Desk receives the alerts.",primaryLabel:a?"Done":"Activate everything — $199/month",secondaryLabel:a?"Close":"Keep exploring",activateOnNext:!a,tooltipPosition:"below",tooltipGap:8}],d=0,z()}const c={homeScreen:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179630/unnamed_lbsctp.jpg",guestHome:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179605/IMG_2475_1_jxip3r.png",guestBook:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179611/IMG_2476_1_tqbmgz.png",guestMessagesImg:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179605/IMG_2477_1_zgodnn.png",frontdeskMessages:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179656/unnamed_2_qfhkrr.png",guestMessageNotifVideo:"https://res.cloudinary.com/dkmr3h5jb/video/upload/f_mp4,q_auto/ScreenRecording_06-10-2026_23-43-50_1_z22p4m.mp4",guestInstallVideo:"https://res.cloudinary.com/dkmr3h5jb/video/upload/f_mp4,q_auto/ScreenRecording_06-11-2026_00-03-19_1_lgdf59.mp4"},Le="32px";function Y(e,t){return e.replace("/image/upload/","/image/upload/f_auto,q_auto,w_"+(t||400)+"/")}function q(e){return`border-radius:${Le};box-shadow:0 10px 36px rgba(0,0,0,0.22);${e||""}`}function pe(e){const t=Math.min(window.devicePixelRatio||1,2),o=Math.round(Math.min(window.screen.width*t,1600));return e.replace("/image/upload/",`/image/upload/f_auto,q_auto:best,w_${o}/`)}let N=[],G=0,R="ios26",$=null,le="",L=null;function Me(e,t){h(!1),isNativeFrontdeskApp()&&typeof window.setNativeShellVisible=="function"&&window.setNativeShellVisible(!1),N=e,G=t||0;let o=document.getElementById("appsLightbox");if(!o){o=document.createElement("div"),o.id="appsLightbox",o.style.cssText=["position:fixed;inset:0;z-index:102000;background:#000;","display:flex;flex-direction:column;align-items:center;justify-content:flex-start;","overscroll-behavior:contain;touch-action:pan-y;","padding-left:env(safe-area-inset-left,0px);padding-right:env(safe-area-inset-right,0px);"].join(""),document.body.appendChild(o),document.body.style.overflow="hidden",o._keyHandler=i=>{i.key==="ArrowRight"||i.key==="ArrowDown"?H(1):i.key==="ArrowLeft"||i.key==="ArrowUp"?H(-1):i.key==="Escape"&&Z()},document.addEventListener("keydown",o._keyHandler);let a=0;o.addEventListener("touchstart",i=>{a=i.changedTouches[0].clientX},{passive:!0}),o.addEventListener("touchend",i=>{const r=i.changedTouches[0].clientX-a;Math.abs(r)>50&&H(r<0?1:-1)},{passive:!0})}document.body.style.overflow="hidden",K()}function Z(){const e=document.getElementById("appsLightbox");e&&(document.removeEventListener("keydown",e._keyHandler),e.remove(),document.body.style.overflow="",isNativeFrontdeskApp()&&typeof window.setNativeShellVisible=="function"&&(window.setNativeShellVisible(!0),typeof window.syncNativeShellState=="function"&&window.syncNativeShellState()))}function H(e){const t=N.length;t<=1||(G=(G+e+t)%t,K())}function K(){const e=document.getElementById("appsLightbox");if(!e)return;const t=N[G],o=N.length,a=t.type!=="video",i=o>1?`${G+1} / ${o}`:"",r=a?`<img src="${pe(t.src)}" alt="${t.alt||""}"
    ...
    ${Array.from({length:o},(x,u)=>`<div onclick="appsOpenLightbox(_appsLbItems,${u})" style="width:7px;height:7px;border-radius:50%;background:${u===G?"#fff":"rgba(255,255,255,0.35)"};cursor:pointer;transition:background 0.2s;"></div>`).join("")}
  </div>`:"";e.innerHTML=`
  ...
  </svg>`}function Fe(){return`<svg viewBox="44 39.5 21 13" focusable="false" aria-hidden="true">
  ...
  </svg>`}function He(){return`<svg viewBox="0 0 28 28" focusable="false" aria-hidden="true">
  ...
  </svg>`}function Pe(e,t){const o=[...e?[{label:"More",icon:ce()}]:[],{label:"Share",icon:de()},...t?[{label:"View More",icon:Fe()}]:[],{label:"Add to Home Screen",icon:He()}];return`<div class="agic-sequence" aria-label="${o.map(a=>a.label).join(", then ")}">
  ...
  </div>`}function ge(){const e=R==="ios26",t=$==="compact",i=e&&!$?`
    ...
    <section class="agic-sheet" role="dialog" aria-modal="true" aria-label="How guests save your property to their Home Screen">
      <div class="agic-header">
        ${e&&$?'<button type="button" onclick="appsGuestInstallCoachSelectLayout(null)" aria-label="Back">‹</button>':"<span></span>"}
      ...
      ${i}
    </section>`}function ue(){const e=document.getElementById("appsGuestInstallCoach");e&&(e.innerHTML=ge())}function We(){h(!1),Z(),document.getElementById("appsGuestInstallCoach")?.remove(),R="ios26",$=null,le=document.body.style.overflow,document.body.style.overflow="hidden",isNativeFrontdeskApp()&&typeof window.setNativeShellVisible=="function"&&window.setNativeShellVisible(!1),L=t=>{t.key==="Escape"&&fe()},document.addEventListener("keydown",L);const e=document.createElement("div");e.id="appsGuestInstallCoach",e.innerHTML=ge(),document.body.appendChild(e)}function fe(){L&&(document.removeEventListener("keydown",L),L=null),document.getElementById("appsGuestInstallCoach")?.remove(),document.body.style.overflow=le,isNativeFrontdeskApp()&&typeof window.setNativeShellVisible=="function"&&(window.setNativeShellVisible(!0),typeof window.syncNativeShellState=="function"&&window.syncNativeShellState())}function De(e){R=e==="classic"?"classic":"ios26",$=R==="classic"?"standard":null,ue()}function Ne(e){$=["standard","compact"].includes(e)?e:null,ue()}function he(e,t){const o=e||"Video";return`<span class="${"apps-media-badge"+(t==="light"?" apps-media-badge--light":"")}" title="Short video walkthrough"><span class="apps-media-badge__ring" aria-hidden="true"><span class="apps-media-badge__play" aria-hidden="true"></span></span><span class="apps-media-badge__label">${o}</span></span>`}function A(e,t,o,a,i){const r=i?he("Video"):"",s=i?'<span class="apps-q-media" aria-hidden="true"><span class="apps-q-media__play"></span></span>':'<span class="apps-q-chevron" aria-hidden="true">›</span>';return`<button type="button" class="apps-q${i?" apps-q--video":""}" onclick="appsOpenLightbox(${o},${a})">
    ...
    ${s}
  </button>`}function Re(){const e=navigator.userAgent||"";return/iPad|iPhone|iPod/.test(e)&&!window.MSStream?"ios":/android/i.test(e)?"android":"ios"}function I(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function xe(){return String(n.guestelWalletSubtitle||n.activeHotelContext?.address||"Direct booking").trim()}function me(){const e=document.getElementById("guestelWalletSubtitleInput"),t=String(e?.value||xe()).trim()||"Direct booking",o=document.getElementById("guestelWalletPreviewSubtitle");o&&(o.textContent=t);const a=document.getElementById("guestelWalletSubtitleCount");a&&(a.textContent=`${String(e?.value||"").length}/64`)}async function Ve(){const e=document.getElementById("guestelWalletSubtitleInput"),t=document.getElementById("guestelWalletSubtitleSave"),o=String(e?.value||"").replace(/\s+/g," ").trim().slice(0,64);t&&(t.disabled=!0);try{const a=await api("POST","/api/crm/guestel-wallet-card",{subtitle:o});if(!a?.success)throw new Error(a?.message||"Could not save the Guestel card.");n.guestelWalletSubtitle=String(a.subtitle||"").trim(),e&&(e.value=n.guestelWalletSubtitle||String(a.fallbackSubtitle||"").trim()),me(),toast("Guestel card updated.","success")}catch(a){toast(a?.message||"Could not save the Guestel card.","error")}finally{t&&(t.disabled=!1)}}function je(){const e=document.getElementById("edit-offer-enabled")?.checked,t=document.getElementById("edit-offer-kind")?.value==="amount"?"amount":"percent",o=Number(document.getElementById("edit-offer-value")?.value)||0,a=document.getElementById("offerPreview");if(a){if(!e||o<=0){a.textContent="Guests see their normal rate.",a.style.opacity="0.6";return}a.style.opacity="1",a.textContent=t==="amount"?`Returning guests see $${o} off per night.`:`Returning guests see ${o}% off the direct rate.`}}async function qe(){const e=!!document.getElementById("edit-offer-enabled")?.checked,t=document.getElementById("edit-offer-kind")?.value==="amount"?"amount":"percent",o=Number(document.getElementById("edit-offer-value")?.value)||0;try{await api("POST","/api/crm/hotel-info",{returnOfferEnabled:e,returnOfferKind:t,returnOfferValue:o}),n.returnOfferEnabled=e,n.returnOfferKind=t,n.returnOfferValue=o,toast(e?"Returning-guest offer saved!":"Offer turned off","success")}catch{toast("Could not save offer","error")}}function Q(){const e=(n.editRooms||[]).flatMap(t=>Array.isArray(t?.images)?t.images:[]).map(t=>String(t?.url||"").trim()).find(Boolean)||"";return String(n.guestelWalletImageUrl||n.guestelWalletFallbackImageUrl||e||"").trim()}function U(e){const t=document.getElementById("guestelWalletPreviewImage");if(!t)return;const o=String(e||Q()).trim();t.classList.toggle("has-image",!!o),t.innerHTML=o?`<img src="${I(o)}" alt="Guestel wallet cover">`:"<span>Add a room photo</span>";const a=document.getElementById("guestelWalletImageRemove");a&&(a.hidden=!String(n.guestelWalletImageUrl||"").trim())}async function Ue(e){const t=e?.files?.[0];if(!t)return;const o=document.getElementById("guestelWalletImageButton"),a=n.guestelWalletImageUrl;o&&(o.disabled=!0,o.textContent="Uploading…");const i=new FormData;i.append("image",t);try{const r=new URLSearchParams;n.activeHotelId&&r.set("hotelId",n.activeHotelId);const s=await fetch(`/api/crm/guestel-wallet-image?${r}`,{method:"POST",headers:{"x-crm-token":n.token,...isNativeFrontdeskApp()?{"x-marketel-client":"ios"}:{}},body:i}),l=await s.json();if(!s.ok||!l?.success||!l.imageUrl)throw new Error(l?.message||"Could not update the Guestel cover.");n.guestelWalletImageUrl=l.imageUrl,U(l.imageUrl),toast("Guestel cover updated.","success")}catch(r){U(a),toast(r?.message||"Could not update the Guestel cover.","error")}finally{e.value="",o&&(o.disabled=!1,o.textContent=n.guestelWalletImageUrl?"Change cover":"Choose custom cover")}}async function Ye(){const e=document.getElementById("guestelWalletImageRemove");e&&(e.disabled=!0);try{const t=await api("DELETE","/api/crm/guestel-wallet-image");if(!t?.success)throw new Error(t?.message||"Could not reset the Guestel cover.");n.guestelWalletImageUrl="",U(Q());const o=document.getElementById("guestelWalletImageButton");o&&(o.textContent="Choose custom cover"),toast("Guestel will use your first room photo.","success")}catch(t){toast(t?.message||"Could not reset the Guestel cover.","error")}finally{e&&(e.disabled=!1)}}function Ze(e){const t=document.getElementById("appsView");if(!t)return;const o=document.body.classList.contains("frontdesk-editor-preview")||new URLSearchParams(window.location.search).get("previewEditor")==="1",a=(n.activeHotelId||"")+"|"+(n.activeHotelAppIcon||"")+"|"+(n.activeHotelDomain||"")+"|"+(n.guestelWalletImageUrl||"")+"|"+(n.guestelWalletFallbackImageUrl||"")+"|"+(n.guestelWalletSubtitle||"")+"|"+(o?"native-preview":"standard");e||t.dataset.appsKey!==a||!t.querySelector(".apps-page")?(be(),t.dataset.appsKey=a):(isNativeFrontdeskApp()||o)&&X()}function be(){const e=document.getElementById("appsView");if(!e)return;const t=n.activeHotelName||"Your Property";t.trim().charAt(0).toUpperCase();const o=n.activeHotelDomain||"",a=o?"https://"+o:"#",i=n.activeHotelId?`https://clip.mktel.co/clip/${encodeURIComponent(n.activeHotelId)}?intent=book&ref=frontdesk-guestel`:a;function r(Ce){return JSON.stringify(Ce).replace(/"/g,"&quot;")}const s=Y(c.guestHome,520),l=[{type:"image",src:c.homeScreen,alt:"Owner and guest Home Screens",title:"Two different actions. Two different phones.",caption:`You download <strong>Marketel Front Desk</strong> from the App Store. Guests never download Front Desk; they save <strong>${t}</strong> from your booking page.`}],v=[{type:"image",src:c.guestHome,alt:"Guest home screen",title:"What your guests see — Home",caption:"Their stay info — check-in time, your WiFi password, and more."},{type:"image",src:c.guestBook,alt:"Guest book screen",title:"What your guests see — Book a room",caption:"They book directly with you. You keep the money — no middleman."},{type:"image",src:c.guestMessagesImg,alt:"Guest messages",title:"What your guests see — Message you",caption:`They text you from the app — like "What's the WiFi password?"`}],x=[{type:"video",src:c.guestInstallVideo,poster:c.guestHome,alt:"Guest adds property to phone",title:"How guests put your property on their phone",caption:"They open your booking page and tap <strong>Add to Home Screen</strong>. Your property gets its own icon. No App Store is involved."}],u=[{type:"image",src:c.guestMessagesImg,alt:"Guest sends message",title:"Your guest texts you",caption:"Like “How do I connect to WiFi?” — they message you from the property they saved."},{type:"image",src:c.frontdeskMessages,alt:"You reply",title:"You text them back",caption:"Open <strong>Guest Reach</strong> in Marketel Front Desk, choose the conversation, and reply."},{type:"video",src:c.guestMessageNotifVideo,poster:c.guestMessagesImg,alt:"Guest gets reply alert",title:"Their phone buzzes with your answer",caption:"They get your reply on their phone — like a text from you."}],m=isStandaloneApp(),p=isNativeFrontdeskApp(),g=document.body.classList.contains("frontdesk-editor-preview")||new URLSearchParams(window.location.search).get("previewEditor")==="1",b=p||g,V=String(n.nativeNotificationState||""),M=g||V==="registered",f=p?M:(p?M:m)&&typeof Notification<"u"&&Notification.permission==="granted";Number(n.bookingReviewSettings?.reminderMinutes??15),p||m&&f||m||String(n.frontdeskAppStoreUrl||"").trim(),i!=="#"&&`${i.replace("https://","")}`,`${s}${r(x)}${A("What guests see on their phone","",r(v),0,!1)}${A("How guests save your property","",r(x),0,!0)}${A("Guest texts you, you text back","",r(u),0,!0)}${A("Your owner app and their saved property","",r(l),0,!1)}`,a!=="#"&&`${a}`;const B=xe(),k=Q(),_=!!String(n.guestelWalletImageUrl||"").trim(),w=Number(n.returnOfferValue)||0,S=n.returnOfferKind==="amount",ve=n.returnOfferEnabled&&w>0?S?`Returning guests see $${w} off per night.`:`Returning guests see ${w}% off the direct rate.`:"Guests see their normal rate.",we=`
    <div class="apps-step-card" id="tour-native-guest-share">
      ...
      <div class="guestel-wallet-editor">
        <div class="guestel-wallet-card" aria-label="Preview of ${I(t)} in Guestel">
          <div class="guestel-wallet-cover${k?" has-image":""}" id="guestelWalletPreviewImage">${k?`<img src="${I(k)}" alt="Guestel wallet cover">`:"<span>Choose a cover photo</span>"}</div>
          <div class="guestel-wallet-copy">
            <strong>${I(t)}</strong>
            <span id="guestelWalletPreviewSubtitle">${I(B)}</span>
          </div>
        </div>
        ...
        <div class="guestel-wallet-field">
          <input id="guestelWalletSubtitleInput" maxlength="64" value="${I(B)}" placeholder="Location or a short reason to book direct" oninput="updateGuestelWalletPreview()">
          <span id="guestelWalletSubtitleCount">${B.length}/64</span>
        </div>
      ...
      <div class="apps-offer-row">
        <input type="number" min="0" inputmode="numeric" id="edit-offer-value" value="${w||10}" oninput="updateReturnOfferPreview()">
        <select id="edit-offer-kind" onchange="updateReturnOfferPreview()">
          <option value="percent" ${S?"":"selected"}>% off</option>
          <option value="amount" ${S?"selected":""}>$ off / night</option>
        </select>
      </div>
      <div id="offerPreview" class="apps-offer-preview">${ve}</div>
    ...
    ${guestBroadcastCardHtml({compact:!0})}`,E=!!String(n.frontdeskAppStoreUrl||"").trim(),Se=b?ke:`
    <section style="min-height:52vh;display:grid;place-items:center;padding:34px 0;">
      <div style="width:min(100%,430px);padding:28px 24px;border:1.5px solid var(--border);border-radius:22px;background:#fff;text-align:center;box-shadow:0 14px 40px rgba(26,43,34,.09);">
        ...
        <button type="button" onclick="openFrontdeskAppDownload()" ${E?"":'aria-disabled="true"'} style="width:100%;min-height:50px;border:0;border-radius:13px;background:${E?"var(--green)":"#dce8e1"};color:${E?"#fff":"#527061"};font-family:inherit;font-size:15px;font-weight:800;cursor:${E?"pointer":"default"};">${E?"Download Marketel Front Desk":"Front Desk app coming soon"}</button>
      </div>
