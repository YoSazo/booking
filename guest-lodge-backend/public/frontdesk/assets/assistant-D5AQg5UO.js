import{e as q,c as a}from"./settings-hrQdF05L.js";let I=!1,m=null;const O=12e3;function l(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function $(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function H(t,e,i){let n=null;const o=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(i)),e)});return Promise.race([t,o]).finally(()=>clearTimeout(n))}function s(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function A(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function S(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function F(){return S().filter(t=>t.verified)}function N(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function x(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function _(){const t=[a.editRooms,a.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const i=String(e[0]?.name||"").trim();if(i)return i}return"Queen Room"}function C(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function j(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const o=Math.floor(n/24);return o<7?`${o} day${o===1?"":"s"} ago`:T(t)}function V(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",o=e==="unavailable";return`<div class="fda-section">
    <div class="fda-section-title">${i?"Booking alerts are on":n?"Connecting this iPhone":o?"Booking alerts need attention":"Turn on booking alerts"}</div>
  ...
  </div>`}function T(t,e={}){if(!t)return"";const i=new Date(t);if(!Number.isFinite(i.getTime()))return"";const n=Date.now(),o=Math.round((i.getTime()-n)/6e4);if(e.relative&&o>0&&o<60)return`in ${o} min`;if(e.relative&&o>=60&&o<1440){const d=Math.round(o/60);return`in ${d} hr${d===1?"":"s"}`}return i.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function U(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function R(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const i=Number(e[1]);if(!Number.isInteger(i)||i<0||i>23)return"Not set";const n=i>=12?"PM":"AM";return`${i%12||12}:${e[2]} ${n}`}function D(t,e,i=""){const n=s(e||"");return`<div class="fda-time-control${i?" is-disabled":""}">
    <span class="fda-time-value">${s(R(e))}</span>
    <input id="${s(t)}" type="time" value="${n}" ${i} oninput="updateAssistantTimeDisplay(this)">
  ...
  `,document.head.appendChild(t)}async function p({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(m&&!t)return m;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",w(),m=H(l("GET","/api/crm/frontdesk-assistant"),O,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,m=null,w(),document.getElementById("frontDeskAssistantOverlay")&&g()}),m}const z="marketelAssistantConfigured";function Y(){try{return localStorage.getItem(z+":"+(a.activeHotelId||""))==="1"}catch{return!1}}function W(t){try{const e=z+":"+(a.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function G(){const t=a.assistantData,e=S(),i=t?!!t.config?.enabled&&e.length>0:Y();t&&W(i);const n=C(),o=!!n&&(n.type==="availability_warning"||n.status==="attention");window.marketelAssistantPillLabel=i?o?"Needs your review":"Front Desk":"Set up Front Desk",window.syncNativeShellState?.(),!t&&!a.assistantLoading&&!a.assistantError&&p().catch(()=>{})}function Z(){window.marketelAssistantPillLabel="Front Desk"}function w(){E();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!x()){t.innerHTML=`<div class="fda-card is-off">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${s(j(r.createdAt))} · View activity</span>
      </span>
      ...
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${r?s(a.assistantError):"Tell it when a walk-in or another channel takes a room. It updates Availability and your direct booking page."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${r?"Retry":"Open"}</button>
      </div>
    </div>`,a.assistantLoading||p().catch(()=>{});return}const i=F(),n=a.assistantData.config||{},o=a.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${T(n.nextCheckAt,{relative:!0})}`:"",f=o.enabled?` · no reply ${o.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${s(U(n.checkFrequency))}${s(r)}${s(f)}</div>
        </div>
  ...
  </div>`}function J(t){const e=s((t.name||"?").charAt(0).toUpperCase()),i=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      <div class="fda-avatar">${e}</div>
      <div class="fda-person-copy">
        <div class="fda-person-name">${s(t.name)}</div>
        <div class="fda-person-meta">${s(i)}</div>
      </div>
  ...
  </div>`}function Q(){const t=(a.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
  ...
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function K(){const t=a.assistantData;if(!t)return a.assistantError&&!a.assistantLoading?`<div class="fda-section">
        <div class="fda-section-title">Assistant could not connect</div>
        <div class="fda-section-sub">${s(a.assistantError)}</div>
        <button type="button" class="fda-btn primary full" onclick="retryFrontDeskAssistant()">Retry</button>
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},n=S(),o=t.capabilities||{},d=N(),r=Number(o.maxRecipients||3),f=n.length<r,u=d?"":"disabled",v=d&&o.manualAvailability?"":"disabled",P=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",L=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),y=[];o.smsConfigured||y.push("Marketel's texting number still needs its server credentials."),o.manualAvailability||y.push("The Assistant works with properties whose Availability is managed in Marketel.");const B=y.length?`<div class="fda-section fda-lock">
    ...
    ${!!e.enabled&&n.length>0?"":`<div class="fda-section fda-story">
      ...
      <div class="fda-bubble assistant"><strong>Front Desk</strong><br>New booking: ${s(_())}, tonight. Is it still free?</div>
    ...
    <div class="fda-section">
      ...
      ${n.length?n.map(J).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
  ...
  <div class="fda-sheet-body">${K()}</div>`,M())}function X(){document.getElementById("frontDeskAssistantSheet")&&g()}function M(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,i=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${i} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${i} minutes keeps the booking and emails the guest a confirmation.`}function tt(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=R(t.value))}function et(){if(!x()){window.openFrontdeskAppDownload?.();return}$(!1),E();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&k()}),document.body.appendChild(t)),document.body.style.overflow="hidden",g(),requestAnimationFrame(()=>$(!1)),a.assistantData?p({force:!0}).catch(()=>{}):p().catch(()=>{})}function it(){if(!x()){window.openFrontdeskAppDownload?.();return}et()}function k(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",$(!0)}function at(){a.assistantError="",g(),p({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(a.assistantData=t.data),w(),g(),e&&c(e,"success"),t}async function nt(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),i=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!i)return c("Enter a name and mobile number.","error");try{const n=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:i});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function ot(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(i){c(i.message||"That code could not be verified.","error")}}async function st(t){try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function rt(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function dt(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await l("PUT","/api/crm/frontdesk-assistant",t)),await l("POST","/api/crm/booking-approval",e),await p({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(i){c(i.message||"Could not save the assistant.","error")}}async function ct(){try{return b(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function lt(){try{b(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function ft(){const t=a.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const o=window.webkit?.messageHandlers?.marketelShell;if(o&&typeof o.postMessage=="function"){o.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),i=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=i,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(i),2e3)}async function pt(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){k(),c("Front Desk app access is managed with your Marketel account.","info");return}k();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const ut={activateFromAssistant:pt,addAssistantRecipient:nt,closeFrontDeskAssistant:k,loadFrontDeskAssistant:p,openFrontDeskAssistant:it,removeAssistantRecipient:rt,refreshFrontDeskAssistantSheet:X,renderFrontDeskAssistantCard:w,renderAssistantPill:G,hideAssistantPill:Z,retryFrontDeskAssistant:at,resendAssistantCode:st,runAssistantCheckNow:lt,saveAssistantContact:ft,saveAssistantSettings:dt,sendAssistantTest:ct,updateAssistantPolicySummary:M,updateAssistantTimeDisplay:tt,verifyAssistantRecipient:ot};function gt(){I||(I=!0,E(),q(ut))}export{pt as activateFromAssistant,nt as addAssistantRecipient,k as closeFrontDeskAssistant,Z as hideAssistantPill,gt as install,p as loadFrontDeskAssistant,it as openFrontDeskAssistant,X as refreshFrontDeskAssistantSheet,rt as removeAssistantRecipient,G as renderAssistantPill,w as renderFrontDeskAssistantCard,st as resendAssistantCode,at as retryFrontDeskAssistant,lt as runAssistantCheckNow,ft as saveAssistantContact,dt as saveAssistantSettings,ct as sendAssistantTest,M as updateAssistantPolicySummary,tt as updateAssistantTimeDisplay,ot as verifyAssistantRecipient};
