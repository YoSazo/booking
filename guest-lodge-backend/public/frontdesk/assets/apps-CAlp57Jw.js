import{c as g,e as it}from"./settings-DGHIPyBf.js";const l={homeScreen:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179630/unnamed_lbsctp.jpg",guestHome:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179605/IMG_2475_1_jxip3r.png",guestBook:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179611/IMG_2476_1_tqbmgz.png",guestMessagesImg:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179605/IMG_2477_1_zgodnn.png",frontdeskMessages:"https://res.cloudinary.com/dkmr3h5jb/image/upload/v1781179656/unnamed_2_qfhkrr.png",guestMessageNotifVideo:"https://res.cloudinary.com/dkmr3h5jb/video/upload/f_mp4,q_auto/ScreenRecording_06-10-2026_23-43-50_1_z22p4m.mp4",guestInstallVideo:"https://res.cloudinary.com/dkmr3h5jb/video/upload/f_mp4,q_auto/ScreenRecording_06-11-2026_00-03-19_1_lgdf59.mp4"},nt="32px";function b(e,t){return e.replace("/image/upload/","/image/upload/f_auto,q_auto,w_"+(t||400)+"/")}function y(e){return`border-radius:${nt};box-shadow:0 10px 36px rgba(0,0,0,0.22);${e||""}`}function F(e){const t=Math.min(window.devicePixelRatio||1,2),o=Math.round(Math.min(window.screen.width*t,1600));return e.replace("/image/upload/",`/image/upload/f_auto,q_auto:best,w_${o}/`)}let I=[],h=0;function st(e,t){w(!1),I=e,h=t||0;let o=document.getElementById("appsLightbox");if(!o){o=document.createElement("div"),o.id="appsLightbox",o.style.cssText=["position:fixed;inset:0;z-index:102000;background:#000;","display:flex;flex-direction:column;align-items:center;justify-content:flex-start;","overscroll-behavior:contain;touch-action:pan-y;","padding-left:env(safe-area-inset-left,0px);padding-right:env(safe-area-inset-right,0px);"].join(""),document.body.appendChild(o),document.body.style.overflow="hidden",o._keyHandler=i=>{i.key==="ArrowRight"||i.key==="ArrowDown"?$(1):i.key==="ArrowLeft"||i.key==="ArrowUp"?$(-1):i.key==="Escape"&&S()},document.addEventListener("keydown",o._keyHandler);let n=0;o.addEventListener("touchstart",i=>{n=i.changedTouches[0].clientX},{passive:!0}),o.addEventListener("touchend",i=>{const p=i.changedTouches[0].clientX-n;Math.abs(p)>50&&$(p<0?1:-1)},{passive:!0})}document.body.style.overflow="hidden",A()}function S(){const e=document.getElementById("appsLightbox");e&&(document.removeEventListener("keydown",e._keyHandler),e.remove(),document.body.style.overflow="")}function $(e){const t=I.length;t<=1||(h=(h+e+t)%t,A())}function A(){const e=document.getElementById("appsLightbox");if(!e)return;const t=I[h],o=I.length,n=t.type!=="video",i=o>1?`${h+1} / ${o}`:"",p=n?`<img src="${F(t.src)}" alt="${t.alt||""}"
          ...
          ${t.poster?`poster="${b(t.poster,400)}"`:""}>
    ...
    <div style="flex-shrink:0;padding:8px 20px max(20px,env(safe-area-inset-bottom));text-align:center;width:100%;max-width:420px;margin:0 auto;box-sizing:border-box;">
      ...
      ${s}
    </div>`}function j(e,t){const o=e||"Video";return`<span class="${"apps-media-badge"+(t==="light"?" apps-media-badge--light":"")}" title="Short video walkthrough"><span class="apps-media-badge__ring" aria-hidden="true"><span class="apps-media-badge__play" aria-hidden="true"></span></span><span class="apps-media-badge__label">${o}</span></span>`}function m(e,t,o,n,i){const p=i?j("Video"):"",a=i?'<span class="apps-q-media" aria-hidden="true"><span class="apps-q-media__play"></span></span>':'<span class="apps-q-chevron" aria-hidden="true">›</span>';return`<button type="button" class="apps-q${i?" apps-q--video":""}" onclick="appsOpenLightbox(${o},${n})">
    ...
    ${a}
      ...
      </div>`:`<button type="button" class="apps-story-primary" onclick="handleInstallFrontdesk()">${D}</button>`,P=a!=="#"?'<button type="button" class="apps-story-secondary" onclick="openGuestBookingEngine({focusInstall:true})">Go to direct booking page</button>':'<div class="apps-story-domain-note">Your direct booking domain is still setting up. Once it is ready, guests install from that page.</div>',B="width:56px;height:56px;border-radius:14px;flex-shrink:0;overflow:hidden;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 14px rgba(0,0,0,0.1);box-sizing:border-box;",Y=o?B+"background:#fff;border:1px solid var(--border);padding:0;":B,R=o?`<img src="${o}" alt="Hotel logo" style="width:100%;height:100%;object-fit:contain;">`:`<span style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:var(--green);color:#fff;border-radius:14px;font-size:24px;font-weight:800;">${n}</span>`,O=`
    <div class="apps-icon-card">
      ...
      ${a==="#"?'<p style="font-size:12px;color:var(--text-muted);margin:12px 0 0;">Your booking domain is still setting up.</p>':""}`,U=a!=="#"?`
      ...
      </div>`:'<div id="guestInstallStats" style="display:none;"></div>',W=`
    <div class="apps-loop">
    ...
    <section class="apps-story">
      ...
      <div class="apps-story-line">
        ...
        <div class="apps-story-actions">${P}</div>
      </div>
      ...
      <div class="apps-story-line">
        ...
        <div class="apps-q-list">
          ${m("What guests see on their phone","",d(r),0,!1)}
          ${m("How guests add your hotel","",d(f),0,!0)}
          ${m("Guest texts you, you text back","",d(C),0,!0)}
          ${m("Your app and theirs — side by side","",d(c),0,!1)}
        </div>
        ${p!=="#"?`<button onclick="window.open('${p}','_blank')" style="width:100%;padding:13px;border-radius:12px;border:1.5px solid var(--border);background:none;color:var(--text);font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;margin-top:4px;">Preview guest website ↗</button>`:""}`,K=z=>`
    ...
    <div class="apps-step-card"${z?' id="tour-guest-icon-section"':""}>
    ...
    ${Q}
    ${W}
    ${u?tt:L(!1)}`,ot=u?"Front Desk is installed. Guests can install your hotel from the direct booking page.":"Install Front Desk first. Then guests can install your hotel from the direct booking page.";e.innerHTML=`
  <style>
  ...
  <div class="apps-page">
    ...
    <div style="flex-shrink:0;width:100%;max-width:400px;margin:0 auto;padding:12px 20px max(16px,env(safe-area-inset-bottom));box-sizing:border-box;text-align:center;">
      ...
      <div style="display:flex;gap:6px;justify-content:center;">${d}</div>
    </div>`,document.getElementById("appsTourNextBtn").onclick=()=>{if(n){const c=v;if(w(!0),!c){const r=document.getElementById("appsView");r&&r.scrollIntoView({behavior:"smooth",block:"start"})}}else x++,H()},document.getElementById("appsTourSkipBtn").onclick=()=>w(!0)}function rt(e){const t=e&&e.replay,o=e&&e.chainFromSettingsTour;if(v=!!o,!t&&!o&&localStorage.getItem("appsTourDone")||document.getElementById("appsTourLightbox"))return;const n=g.activeHotelName||"Your Hotel",i=n.length>13?n.slice(0,13)+"…":n,p=n.trim().charAt(0).toUpperCase(),a=g.activeHotelAppIcon||"",d=a?`<div style="width:52px;height:52px;border-radius:14px;background:#fff;padding:8px;box-sizing:border-box;flex-shrink:0;display:flex;align-items:center;justify-content:center;"><img src="${a}" alt="${n}" style="width:100%;height:100%;object-fit:contain;"></div>`:`<div style="width:52px;height:52px;border-radius:14px;background:#2E7D5B;color:#fff;font-size:22px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${p}</div>`;_=[{type:"video",src:l.guestInstallVideo,poster:b(l.guestHome,400),title:"Guests save your hotel to their phone",caption:`They tap <strong>Add to Home Screen</strong> on your booking page or scan your QR. Tap <strong>Change your icon</strong> in Guest App so they see <strong>${i}</strong>.`},{type:"image",src:b(l.guestHome,520),alt:"Guest home screen",title:"What guests see after they install",caption:"Their stay info, direct booking, and a way to message you — all from one icon on their phone."},{type:"cta",title:"In Guest App",caption:"",ctaHtml:`
        ...
        </div>`}],x=0,S(),w(!1);const s=document.createElement("div");s.id="appsTourLightbox",s.style.cssText=["position:fixed;inset:0;z-index:102001;background:#000;","display:flex;flex-direction:column;","overscroll-behavior:contain;touch-action:pan-y;","padding-left:env(safe-area-inset-left,0px);padding-right:env(safe-area-inset-right,0px);"].join("");let c=0;s._swipeStart=r=>{c=r.changedTouches[0].clientX},s._swipeEnd=r=>{const f=r.changedTouches[0].clientX-c;Math.abs(f)>50&&M(f<0?1:-1)},s.addEventListener("touchstart",s._swipeStart,{passive:!0}),s.addEventListener("touchend",s._swipeEnd,{passive:!0}),document.body.appendChild(s),document.body.style.overflow="hidden",H()}const pt={appsCloseLightbox:S,appsCloudinaryFull:F,appsCloudinaryImg:b,appsLbNav:$,appsLbRender:A,appsOpenLightbox:st,appsPhoneImgStyle:y,appsQuestionRow:m,appsTourClose:w,appsTourNav:M,appsTourRender:H,appsVideoBadgeHtml:j,detectAppPlatform:at,ensureAppsViewRendered:T,loadGuestInstallStats:E,renderAppsView:G,startAppsTour:rt};function ut(){it(pt)}export{pt as default,ut as install};
