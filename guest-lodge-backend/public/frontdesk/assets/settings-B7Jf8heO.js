const s={token:"",bookings:[],guestMessages:[],currentFilter:"settings",bookingCallFilter:"all",manualAvailability:{rooms:[],overrides:{}},manualSelectedRoom:"",availabilityYear:new Date().getFullYear(),availabilityMonth:new Date().getMonth(),availabilityEditingDay:"",availabilityDaySaving:!1,editingRoomName:"",pendingDeleteRoomName:"",currentHotelPms:"",revenueEnabled:!1,hotelSubscribed:!1,revenuePeriod:"30d",revenueCache:{},revenueLoading:!1,revenueError:"",blockedDemand:{total:0,today:0,recent:[]},bookingsSubview:"bookings",launchStatus:null,growthFunnel:null,growthChecklist:{},growthPeriod:"30d",ALLOWED_REVENUE_PERIODS:new Set(["today","7d","30d","all"]),OTA_COMMISSION_RATE:.25,activeHotelId:"",activeHotelName:"",activeHotelAppIcon:"",appsViewPlatform:"ios",activeHotelDomain:"",activeHotelContext:null,settingsTourActive:!1,bootInFlight:!1,CRM_HOTEL_BY_HOST:{"guestlodgeminot.clickinns.com":"guest-lodge-minot","booking-kappa-nine.vercel.app":"guest-lodge-minot","stcroix.clickinns.com":"st-croix-wisconsin","homeplacesuites.clickinns.com":"home-place-suites","myhomeplacesuites.com":"home-place-suites","www.myhomeplacesuites.com":"home-place-suites","suitestay.clickinns.com":"suite-stay","clickinns.com":"suite-stay","www.clickinns.com":"suite-stay"},CRM_HOTEL_LABELS:{"guest-lodge-minot":"Guest Lodge Minot","st-croix-wisconsin":"St. Croix Wisconsin","home-place-suites":"Home Place Suites","suite-stay":"Suite Stay"},deferredInstallPrompt:null,frontdeskInstalled:!1,_magicLoginPending:!1,editRooms:[],editRoomsLoadPromise:null,messageUnreadCount:0,messagesInboxOpen:!1,messagesThreadPickerOpen:!1,selectedMessageThread:"",bookingsVirtualList:[],bookingsVirtualRaf:0};let W=null;function vt(){return typeof lucide<"u"?Promise.resolve():W||(W=new Promise((e,t)=>{const o=document.createElement("script");o.src="https://unpkg.com/lucide@0.469.0/dist/umd/lucide.min.js",o.async=!0,o.onload=()=>e(),o.onerror=()=>t(new Error("lucide load failed")),document.head.appendChild(o)}),W)}async function Le(e){if(!e||!e.type.startsWith("image/")||e.type==="image/webp"&&e.size<4e5)return e;try{const t=await createImageBitmap(e),o=1600,i=1200;let n=t.width,r=t.height;const a=Math.min(1,o/n,i/r);n=Math.round(n*a),r=Math.round(r*a);const d=document.createElement("canvas");d.width=n,d.height=r,d.getContext("2d").drawImage(t,0,0,n,r),t.close();const u=await new Promise((h,v)=>{d.toBlob(S=>S?h(S):v(new Error("encode failed")),"image/webp",.82)}),f=(e.name||"room-photo").replace(/\.[^.]+$/,"")||"room-photo";return new File([u],f+".webp",{type:"image/webp"})}catch{return e}}function wt(){const e=()=>{s.currentFilter==="bookings"?loadMessages():loadMessageBadges()};"requestIdleCallback"in window?requestIdleCallback(e,{timeout:2500}):setTimeout(e,600)}function Re(e){Object.assign(window,e)}async function be(){const e=document.getElementById("settingsList");if(e){e.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const t=await api("GET","/api/crm/verify"),i="https://"+(t?.domain||s.activeHotelId+".mktel.co"),n="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(i),r=await api("GET","/api/crm/rooms");let a={nightly:69,weekly:299,monthly:999};r?.rates&&(a=r.rates);const d=r?.rooms||[];let u="";t?.subscribed||(u+=goLiveInlineCardHtml()),d.length?d.forEach(f=>{const h=f.images&&f.images.length>0;u+=`
    ...
    `,e.innerHTML=u}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load settings</div></div>'}}}function He(){const e=document.getElementById("settings-booking-url");e&&navigator.clipboard.writeText(e.value).then(()=>{localStorage.setItem("linkCopied","1"),N(),toast("Link copied!","success")}).catch(()=>toast("Copy failed","error"))}function Fe(){localStorage.setItem("settingsTourDone","1");const e=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",e);let t=0;const o=setInterval(()=>{t++;const i=document.getElementById("edit-rate-nightly");if(i||t>20){if(clearInterval(o),!i)return;const n=i.closest(".accordion-body");if(n&&n.style.display==="none"){n.style.display="block";const r=n.previousElementSibling?.querySelector(".accordion-arrow");r&&(r.style.transform="rotate(90deg)")}setTimeout(()=>{i.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const r=document.getElementById("checklistPointer");r&&r.remove();const a=i.getBoundingClientRect(),d=document.createElement("div");d.id="checklistPointer",d.style.cssText=`position:fixed;z-index:100000;left:50%;transform:translateX(-50%);top:${a.bottom+12}px;max-width:240px;width:calc(100% - 40px);`,d.innerHTML=`
      ...
      `,document.body.appendChild(a),a.querySelectorAll(".onboarding-opt").forEach(d=>{d.addEventListener("click",()=>{t[r.key]=d.dataset.value,d.style.background="#1a1a2e",d.style.color="white",d.style.fontWeight="600",setTimeout(()=>{e++,i()},250)})}))}i()}function _e(){["onboardingDone","settingsTourDone","settingsTourStep","linkCopied","ratesChanged","appsTourDone","postActivationTourDone"].forEach(o=>{localStorage.removeItem(o)});const e=new URL(window.location.href);e.searchParams.set("welcome","1");const t=e.searchParams.get("hotelId")||s.activeHotelId||"";t&&e.searchParams.set("hotelId",t),location.href=e.pathname+"?"+e.searchParams.toString()}function we(){const e=document.createElement("div");e.id="welcomeModalOverlay",e.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.72);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:24px;";function t(){localStorage.setItem("onboardingDone","1"),localStorage.removeItem("settingsTourDone"),localStorage.removeItem("settingsTourStep");try{const n=new URL(window.location);n.searchParams.delete("welcome"),window.history.replaceState({},"",n)}catch{}const i=typeof ae=="function"?ae:typeof window.startSettingsTour=="function"?window.startSettingsTour:null;i&&i(),e.remove()}function o(){e.innerHTML=`
  ...
  `,document.body.appendChild(o),document.getElementById("activatedModalTour").onclick=()=>{o.remove(),ce()},document.getElementById("activatedModalSkip").onclick=()=>{o.remove(),localStorage.setItem("postActivationTourDone","1");try{setFilter("bookings")}catch{}}}function ke(){return isStandaloneApp()||s.frontdeskInstalled?pushSupported()?typeof Notification<"u"&&Notification.permission==="granted"?"":ne():"":ne()}function ne(){const e=isStandaloneApp()||s.frontdeskInstalled,t=typeof Notification<"u"&&Notification.permission==="granted";let o;return e&&pushSupported()?o=t?`<p style="font-size:12px;color:var(--text-muted);margin:0 0 14px;line-height:1.5;">You're all set — you'll get an alert on this phone every time a guest books.</p>
    ...
    <div style="background:white;border-radius:20px;max-width:340px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
        ...
        ${b}
      </div>`;if(p.tooltipAnchor==="card-bottom"||p.tooltipAnchor==="card-top"){const j=C(p.highlightSelector)||l,fe=C(p.anchorSelector);if(j){const P=j.getBoundingClientRect(),q=fe?fe.getBoundingClientRect():null,T=Math.min(280,window.innerWidth-32),ye=P.left+P.width/2,R=Math.max(16,Math.min(ye-T/2,window.innerWidth-T-16)),U=Math.max(20,Math.min(ye-R,T-20)),V=132;if(p.tooltipAnchor==="card-bottom"){const oe=window.innerHeight-P.bottom,he=Math.min(P.bottom+12,window.innerHeight-V-16);if(oe>=V+20)g.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${he}px;max-width:${T}px;width:${T}px;`,g.innerHTML=`
              ...
              ${x}`;else{const ie=q?q.top:P.top;g.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-ie+10}px;max-width:${T}px;width:${T}px;`,g.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`}}else{const oe=P.top;if(P.top-16>=V+24)g.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-oe+10}px;max-width:${T}px;width:${T}px;`,g.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`;else{const ie=Math.min(P.top+(q?q.height:48)+12,window.innerHeight-V-16);g.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${ie}px;max-width:${T}px;width:${T}px;`,g.innerHTML=`
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:8px solid #1a1a2e;margin-left:${U-8}px;"></div>
              ${x}`}}document.body.appendChild(g),v();return}}const m=y||Q(p,l);if(!m){g.style.cssText="position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:280px;width:calc(100% - 32px);",g.innerHTML=x,document.body.appendChild(g),v();return}const k=window.innerHeight-m.bottom,z=m.top,E=Math.min(280,window.innerWidth-32),I=m.left+m.width/2,B=Math.max(16,Math.min(I-E/2,window.innerWidth-E-16)),$=Math.max(20,Math.min(I-B,E-20));let A=p.tooltipPosition||(k>150?"below":z>150?"above":"center");if(A==="below"&&k<130&&z>k&&(A="above"),A==="above"&&z<130&&k>z&&(A="below"),A==="below"&&m.bottom+150>window.innerHeight&&(A="above"),A==="below"){const j=Math.min(m.bottom+8,window.innerHeight-24);g.style.cssText=`position:fixed;z-index:100000;left:${B}px;top:${j}px;max-width:${E}px;width:${E}px;`,g.innerHTML=`
