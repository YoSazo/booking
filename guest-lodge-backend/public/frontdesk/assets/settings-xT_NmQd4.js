const p={token:"",bookings:[],guestMessages:[],currentFilter:"settings",manualAvailability:{rooms:[],overrides:{}},manualSelectedRoom:"",availabilityYear:new Date().getFullYear(),availabilityMonth:new Date().getMonth(),availabilityEditingDay:"",availabilityDaySaving:!1,editingRoomName:"",pendingDeleteRoomName:"",currentHotelPms:"",revenueEnabled:!1,hotelSubscribed:!1,revenuePeriod:"30d",revenueCache:{},revenueLoading:!1,revenueError:"",ALLOWED_REVENUE_PERIODS:new Set(["today","7d","30d","all"]),OTA_COMMISSION_RATE:.25,activeHotelId:"",activeHotelName:"",activeHotelAppIcon:"",appsViewPlatform:"ios",activeHotelDomain:"",activeHotelContext:null,bootInFlight:!1,CRM_HOTEL_BY_HOST:{"guestlodgeminot.clickinns.com":"guest-lodge-minot","booking-kappa-nine.vercel.app":"guest-lodge-minot","stcroix.clickinns.com":"st-croix-wisconsin","homeplacesuites.clickinns.com":"home-place-suites","myhomeplacesuites.com":"home-place-suites","www.myhomeplacesuites.com":"home-place-suites","suitestay.clickinns.com":"suite-stay","clickinns.com":"suite-stay","www.clickinns.com":"suite-stay"},CRM_HOTEL_LABELS:{"guest-lodge-minot":"Guest Lodge Minot","st-croix-wisconsin":"St. Croix Wisconsin","home-place-suites":"Home Place Suites","suite-stay":"Suite Stay"},deferredInstallPrompt:null,frontdeskInstalled:!1,_magicLoginPending:!1,editRooms:[],messageUnreadCount:0,messagesInboxOpen:!1,messagesThreadPickerOpen:!1,selectedMessageThread:"",bookingsVirtualList:[],bookingsVirtualRaf:0};let P=null;function Ce(){return typeof lucide<"u"?Promise.resolve():P||(P=new Promise((e,t)=>{const o=document.createElement("script");o.src="https://unpkg.com/lucide@0.469.0/dist/umd/lucide.min.js",o.async=!0,o.onload=()=>e(),o.onerror=()=>t(new Error("lucide load failed")),document.head.appendChild(o)}),P)}async function F(e){if(!e||!e.type.startsWith("image/")||e.type==="image/webp"&&e.size<4e5)return e;try{const t=await createImageBitmap(e),o=1600,a=1200;let i=t.width,n=t.height;const d=Math.min(1,o/i,a/n);i=Math.round(i*d),n=Math.round(n*d);const l=document.createElement("canvas");l.width=i,l.height=n,l.getContext("2d").drawImage(t,0,0,i,n),t.close();const u=await new Promise((m,w)=>{l.toBlob(b=>b?m(b):w(new Error("encode failed")),"image/webp",.82)}),x=(e.name||"room-photo").replace(/\.[^.]+$/,"")||"room-photo";return new File([u],x+".webp",{type:"image/webp"})}catch{return e}}function De(){const e=()=>{p.currentFilter==="bookings"?loadMessages():loadMessageBadges()};"requestIdleCallback"in window?requestIdleCallback(e,{timeout:2500}):setTimeout(e,600)}function Q(e){Object.assign(window,e)}async function N(){const e=document.getElementById("settingsList");if(e){e.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const t=await api("GET","/api/crm/verify"),a="https://"+(t?.domain||p.activeHotelId+".bookmarketel.com"),i="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(a),n=await api("GET","/api/crm/rooms");let d={nightly:69,weekly:299,monthly:999};n?.rates&&(d=n.rates);const l=n?.rooms||[];let u="";l.length?l.forEach(x=>{const m=x.images&&x.images.length>0;u+=`
    ...
    `,e.innerHTML=u}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load settings</div></div>'}}}function J(){const e=document.getElementById("settings-booking-url");e&&navigator.clipboard.writeText(e.value).then(()=>{localStorage.setItem("linkCopied","1"),S(),toast("Link copied!","success")}).catch(()=>toast("Copy failed","error"))}function K(){localStorage.setItem("settingsTourDone","1");const e=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",e);let t=0;const o=setInterval(()=>{t++;const a=document.getElementById("edit-rate-nightly");if(a||t>20){if(clearInterval(o),!a)return;const i=a.closest(".accordion-body");if(i&&i.style.display==="none"){i.style.display="block";const n=i.previousElementSibling?.querySelector(".accordion-arrow");n&&(n.style.transform="rotate(90deg)")}setTimeout(()=>{a.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const n=document.getElementById("checklistPointer");n&&n.remove();const d=a.getBoundingClientRect(),l=document.createElement("div");l.id="checklistPointer",l.style.cssText=`position:fixed;z-index:100000;left:50%;transform:translateX(-50%);top:${d.bottom+12}px;max-width:240px;width:calc(100% - 40px);`,l.innerHTML=`
          ...
          `,document.body.appendChild(l),setTimeout(()=>{const u=document.getElementById("checklistPointer");u&&u.remove()},6e3)},1e3)},100)}},200)}function I(e){updatePropertyStatusStrip(),applySetupStepFocus();const t=e==="photo"?document.getElementById("launchStepRates"):e==="rates"?document.querySelector('[data-launch-step="link"]'):null;t&&setTimeout(()=>t.scrollIntoView({behavior:"smooth",block:"start"}),400)}function X(){const t="https://"+(p.activeHotelDomain||p.activeHotelId+".bookmarketel.com");navigator.clipboard.writeText(t).then(()=>{localStorage.setItem("linkCopied","1"),S(),I("link"),toast("Link copied!","success"),loadBookings()}).catch(()=>toast("Copy failed","error"))}function Z(e,t){localStorage.setItem("settingsTourDone","1");const o=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",o);let a=0;const i=setInterval(()=>{a++;const n=document.querySelector(e);if(n||a>20){if(clearInterval(i),!n)return;n.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const d=document.getElementById("checklistPointer");d&&d.remove();const l=n.getBoundingClientRect(),u=document.createElement("div");u.id="checklistPointer",u.style.cssText=`
        ...
        `,document.body.appendChild(u),setTimeout(()=>{const x=document.getElementById("checklistPointer");x&&x.remove()},6e3)},1e3)}},200)}async function ee(e,t){const o=e.target.files[0];if(!o)return;const a=await F(o),i=new FormData;i.append("image",a,a.name||"room.webp");try{const d=await(await fetch(`/api/crm/rooms/${t}/images?hotelId=${encodeURIComponent(i.activeHotelId)}`,{method:"POST",headers:{"x-crm-token":i.token},body:i})).json();d.success?(toast("Photo uploaded!","success"),N()):toast(d.message||"Upload failed","error")}catch{toast("Upload failed","error")}}async function te(){const e=parseFloat(document.getElementById("settings-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("settings-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("settings-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),toast("Rates saved","success")}catch{toast("Failed to save rates","error")}}async function oe(){const e=document.getElementById("settings-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),p.token=e;try{localStorage.setItem("crmToken",p.token)}catch{}toast("PIN updated!","success"),document.getElementById("settings-new-pin").value=""}catch{toast("Failed to change PIN","error")}}async function ie(){const e=document.getElementById("settings-support-msg")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),toast("Message sent!","success"),document.getElementById("settings-support-msg").value=""}catch{toast("Failed to send","error")}}function ne(){const e=p.activeHotelDomain||p.activeHotelId+".bookmarketel.com",o=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:5173/?hotelId="+encodeURIComponent(p.activeHotelId)+"&preview=1":"https://"+e+"?preview=1";window.open(o,"_blank")}function re(){const e=document.getElementById("previewSiteBar");e&&(e.style.display=p.currentFilter==="settings"?"block":"none")}function S(){if(localStorage.getItem("settingsTourDone"))return;const e=parseInt(localStorage.getItem("settingsTourStep")||"0"),t=p.editRooms.some(d=>d.images&&d.images.length>0),o=!!localStorage.getItem("ratesChanged"),a=!!localStorage.getItem("linkCopied");e===1&&t&&localStorage.setItem("settingsTourStep","2"),e===2&&o&&localStorage.setItem("settingsTourStep","3"),e===3&&a&&localStorage.setItem("settingsTourStep","4");const i=document.getElementById("tourTooltip");i&&i.remove();const n=document.getElementById("tourBlurOverlay");n&&n.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(d=>{d.style.position=d.dataset.tourOrigPosition||"",d.style.zIndex="",d.style.boxShadow="",d.removeAttribute("data-tour-highlighted"),delete d.dataset.tourOrigPosition}),document.body.style.overflow=""}function ae(){let e=0;const t={},o=[{title:"Why do you want a booking engine?",key:"why",type:"text",placeholder:"e.g. I want guests to book directly instead of calling me..."},{title:"How do guests currently book with you?",key:"currentBooking",type:"choice",options:[{label:"They call me or walk in",value:"phone_walkin"},{label:"Through Booking.com / Expedia",value:"ota"},{label:"I have a website but no booking system",value:"website_no_booking"},{label:"I don't take bookings online yet",value:"no_online"}]},{title:"How many rooms do you have?",key:"roomCount",type:"choice",options:[{label:"1–5 rooms",value:"1-5"},{label:"6–15 rooms",value:"6-15"},{label:"16–50 rooms",value:"16-50"},{label:"50+ rooms",value:"50+"}]},{title:"What's most important to you?",key:"priority",type:"choice",options:[{label:"Stop paying OTA commissions",value:"no_commission"},{label:"Get more direct bookings",value:"more_bookings"},{label:"Have a professional online presence",value:"professional"},{label:"Make it easier for guests to book",value:"easier_booking"}]}];function a(){let i=document.getElementById("onboardingOverlay");if(i&&i.remove(),e>=o.length){localStorage.setItem("onboardingDone","1");try{api("POST","/api/crm/onboarding-answers",t).catch(()=>{})}catch{}H();return}const n=o[e],d=document.createElement("div");d.id="onboardingOverlay",d.style.cssText="position:fixed;inset:0;z-index:100001;background:linear-gradient(135deg, #1a2b22 0%, #2E7D5B 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;",n.type==="text"?(d.innerHTML=`
      ...
      `,document.body.appendChild(d),document.getElementById("onboardingTextSubmit").onclick=()=>{const l=document.getElementById("onboardingTextInput").value.trim();l&&(t[n.key]=l,e++,a())}):(d.innerHTML=`
        <div style="max-width:360px;width:100%;text-align:center;">
          <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-bottom:8px;">Question ${e+1} of ${o.length}</div>
          <h2 style="font-size:20px;font-weight:700;color:white;margin:0 0 24px;line-height:1.3;">${n.title}</h2>
      ...
      `,document.body.appendChild(d),d.querySelectorAll(".onboarding-opt").forEach(l=>{l.addEventListener("click",()=>{t[n.key]=l.dataset.value,l.style.background="#1a1a2e",l.style.color="white",l.style.fontWeight="600",setTimeout(()=>{e++,a()},250)})}))}a()}function H(){const e=document.createElement("div");e.id="welcomeModalOverlay",e.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px;",e.innerHTML=`
  ...
  `,document.body.appendChild(t),document.getElementById("activatedModalDone").onclick=()=>{t.remove();try{setFilter("bookings")}catch{}}}function R(){const e=isStandaloneApp()||p.frontdeskInstalled,t=typeof Notification<"u"&&Notification.permission==="granted";let o;return e&&pushSupported()?o=t?`<p style="font-size:12px;color:var(--text-muted);margin:0 0 14px;line-height:1.5;">You're all set — you'll get an alert on this phone every time a guest books.</p>
    ...
    <div id="iosInstallSheetCard" style="background:#fff;width:100%;max-width:440px;border-radius:20px 20px 0 0;padding:24px 22px 32px;box-shadow:0 -8px 40px rgba(0,0,0,0.2);">
      ...
      <button id="iosInstallSheetDone" style="width:100%;margin-top:22px;padding:14px;border-radius:12px;border:none;background:#2E7D5B;color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;">Got it</button>
    </div>`,document.body.appendChild(n),typeof lucide<"u"&&lucide.createIcons();const l=()=>n.remove();n.addEventListener("click",m=>{m.target===n&&l()});const u=document.getElementById("iosInstallSheetCard");u&&u.addEventListener("click",m=>m.stopPropagation());const x=document.getElementById("iosInstallSheetDone");x&&(x.onclick=l)}function D(){if(localStorage.getItem("settingsTourDone"))return;let e=parseInt(localStorage.getItem("settingsTourStep")||"0");const t=[{target:"",text:"",openAccordion:!1,tab:"settings",customModal:"homescreen"},{target:"#edit-hotel-phone",text:"Tap any field here to edit your hotel info — name, address, phone.",openAccordion:!1,tab:"settings"},{target:"#editRoomsCards .booking-card > div:first-child",text:"Add a photo of your room — guests book more when they see one.",openAccordion:!1,tab:"settings"},{target:"#edit-rate-nightly",text:"You can change your rates here — nightly, weekly, and monthly.",openAccordion:!0,targetParent:!0,tab:"settings"},{target:'button[onclick*="copyBookingLink"]',text:"This is your booking URL — the link guests use to book with you directly.",openAccordion:!1,tab:"settings"},{target:'button[onclick*="openPreviewSite"]',text:"Preview your booking engine to see exactly what your guests see.",openAccordion:!1,tab:"settings"},{target:"#edit-new-pin",text:"This is your login PIN — you'll need it to access this dashboard next time. You can change it here anytime.",openAccordion:!0,tab:"settings"},{target:"#bookingsList",text:"",openAccordion:!1,tab:"bookings",customModal:!0},{target:"#availabilityCalendarWrap",text:"",openAccordion:!1,tab:"availability",customModal:"availability"},{target:".revenue-savings-pill",text:"See how much revenue your booking page generates and how much you're saving vs OTA commissions.",openAccordion:!1,tab:"revenue",waitForVisible:!0},{target:"#paymentsExplainer",text:"When guests book through your engine, their card is verified but never charged. This prevents no-shows — and you collect payment however you want when they physically check in.",openAccordion:!1,tab:"revenue",waitForVisible:!0,tooltipPosition:"above"},{target:"#bookingsList",text:"",openAccordion:!1,tab:"bookings",customModal:"finale"}];function o(){const r=document.getElementById("tourTooltip");r&&r.remove();const s=document.getElementById("tourBlurOverlay");s&&s.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(c=>{c.style.position=c.dataset.tourOrigPosition||"",c.style.zIndex="",c.style.boxShadow="",c.removeAttribute("data-tour-highlighted"),delete c.dataset.tourOrigPosition}),document.body.style.overflow=""}function a(){o(),x()}function i(){if(o(),e>=t.length){localStorage.setItem("settingsTourDone","1"),localStorage.removeItem("settingsTourStep");return}const r=t[e];if(r.tab==="revenue"&&!p.revenueEnabled){e++,localStorage.setItem("settingsTourStep",String(e)),i();return}if(r.tab&&r.tab!==p.currentFilter){const s=document.querySelector(`.tab[data-nav-filter="${r.tab}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${r.tab}"]`);s&&setFilter(r.tab,s),r.customModal?setTimeout(()=>{n(r)},150):setTimeout(()=>{n(r)},800);return}setTimeout(()=>{n(r)},100)}function n(r){if(r.customModal==="homescreen"){u();return}if(r.customModal===!0||r.customModal==="bookings"){b();return}if(r.customModal==="availability"){w();return}if(r.customModal==="finale"){x();return}if(r.waitForVisible){const g=r.target.split(",").map(f=>f.trim());let h=0;const v=20,y=()=>{h++;let f=null;for(const k of g){if(f=document.querySelector(k),f&&f.offsetParent!==null)break;f=null}f?d(f,r):h<v?setTimeout(y,200):(e++,localStorage.setItem("settingsTourStep",String(e)),i())};y();return}let s=null;const c=r.target.split(",").map(g=>g.trim());for(const g of c){if(s=document.querySelector(g),s&&!r.openAccordion&&s.offsetParent===null&&getComputedStyle(s).position!=="fixed"){s=null;continue}if(s)break}if(!s){e++,localStorage.setItem("settingsTourStep",String(e)),i();return}d(s,r)}function d(r,s){if(s.openAccordion){const g=r.closest(".accordion-body");if(g&&g.style.display==="none"){g.style.display="block";const h=g.previousElementSibling?.querySelector(".accordion-arrow");h&&(h.style.transform="rotate(90deg)")}}s.targetParent&&(r=r.closest(".booking-card")||r.closest(".accordion-body")||r);const c=document.createElement("div");c.id="tourBlurOverlay",c.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.6);",document.body.appendChild(c),document.body.style.overflow="hidden",r.dataset.tourOrigPosition||(r.dataset.tourOrigPosition=r.style.position||""),r.style.position=r.style.position||"relative",r.style.zIndex="99999",r.style.isolation="isolate",r.style.boxShadow="0 0 0 4px #2E7D5B, 0 0 20px rgba(46,125,91,0.3)",r.setAttribute("data-tour-highlighted","1"),r.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{l(r,s)},500)}function l(r,s){const c=r.getBoundingClientRect(),g=document.createElement("div");g.id="tourTooltip";const h=window.innerHeight-c.bottom,v=c.top,y=Math.min(280,window.innerWidth-32),f=c.left+c.width/2,k=Math.max(16,Math.min(f-y/2,window.innerWidth-y-16)),T=Math.max(20,Math.min(f-k,y-20)),E='<div style="margin-top:8px;"><button id="tourSkipBtn" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:11px;font-family:inherit;cursor:pointer;padding:4px 8px;">Skip tour</button></div>',z=s.tooltipPosition||(h>130?"below":v>130?"above":"center");z==="below"?(g.style.cssText=`position:fixed;z-index:100000;left:${k}px;top:${c.bottom+12}px;max-width:${y}px;width:${y}px;`,g.innerHTML=`
        ...
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          ...
          ${E}
        </div>`):z==="above"?(g.style.cssText=`position:fixed;z-index:100000;left:${k}px;bottom:${window.innerHeight-c.top+12}px;max-width:${y}px;width:${y}px;`,g.innerHTML=`
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          <p style="margin:0 0 12px;">${s.text}</p>
          <button id="tourNextBtn" style="padding:8px 20px;border-radius:6px;border:none;background:#2E7D5B;color:white;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;">${e<t.length-1?"Next →":"Got it!"}</button>
          ${E}
        </div>
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${T-8}px;"></div>`):(g.style.cssText=`position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:${y}px;width:${y}px;`,g.innerHTML=`
        <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
          <p style="margin:0 0 12px;">${s.text}</p>
          <button id="tourNextBtn" style="padding:8px 20px;border-radius:6px;border:none;background:#2E7D5B;color:white;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;">${e<t.length-1?"Next →":"Got it!"}</button>
          ${E}
        </div>`),document.body.appendChild(g),document.getElementById("tourNextBtn").onclick=()=>{o(),e++,localStorage.setItem("settingsTourStep",String(e)),i()},document.getElementById("tourSkipBtn").onclick=()=>{a()}}function u(){const r=document.createElement("div");r.id="tourBlurOverlay",r.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(r),document.body.style.overflow="hidden";const s=p.activeHotelName||"Your Hotel",c=s.trim().charAt(0).toUpperCase(),g=s.length>10?s.slice(0,10):s,h="width:32px;display:flex;flex-direction:column;align-items:center;gap:5px;",v="width:32px;height:32px;border-radius:9px;box-sizing:border-box;",y="height:8px;max-width:46px;line-height:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;",f=`<div style="${h}"><div style="${v}background:rgba(255,255,255,0.22);"></div><div style="${y}"></div></div>`,k=`<div style="${h}"><div style="${v}background:#fff;color:#2E7D5B;font-weight:800;font-size:15px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);">${c}</div><div style="${y}font-size:7.5px;color:#fff;font-weight:700;">${g}</div></div>`,T=[f,f,f,f,k,f,f,f].join(""),E=document.createElement("div");if(E.id="tourTooltip",E.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:20px 16px;",E.innerHTML=`
      ...
      </div>`,document.body.appendChild(s),document.getElementById("activateNowBtn").onclick=()=>{s.remove();const c=p.token;fetch("/api/crm/go-live",{method:"POST",headers:{"Content-Type":"application/json","x-crm-token":c},body:JSON.stringify({hotelId:p.activeHotelId})}).then(g=>g.json()).then(g=>{g.success&&g.url?window.location.href=g.url:toast("Something went wrong. Try again.","error")}).catch(()=>toast("Something went wrong. Try again.","error"))},document.getElementById("activateLaterBtn").onclick=()=>{s.remove();const c=document.querySelector('.tab[data-nav-filter="bookings"]')||document.querySelector('.mobile-nav-item[data-nav-filter="bookings"]');c&&setFilter("bookings",c)}}function w(){const r=document.createElement("div");r.id="tourBlurOverlay",r.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(r),document.body.style.overflow="hidden";let s=0;const c=[`<div style="padding:20px 18px 0;">
      ...
      <div style="padding:0 14px 14px;">
      ...
      </div>`],g=document.createElement("div");g.id="tourTooltip",g.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;";function h(){const y=s>=c.length-1?"Next — Revenue →":"Next →";g.innerHTML=`
        ...
        </div>`,document.getElementById("tourNextBtn").onclick=()=>{s<c.length-1?(s++,h()):(o(),e++,localStorage.setItem("settingsTourStep",String(e)),i())},document.getElementById("tourSkipBtn").onclick=()=>{a()}}if(document.body.appendChild(g),h(),!document.getElementById("tourModalAnimStyle")){const v=document.createElement("style");v.id="tourModalAnimStyle",v.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(v)}}function b(){const r=document.createElement("div");r.id="tourBlurOverlay",r.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(r),document.body.style.overflow="hidden";const s=document.createElement("div");if(s.id="tourTooltip",s.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;",s.innerHTML=`
      <div style="background:white;border-radius:20px;max-width:340px;width:100%;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      <div data-setup-focus="rates" class="booking-card" id="launchStepRates" style="margin-bottom:14px;">
        <div style="padding:14px 18px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;" onclick="toggleSection(this)">
          ...
          <span style="font-size:18px;color:var(--text-muted);transition:transform 0.2s;transform:rotate(${b?"90deg":"0deg"});" class="accordion-arrow">›</span>
        </div>
        <div class="accordion-body" style="display:${b?"block":"none"};padding:0 18px 18px;">
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px;">
            <div>
              <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--text-muted);margin-bottom:4px;">Nightly</div>
      ...
      <div data-setup-focus="extra" class="booking-card" style="margin-bottom:14px;">
        ...
        <div class="accordion-body" style="display:none;padding:0 18px 18px;">
          ...
          <div style="margin-bottom:12px;">
            <input type="text" id="edit-new-pin" value="${p.token}" placeholder="New PIN (4+ characters)" style="width:100%;font-size:16px;padding:8px 10px;border-radius:8px;border:1.5px solid var(--border);font-family:inherit;outline:none;text-align:center;letter-spacing:2px;">
          </div>
    ...
    `;e.innerHTML=r,B(),updatePropertyStatusStrip(),applySetupStepFocus(),typeof lucide<"u"&&lucide.createIcons(),setTimeout(()=>{p.currentFilter==="settings"&&localStorage.getItem("onboardingDone")&&!localStorage.getItem("settingsTourDone")&&!document.getElementById("welcomeModalOverlay")&&D()},300)}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>'}}}function A(){B()}function B(){const e=document.getElementById("editRoomsCards")||document.getElementById("editRoomsList");if(!p.editRooms.length){e.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>';return}e.innerHTML=p.editRooms.map(t=>{const o=(t.amenities||"").split("•").map(i=>i.trim()).filter(Boolean),a=t.images||[];return`
    ...
    `),document.getElementById("amenityPickerModal").addEventListener("click",M),a=document.getElementById("amenityPickerModal"));const i=document.getElementById("amenityPickerGrid");i.innerHTML=G.map(n=>{const d=o.some(l=>l.includes(n.key));return`<button onclick="toggleAmenityPreset(this,'${n.key}')" data-key="${n.key}" style="display:inline-flex;align-items:center;gap:5px;padding:7px 12px;border-radius:8px;border:1.5px solid ${d?"#2E7D5B":"#e5e7eb"};background:${d?"#E8F5EE":"white"};color:${d?"#2E7D5B":"#1a1a2e"};font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;"><i data-lucide="${n.icon}" style="width:14px;height:14px;"></i> ${n.label}</button>`}).join(""),document.getElementById("amenityCustomInput").value="",a.style.display="flex",typeof lucide<"u"&&lucide.createIcons()}function ce(e,t){const o=e.style.borderColor==="rgb(46, 125, 91)";e.style.borderColor=o?"#e5e7eb":"#2E7D5B",e.style.background=o?"white":"#E8F5EE",e.style.color=o?"#1a1a2e":"#2E7D5B"}function M(){document.getElementById("amenityPickerModal").style.display="none",L=null}function ge(){const e=p.editRooms.find(i=>i.id===L);if(!e){M();return}const t=document.getElementById("amenityPickerGrid"),o=[];t.querySelectorAll("button").forEach(i=>{if(i.style.background==="rgb(232, 245, 238)"){const n=G.find(d=>d.key===i.dataset.key);n&&o.push(n.label)}});const a=document.getElementById("amenityCustomInput").value.trim();a&&o.push(a),e.amenities=o.join(" • "),M(),A(),typeof lucide<"u"&&lucide.createIcons()}function ue(e){Y(e)}function xe(e,t){const o=p.editRooms.find(i=>i.id===e);if(!o)return;const a=(o.amenities||"").split("•").map(i=>i.trim()).filter(Boolean);o.amenities=a.filter(i=>i!==t).join(" • "),A(),typeof lucide<"u"&&lucide.createIcons()}async function me(){const e=document.getElementById("edit-hotel-name")?.value.trim(),t=document.getElementById("edit-hotel-subtitle")?.value.trim(),o=document.getElementById("edit-hotel-address")?.value.trim(),a=document.getElementById("edit-hotel-phone")?.value.trim(),i=document.getElementById("edit-hotel-policy")?.value.trim();try{await api("POST","/api/crm/hotel-info",{name:e,subtitle:t,address:o,phone:a,cancellationPolicy:i}),toast("Hotel info saved!","success")}catch{toast("Failed to save","error")}}async function fe(){const e=parseFloat(document.getElementById("edit-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("edit-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("edit-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),localStorage.setItem("ratesChanged","1"),S(),updatePropertyStatusStrip(),I("rates"),toast("Rates saved!","success")}catch{toast("Failed to save rates","error")}}async function ye(){const e=document.getElementById("edit-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),p.token=e;try{localStorage.setItem("crmToken",p.token)}catch{}toast("PIN updated!","success")}catch{toast("Failed to change PIN","error")}}function he(e){navigator.clipboard.writeText(e).then(()=>{localStorage.setItem("linkCopied","1"),S(),I("link"),toast("Link copied!","success")}).catch(()=>{toast("Failed to copy","error")})}function be(e){const t=e.nextElementSibling,o=e.querySelector(".accordion-arrow");t.style.display==="none"?(t.style.display="block",o&&(o.style.transform="rotate(90deg)")):(t.style.display="none",o&&(o.style.transform="rotate(0deg)"))}async function ve(){try{const e=await api("POST","/api/crm/go-live");e.success&&e.url?window.location.href=e.url:toast(e.message||"Failed to start checkout","error")}catch{toast("Failed to start checkout. Try again.","error")}}async function we(){try{const e=await api("GET","/api/crm/billing-portal");e.success&&e.url?window.location.href=e.url:toast(e.message||"Contact support@bookmarketel.com to manage your subscription.","error")}catch{toast("Contact support@bookmarketel.com to manage your subscription.","error")}}async function ke(){const e=document.getElementById("supportMessage")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),document.getElementById("supportMessage").value="",toast("Message sent! We'll reply to your email.","success")}catch{toast("Failed to send. Email support@bookmarketel.com directly.","error")}}async function Ee(e){const t=p.editRooms.find(l=>l.id===e);if(!t){toast("Room not found — try refreshing","error");return}const o=document.getElementById("edit-name-"+e)?.value.trim(),a=document.getElementById("edit-desc-"+e)?.value.trim(),i=parseInt(document.getElementById("edit-occ-"+e)?.value)||4,n=parseInt(document.getElementById("edit-units-"+e)?.value)||1,d={id:e,name:o||t.name,description:a||"",amenities:t.amenities||"",maxOccupancy:i,totalUnits:n};try{const l=await api("POST","/api/crm/rooms",d);if(l&&l.success===!1){toast(l.message||"Failed to save","error");return}t.name=d.name,t.description=d.description,t.maxOccupancy=i,t.totalUnits=n,toast("Room saved!","success")}catch(l){toast("Failed to save: "+(l.message||""),"error")}}async function ze(e,t){const o=Array.from(e.target.files);if(!o.length)return;const i=document.getElementById("edit-card-"+t)?.querySelector("div:first-child");i&&(i.style.position="relative",i.insertAdjacentHTML("beforeend",'<div id="upload-spinner-'+t+'" style="position:absolute;inset:0;background:rgba(255,255,255,0.8);display:flex;align-items:center;justify-content:center;z-index:5;flex-direction:column;gap:6px;"><div style="width:24px;height:24px;border:3px solid var(--border);border-top-color:var(--green);border-radius:50%;animation:spin 0.7s linear infinite;"></div><div id="upload-progress-'+t+'" style="font-size:12px;color:var(--text-muted);font-weight:600;">0 / '+o.length+"</div></div>"));let n=0;for(const l of o){const u=await F(l),x=new FormData;x.append("image",u,u.name||"room.webp");try{const b=await(await fetch(`/api/crm/rooms/${t}/images`,{method:"POST",headers:{"x-crm-token":x.token},body:x})).json();if(b.success&&b.image){const r=x.editRooms.find(s=>s.id===t);r&&(r.images||(r.images=[]),r.images.push(b.image),r.imageUrl||(r.imageUrl=b.image.url)),n++}}catch{}const m=document.getElementById("upload-progress-"+t);m&&(m.textContent=n+" / "+o.length)}const d=document.getElementById("upload-spinner-"+t);d&&d.remove(),B(),S(),I("photo"),toast(n+" photo"+(n!==1?"s":"")+" added!","success")}function q(e,t=512){return new Promise((o,a)=>{const i=new Image,n=URL.createObjectURL(e);i.onload=()=>{try{const d=Math.min(i.naturalWidth,i.naturalHeight),l=(i.naturalWidth-d)/2,u=(i.naturalHeight-d)/2,x=document.createElement("canvas");x.width=t,x.height=t;const m=x.getContext("2d");m.imageSmoothingQuality="high",m.drawImage(i,l,u,d,d,0,0,t,t),URL.revokeObjectURL(n),x.toBlob(w=>w?o(w):a(new Error("crop failed")),"image/png",.92)}catch(d){URL.revokeObjectURL(n),a(d)}},i.onerror=()=>{URL.revokeObjectURL(n),a(new Error("load failed"))},i.src=n})}function W(){const e=document.getElementById("appsAppIconPreview");e&&(e.innerHTML='<div style="width:24px;height:24px;border:3px solid rgba(255,255,255,0.5);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;"></div>')}function _(e){const t=document.getElementById("appsAppIconPreview");t&&(t.style.background="#fff",t.innerHTML='<img src="'+e+'" alt="App icon" style="width:100%;height:100%;object-fit:cover;">')}async function Se(e){const t=e.files&&e.files[0];if(!t)return;W();const o=new FormData;try{const a=await q(t,512);o.append("icon",a,"app-icon.png")}catch{o.append("icon",t)}try{const i=await(await fetch("/api/crm/hotel-app-icon",{method:"POST",headers:{"x-crm-token":o.token},body:o})).json();if(i.success&&i.appIconUrl){o.activeHotelAppIcon=i.appIconUrl,_(i.appIconUrl);const n=document.getElementById("appsView");n&&(n.dataset.appsKey=(o.activeHotelId||"")+"|"+i.appIconUrl+"|"+(o.activeHotelDomain||"")),typeof updateFrontdeskManifestLink=="function"&&updateFrontdeskManifestLink(),toast("Logo updated! Guests will see it on their phone.","success")}else{toast(i.message||"Failed to upload icon","error");const n=(o.activeHotelName||"H").trim().charAt(0).toUpperCase()||"🏨",d=document.getElementById("appsAppIconPreview");d&&(d.style.background="var(--green)",d.innerHTML=n)}}catch{toast("Failed to upload icon","error")}e.value=""}async function Ie(e,t){if(confirm("Delete this photo?"))try{await api("DELETE",`/api/crm/rooms/${e}/images/${t}`);const o=p.editRooms.find(a=>a.id===e);o&&o.images&&(o.images=o.images.filter(a=>a.id!==t),o.imageUrl=o.images[0]?.url||null),B(),toast("Photo deleted","success")}catch{toast("Failed to delete","error")}}async function Be(e){if(confirm("Delete this room type?"))try{await api("DELETE",`/api/crm/rooms/${e}`),toast("Room deleted","success"),$()}catch{toast("Failed to delete","error")}}function Te(){const e=document.getElementById("editRoomsList");document.getElementById("editAddForm")||(e.insertAdjacentHTML("beforeend",`
  ...
  `),document.getElementById("editNewRoomName").focus())}function Pe(){const e=document.getElementById("editNewRoomName").value.trim();e&&api("POST","/api/crm/rooms",{name:e,maxOccupancy:4,totalUnits:5}).then(()=>{toast("Room added","success"),$()}).catch(()=>toast("Failed to add","error"))}const V={addAmenityPrompt:ue,advanceSetupAfterAction:I,advanceTourIfNeeded:S,changePin:ye,checklistGoTo:Z,checklistGoToRates:K,closeAmenityPicker:M,confirmAmenityPicker:ge,confirmEditAddRoom:Pe,copyBookingLink:he,copyBookingLinkFromChecklist:X,deleteEditImage:Ie,deleteEditRoom:Be,frontdeskInstallCardHtml:R,getAmenityIcon:U,goLive:ve,handleInstallFrontdesk:se,loadEditRooms:$,loadSettings:N,maybePromptInstalledNotifications:pe,openAmenityPicker:Y,openBillingPortal:we,openEditAddRoom:Te,openPreviewSite:ne,refreshFrontdeskInstallCard:C,removeAmenity:xe,renderEditRooms:A,renderEditRoomsCards:B,saveEditRoom:Ee,saveHotelInfo:me,saveRates:fe,sendSupportMessage:ke,setAppIconPreviewImage:_,setAppIconPreviewLoading:W,settingsChangePin:oe,settingsCopyLink:J,settingsSaveRates:te,settingsSendSupport:ie,settingsUploadPhoto:ee,showActivatedModal:de,showIosInstallSheet:j,showNotifPromptModal:O,showOnboardingQuestions:ae,showWelcomeModal:H,squareCropImage:q,startSettingsTour:D,toggleAmenityPreset:ce,toggleAppNotifications:le,toggleSection:be,updatePreviewSiteBar:re,uploadAppIcon:Se,uploadEditImages:ze};function Me(){Q(V)}const $e=Object.freeze(Object.defineProperty({__proto__:null,default:V,install:Me},Symbol.toStringTag,{value:"Module"}));export{Ce as a,$e as b,Q as e,p as f,De as s};
