const s={token:"",bookings:[],guestMessages:[],currentFilter:"settings",bookingCallFilter:"all",manualAvailability:{rooms:[],overrides:{}},manualSelectedRoom:"",availabilityYear:new Date().getFullYear(),availabilityMonth:new Date().getMonth(),availabilityEditingDay:"",availabilityDaySaving:!1,editingRoomName:"",pendingDeleteRoomName:"",currentHotelPms:"",revenueEnabled:!1,hotelSubscribed:!1,revenuePeriod:"30d",revenueCache:{},revenueLoading:!1,revenueError:"",blockedDemand:{total:0,today:0,recent:[]},bookingsSubview:"bookings",launchStatus:null,growthFunnel:null,growthChecklist:{},growthPeriod:"30d",ALLOWED_REVENUE_PERIODS:new Set(["today","7d","30d","all"]),OTA_COMMISSION_RATE:.25,activeHotelId:"",activeHotelName:"",activeHotelAppIcon:"",appsViewPlatform:"ios",activeHotelDomain:"",activeHotelContext:null,settingsTourActive:!1,bootInFlight:!1,CRM_HOTEL_BY_HOST:{"guestlodgeminot.clickinns.com":"guest-lodge-minot","booking-kappa-nine.vercel.app":"guest-lodge-minot","stcroix.clickinns.com":"st-croix-wisconsin","homeplacesuites.clickinns.com":"home-place-suites","myhomeplacesuites.com":"home-place-suites","www.myhomeplacesuites.com":"home-place-suites","suitestay.clickinns.com":"suite-stay","clickinns.com":"suite-stay","www.clickinns.com":"suite-stay"},CRM_HOTEL_LABELS:{"guest-lodge-minot":"Guest Lodge Minot","st-croix-wisconsin":"St. Croix Wisconsin","home-place-suites":"Home Place Suites","suite-stay":"Suite Stay"},deferredInstallPrompt:null,frontdeskInstalled:!1,_magicLoginPending:!1,editRooms:[],editRoomsLoadPromise:null,messageUnreadCount:0,messagesInboxOpen:!1,messagesThreadPickerOpen:!1,selectedMessageThread:"",bookingsVirtualList:[],bookingsVirtualRaf:0};let W=null;function Et(){return typeof lucide<"u"?Promise.resolve():W||(W=new Promise((e,t)=>{const o=document.createElement("script");o.src="https://unpkg.com/lucide@0.469.0/dist/umd/lucide.min.js",o.async=!0,o.onload=()=>e(),o.onerror=()=>t(new Error("lucide load failed")),document.head.appendChild(o)}),W)}async function He(e){if(!e||!e.type.startsWith("image/")||e.type==="image/webp"&&e.size<4e5)return e;try{const t=await createImageBitmap(e),o=1600,i=1200;let r=t.width,n=t.height;const a=Math.min(1,o/r,i/n);r=Math.round(r*a),n=Math.round(n*a);const d=document.createElement("canvas");d.width=r,d.height=n,d.getContext("2d").drawImage(t,0,0,r,n),t.close();const c=await new Promise((h,v)=>{d.toBlob(S=>S?h(S):v(new Error("encode failed")),"image/webp",.82)}),g=(e.name||"room-photo").replace(/\.[^.]+$/,"")||"room-photo";return new File([c],g+".webp",{type:"image/webp"})}catch{return e}}function St(){const e=()=>{s.currentFilter==="bookings"?loadMessages():loadMessageBadges()};"requestIdleCallback"in window?requestIdleCallback(e,{timeout:2500}):setTimeout(e,600)}function Fe(e){Object.assign(window,e)}async function be(){const e=document.getElementById("settingsList");if(e){e.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const t=await api("GET","/api/crm/verify"),i="https://"+(t?.domain||s.activeHotelId+".mktel.co"),r="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(i),n=await api("GET","/api/crm/rooms");let a={nightly:69,weekly:299,monthly:999};n?.rates&&(a=n.rates);const d=n?.rooms||[];let c="";t?.subscribed||(c+=goLiveInlineCardHtml()),d.length?d.forEach(g=>{const h=g.images&&g.images.length>0;c+=`
    ...
    `,e.innerHTML=c}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load settings</div></div>'}}}function De(){const e=document.getElementById("settings-booking-url");e&&navigator.clipboard.writeText(e.value).then(()=>{localStorage.setItem("linkCopied","1"),N(),toast("Link copied!","success")}).catch(()=>toast("Copy failed","error"))}function Ne(){localStorage.setItem("settingsTourDone","1");const e=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",e);let t=0;const o=setInterval(()=>{t++;const i=document.getElementById("edit-rate-nightly");if(i||t>20){if(clearInterval(o),!i)return;const r=i.closest(".accordion-body");if(r&&r.style.display==="none"){r.style.display="block";const n=r.previousElementSibling?.querySelector(".accordion-arrow");n&&(n.style.transform="rotate(90deg)")}setTimeout(()=>{i.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const n=document.getElementById("checklistPointer");n&&n.remove();const a=i.getBoundingClientRect(),d=document.createElement("div");d.id="checklistPointer",d.style.cssText=`position:fixed;z-index:100000;left:50%;transform:translateX(-50%);top:${a.bottom+12}px;max-width:240px;width:calc(100% - 40px);`,d.innerHTML=`
          ...
          `,document.body.appendChild(d),setTimeout(()=>{const c=document.getElementById("checklistPointer");c&&c.remove()},6e3)},1e3)},100)}},200)}function Oe(){const t="https://"+(s.activeHotelDomain||s.activeHotelId+".mktel.co");navigator.clipboard.writeText(t).then(()=>{localStorage.setItem("linkCopied","1"),N(),toast("Link copied!","success"),loadBookings()}).catch(()=>toast("Copy failed","error"))}function je(e,t){localStorage.setItem("settingsTourDone","1");const o=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",o);let i=0;const r=setInterval(()=>{i++;const n=document.querySelector(e);if(n||i>20){if(clearInterval(r),!n)return;n.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const a=document.getElementById("checklistPointer");a&&a.remove();const d=n.getBoundingClientRect(),c=document.createElement("div");c.id="checklistPointer",c.style.cssText=`
        ...
        `,document.body.appendChild(c),setTimeout(()=>{const g=document.getElementById("checklistPointer");g&&g.remove()},6e3)},1e3)}},200)}function le(){const e=String(s.token||localStorage.getItem("crmToken")||"").trim();return e&&(s.token=e),e}async function pe(e,t){const o=le();if(!o)throw new Error("Not logged in");const i=await He(t),r=new FormData;r.append("image",i,i.name||"room.webp");const n=new URLSearchParams;s.activeHotelId&&n.set("hotelId",s.activeHotelId),n.set("token",o);const a=await fetch(`/api/crm/rooms/${e}/images?${n}`,{method:"POST",headers:{"x-crm-token":o},body:r}),d=await a.json().catch(()=>({}));if(!a.ok||!d.success)throw new Error(d.message||d.error||`Upload failed (${a.status})`);return d}async function qe(e,t){const o=e.target.files[0];if(o)try{await pe(t,o),toast("Photo uploaded!","success"),be()}catch(i){toast(i.message||"Upload failed","error")}}async function Ue(){const e=parseFloat(document.getElementById("settings-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("settings-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("settings-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),toast("Rates saved","success")}catch{toast("Failed to save rates","error")}}async function Ve(){const e=document.getElementById("settings-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),s.token=e;try{localStorage.setItem("crmToken",s.token)}catch{}toast("PIN updated!","success"),document.getElementById("settings-new-pin").value=""}catch{toast("Failed to change PIN","error")}}async function We(){const e=document.getElementById("settings-support-msg")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),toast("Message sent!","success"),document.getElementById("settings-support-msg").value=""}catch{toast("Failed to send","error")}}function Ge(){const e=s.activeHotelDomain||s.activeHotelId+".mktel.co",o=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:5173/?hotelId="+encodeURIComponent(s.activeHotelId)+"&preview=1":"https://"+e+"?preview=1";window.open(o,"_blank")}function ve(){if((window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1")&&s.activeHotelId)return"http://localhost:5173/?hotelId="+encodeURIComponent(s.activeHotelId);const t=s.activeHotelDomain||"";return t?"https://"+t+"/":""}function Ye(){const e=ve();if(!e){toast("Your booking domain is still setting up.","info");return}window.open(e,"_blank")}function _e(){const e=document.getElementById("previewSiteBar");e&&(e.style.display=s.currentFilter==="settings"?"block":"none")}function N(){if(localStorage.getItem("settingsTourDone"))return;const e=parseInt(localStorage.getItem("settingsTourStep")||"0"),t=s.editRooms.some(a=>a.images&&a.images.length>0),o=!!localStorage.getItem("ratesChanged"),i=!!localStorage.getItem("linkCopied");e===1&&t&&localStorage.setItem("settingsTourStep","2"),e===2&&o&&localStorage.setItem("settingsTourStep","3"),e===3&&i&&localStorage.setItem("settingsTourStep","4");const r=document.getElementById("tourTooltip");r&&r.remove();const n=document.getElementById("tourBlurOverlay");n&&n.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(a=>{a.style.position=a.dataset.tourOrigPosition||"",a.style.zIndex="",a.style.isolation="",a.style.boxShadow="",a.removeAttribute("data-tour-highlighted"),delete a.dataset.tourOrigPosition}),document.body.style.overflow=""}function Qe(){let e=0;const t={},o=[{title:"Why do you want a booking engine?",key:"why",type:"text",placeholder:"e.g. I want guests to book directly instead of calling me..."},{title:"How do guests currently book with you?",key:"currentBooking",type:"choice",options:[{label:"They call me or walk in",value:"phone_walkin"},{label:"Through Booking.com / Expedia",value:"ota"},{label:"I have a website but no booking system",value:"website_no_booking"},{label:"I don't take bookings online yet",value:"no_online"}]},{title:"How many rooms do you have?",key:"roomCount",type:"choice",options:[{label:"1–5 rooms",value:"1-5"},{label:"6–15 rooms",value:"6-15"},{label:"16–50 rooms",value:"16-50"},{label:"50+ rooms",value:"50+"}]},{title:"What's most important to you?",key:"priority",type:"choice",options:[{label:"Stop paying OTA commissions",value:"no_commission"},{label:"Get more direct bookings",value:"more_bookings"},{label:"Have a professional online presence",value:"professional"},{label:"Make it easier for guests to book",value:"easier_booking"}]}];function i(){let r=document.getElementById("onboardingOverlay");if(r&&r.remove(),e>=o.length){localStorage.setItem("onboardingDone","1");try{api("POST","/api/crm/onboarding-answers",t).catch(()=>{})}catch{}we();return}const n=o[e],a=document.createElement("div");a.id="onboardingOverlay",a.style.cssText="position:fixed;inset:0;z-index:100001;background:linear-gradient(135deg, #1a2b22 0%, #2E7D5B 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;",n.type==="text"?(a.innerHTML=`
      ...
      `,document.body.appendChild(a),document.getElementById("onboardingTextSubmit").onclick=()=>{const d=document.getElementById("onboardingTextInput").value.trim();d&&(t[n.key]=d,e++,i())}):(a.innerHTML=`
        <div style="max-width:360px;width:100%;text-align:center;">
          <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-bottom:8px;">Question ${e+1} of ${o.length}</div>
          <h2 style="font-size:20px;font-weight:700;color:white;margin:0 0 24px;line-height:1.3;">${n.title}</h2>
      ...
      `,document.body.appendChild(a),a.querySelectorAll(".onboarding-opt").forEach(d=>{d.addEventListener("click",()=>{t[n.key]=d.dataset.value,d.style.background="#1a1a2e",d.style.color="white",d.style.fontWeight="600",setTimeout(()=>{e++,i()},250)})}))}i()}function Je(){["onboardingDone","settingsTourDone","settingsTourStep","linkCopied","ratesChanged","appsTourDone","postActivationTourDone"].forEach(t=>{localStorage.removeItem(t)});const e=new URL(window.location.href);location.href=e.pathname+"?welcome"}function we(){const e=document.createElement("div");e.id="welcomeModalOverlay",e.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.72);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:24px;";function t(){localStorage.setItem("onboardingDone","1"),localStorage.removeItem("settingsTourDone"),localStorage.removeItem("settingsTourStep");try{const r=new URL(window.location);r.searchParams.delete("welcome"),window.history.replaceState({},"",r)}catch{}const i=typeof ae=="function"?ae:typeof window.startSettingsTour=="function"?window.startSettingsTour:null;i&&i(),e.remove()}function o(){e.innerHTML=`
      ...
      </div>`,document.getElementById("welcomeModalNext").onclick=t}document.body.appendChild(e),o(),typeof invokeLoadEditRooms=="function"&&invokeLoadEditRooms()}function J(){const e=document.getElementById("postActivationTourTooltip");e&&e.remove();const t=document.getElementById("postActivationTourOverlay");t&&t.remove(),document.querySelectorAll("[data-post-activation-highlight]").forEach(o=>{o.style.boxShadow="",o.style.position="",o.style.zIndex="",o.removeAttribute("data-post-activation-highlight")}),document.body.style.overflow=""}function Y(){J(),localStorage.setItem("postActivationTourDone","1");const e=document.querySelector('.tab[data-nav-filter="apps"]')||document.querySelector('.mobile-nav-item[data-nav-filter="apps"]');try{setFilter("apps",e)}catch{}}function ce(){if(localStorage.getItem("postActivationTourDone")){Y();return}J();const e=[{tab:"bookings",navFilter:"bookings",text:"<strong>Bookings</strong> — live reservations land here. You'll get a push alert for each new one."},{tab:"apps",navFilter:"apps",text:"<strong>Phones</strong> — put your guest booking app on their home screen and send install reminders."}];let t=0;function o(){if(J(),t>=e.length){Y();return}const i=e[t],r=document.querySelector(`.tab[data-nav-filter="${i.navFilter}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${i.navFilter}"]`);r&&setFilter(i.tab,r);const n=document.createElement("div");n.id="postActivationTourOverlay",n.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.55);",document.body.appendChild(n),document.body.style.overflow="hidden",setTimeout(()=>{const a=document.querySelector(`.tab[data-nav-filter="${i.navFilter}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${i.navFilter}"]`);a&&(a.setAttribute("data-post-activation-highlight","1"),a.style.position="relative",a.style.zIndex="100003",a.style.boxShadow="0 0 0 3px #fff, 0 0 0 6px #2E7D5B",a.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"}));const d=a?a.getBoundingClientRect():{left:24,bottom:80,width:200},c=document.createElement("div");c.id="postActivationTourTooltip";const g=Math.min(300,window.innerWidth-32),h=Math.max(16,Math.min(d.left+d.width/2-g/2,window.innerWidth-g-16)),v=Math.min(d.bottom+14,window.innerHeight-180);c.style.cssText=`position:fixed;z-index:100004;left:${h}px;top:${v}px;max-width:${g}px;width:${g}px;`;const S=t>=e.length-1;c.innerHTML=`
  ...
  `,document.body.appendChild(o),document.getElementById("activatedModalTour").onclick=()=>{o.remove(),ce()},document.getElementById("activatedModalSkip").onclick=()=>{o.remove(),localStorage.setItem("postActivationTourDone","1");try{setFilter("bookings")}catch{}}}function ke(){return isStandaloneApp()||s.frontdeskInstalled?pushSupported()?typeof Notification<"u"&&Notification.permission==="granted"?"":ne():"":ne()}function ne(){const e=isStandaloneApp()||s.frontdeskInstalled,t=typeof Notification<"u"&&Notification.permission==="granted";let o;return e&&pushSupported()?o=t?`<p style="font-size:12px;color:var(--text-muted);margin:0 0 14px;line-height:1.5;">You're all set — you'll get an alert on this device every time a guest books.</p>
    ...
    </div></div>`}function X(){if(s.currentFilter==="bookings"){renderMessages();return}const e=document.getElementById("frontdeskInstallCard");if(!e)return;const t=ke();t?e.outerHTML=t:e.remove()}function Z(){const e=typeof ensureAppsViewRendered=="function"?ensureAppsViewRendered:window.ensureAppsViewRendered;typeof e=="function"&&e(!0)}function Xe(){if(isIosDevice()){Se({title:(s.activeHotelName?s.activeHotelName+" ":"")+"Front Desk",subtitle:"Add it to your home screen — takes 3 seconds."});return}s.deferredInstallPrompt?(s.deferredInstallPrompt.prompt(),s.deferredInstallPrompt.userChoice.then(e=>{e&&e.outcome==="accepted"&&(s.frontdeskInstalled=!0),s.deferredInstallPrompt=null,X(),Z()}).catch(()=>{})):toast('Use your browser menu → "Install app" / "Add to Home screen".',"info")}async function Ze(){if(typeof Notification<"u"&&Notification.permission==="granted"){try{await api("POST","/api/push/test",{}),toast("Test notification sent","success")}catch{toast("Could not send test","error")}return}await enableNotifications(),X(),Z()}function et(){try{if(!isStandaloneApp()||!pushSupported()||typeof Notification>"u"||Notification.permission!=="default"||localStorage.getItem("notifPromptShown")==="1"||document.getElementById("welcomeModalOverlay")||document.getElementById("activatedModalOverlay"))return;localStorage.setItem("notifPromptShown","1"),setTimeout(Ee,700)}catch{}}function Ee(){if(document.getElementById("notifPromptOverlay"))return;const e=document.createElement("div");e.id="notifPromptOverlay",e.style.cssText="position:fixed;inset:0;z-index:100002;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px;",e.innerHTML=`
    ...
    </div>`,document.body.appendChild(e),document.getElementById("notifPromptEnable").onclick=async()=>{e.remove(),await enableNotifications(),X(),Z()},document.getElementById("notifPromptLater").onclick=()=>e.remove()}function Se({title:e,subtitle:t,iconUrl:o,openUrl:i}={}){const r=document.getElementById("iosInstallSheet");r&&r.remove();const n=document.createElement("div");n.id="iosInstallSheet",n.style.cssText="position:fixed;inset:0;z-index:100003;background:rgba(0,0,0,0.5);backdrop-filter:blur(2px);display:flex;align-items:flex-end;justify-content:center;";const a=o?`<img src="${o}" alt="" style="width:48px;height:48px;border-radius:12px;object-fit:cover;flex-shrink:0;">`:`<div style="width:48px;height:48px;border-radius:12px;flex-shrink:0;background:#2E7D5B;color:#fff;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;">${(s.activeHotelName||"B").trim().charAt(0).toUpperCase()}</div>`;n.innerHTML=`
    ...
    </div>`,document.body.appendChild(n),typeof lucide<"u"&&lucide.createIcons();const d=()=>n.remove(),c=document.getElementById("iosInstallSheetClose");c&&(c.onclick=d)}function H(){let e=document.getElementById("tourBlurOverlay");return e||(e=document.createElement("div"),e.id="tourBlurOverlay",e.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.6);",document.body.appendChild(e),document.body.style.overflow="hidden",e)}function ee(){const e=document.getElementById("tourTooltip");e&&e.remove();const t=document.getElementById("tourBlurOverlay");t&&t.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(i=>{i.style.position=i.dataset.tourOrigPosition||"",i.style.zIndex="",i.style.isolation="",i.style.boxShadow="",i.removeAttribute("data-tour-highlighted"),delete i.dataset.tourOrigPosition});const o=document.getElementById("goLiveBanner");o&&o.dataset.tourHidden&&(delete o.dataset.tourHidden,typeof updateGoLiveBanner=="function"&&updateGoLiveBanner()),document.body.style.overflow=""}function L(e,t){if(!t.openAccordion)return;const o=t.accordionCard?document.querySelector(t.accordionCard):e&&e.closest?e.closest(".booking-card"):null;if(!o)return;const i=o.querySelector(".accordion-body");if(!i)return;if(i.style.display==="none"||getComputedStyle(i).display==="none"){i.style.display="block";const n=o.querySelector(".accordion-arrow");n&&(n.style.transform="rotate(90deg)")}}function C(e){if(!e)return null;for(const t of String(e).split(",").map(o=>o.trim()).filter(Boolean)){const o=document.querySelector(t);if(o&&o.isConnected)return o}return null}function F(e,t){if(t.highlightSelector){const o=C(t.highlightSelector);if(o)return o}if(t.highlightCard){const o=t.accordionCard?document.querySelector(t.accordionCard):e&&e.closest?e.closest(".booking-card"):null;if(o)return o}return t.targetParent&&(e.closest(".booking-card")||e.closest(".accordion-body"))||e}function _(e,t){if(!t)return e;const o=String(t.target||"").split(",").map(i=>i.trim()).filter(Boolean);for(const i of o){const r=document.querySelector(i);if(r&&r.isConnected)return r}if(t.accordionCard){const i=document.querySelector(t.accordionCard);if(i&&i.isConnected)return i}return e&&e.isConnected?e:null}function re(e,t){if(!e||!e.isConnected)return null;const o=e.getBoundingClientRect();return o.width<2||o.height<2||!t&&(o.bottom<8||o.top>window.innerHeight-8)?null:o}function Q(e,t){const o=C(e.anchorSelector);if(o){const i=re(o,!0);if(i)return i}return re(t,!0)}function Te(e,t){const o=t.scrollTarget||t.accordionCard,i=(o?C(o):null)||e;if(!i&&!t.scrollToTop)return Promise.resolve();const r=t.scrollBlock||"center",n=window.matchMedia("(prefers-reduced-motion: reduce)").matches,a=s.settingsTourActive||n?"auto":"smooth";return new Promise(d=>{const c=()=>{const $=t.scrollPadTop??80,l=t.scrollPadBottom??130,p=C(t.anchorSelector)||(i&&i.isConnected?i:null)||(e&&e.isConnected?e:null);if(!p){d();return}const y=p.getBoundingClientRect();y.top<$&&window.scrollBy({top:y.top-$,left:0,behavior:"auto"}),y.bottom>window.innerHeight-l&&window.scrollBy({top:y.bottom-window.innerHeight+l,left:0,behavior:"auto"}),requestAnimationFrame(()=>requestAnimationFrame(d))},g=()=>{i&&i.scrollIntoView({behavior:t.scrollToTop?"auto":a,block:r,inline:"nearest"}),c()};if(t.scrollToTop){if(window.scrollTo({top:0,left:0,behavior:a}),document.documentElement.scrollTop=0,document.body.scrollTop=0,t.scrollToTopOnly){requestAnimationFrame(()=>requestAnimationFrame(d));return}if(a==="auto"){g();return}let $=!1;const l=()=>{$||($=!0,window.removeEventListener("scrollend",p),clearTimeout(y),g())},p=()=>l();"onscrollend"in window&&window.addEventListener("scrollend",p,{once:!0});const y=setTimeout(l,520);return}if(!i){d();return}if(i.scrollIntoView({behavior:a,block:r,inline:"nearest"}),a==="auto"){c();return}let h=!1;const v=()=>{h||(h=!0,window.removeEventListener("scrollend",S),clearTimeout(te),c())},S=()=>v();"onscrollend"in window&&window.addEventListener("scrollend",S,{once:!0});const te=setTimeout(v,620)})}function tt(){ee(),localStorage.setItem("settingsTourStep","handoff");const e=()=>{const o=document.querySelector('.tab[data-nav-filter="apps"]')||document.querySelector('.mobile-nav-item[data-nav-filter="apps"]');o&&setFilter("apps",o);const i=typeof ensureAppsViewRendered=="function"?ensureAppsViewRendered:window.ensureAppsViewRendered;typeof i=="function"&&i(!0);const r=typeof startAppsTour=="function"?startAppsTour:window.startAppsTour;typeof r=="function"&&r({chainFromSettingsTour:!0})},t=typeof loadAppsModule=="function"?loadAppsModule:window.loadAppsModule;typeof t=="function"?t().then(e).catch(e):e()}function D(){s.settingsTourActive=!1,updateGoLiveBanner();const e=document.createElement("div");e.id="tourBlurOverlay",e.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(e),document.body.style.overflow="hidden";const t=document.createElement("div");if(t.id="tourTooltip",t.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;",t.innerHTML=`
    ...
    <div style="background:white;border-radius:20px;max-width:340px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
        ...
        ${b}
      </div>`;if(p.tooltipAnchor==="card-bottom"||p.tooltipAnchor==="card-top"){const j=C(p.highlightSelector)||l,fe=C(p.anchorSelector);if(j){const P=j.getBoundingClientRect(),q=fe?fe.getBoundingClientRect():null,z=Math.min(280,window.innerWidth-32),ye=P.left+P.width/2,R=Math.max(16,Math.min(ye-z/2,window.innerWidth-z-16)),U=Math.max(20,Math.min(ye-R,z-20)),V=132;if(p.tooltipAnchor==="card-bottom"){const oe=window.innerHeight-P.bottom,he=Math.min(P.bottom+12,window.innerHeight-V-16);if(oe>=V+20)m.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${he}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ...
              ${x}`;else{const ie=q?q.top:P.top;m.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-ie+10}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`}}else{const oe=P.top;if(P.top-16>=V+24)m.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-oe+10}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`;else{const ie=Math.min(P.top+(q?q.height:48)+12,window.innerHeight-V-16);m.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${ie}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:8px solid #1a1a2e;margin-left:${U-8}px;"></div>
              ${x}`}}document.body.appendChild(m),v();return}}const f=y||Q(p,l);if(!f){m.style.cssText="position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:280px;width:calc(100% - 32px);",m.innerHTML=x,document.body.appendChild(m),v();return}const k=window.innerHeight-f.bottom,T=f.top,E=Math.min(280,window.innerWidth-32),I=f.left+f.width/2,B=Math.max(16,Math.min(I-E/2,window.innerWidth-E-16)),M=Math.max(20,Math.min(I-B,E-20));let A=p.tooltipPosition||(k>150?"below":T>150?"above":"center");if(A==="below"&&k<130&&T>k&&(A="above"),A==="above"&&T<130&&k>T&&(A="below"),A==="below"&&f.bottom+150>window.innerHeight&&(A="above"),A==="below"){const j=Math.min(f.bottom+8,window.innerHeight-24);m.style.cssText=`position:fixed;z-index:100000;left:${B}px;top:${j}px;max-width:${E}px;width:${E}px;`,m.innerHTML=`
        ...
        ${x}`}else A==="above"?(m.style.cssText=`position:fixed;z-index:100000;left:${B}px;bottom:${Math.max(16,window.innerHeight-f.top+8)}px;max-width:${E}px;width:${E}px;`,m.innerHTML=`
        ${x}
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${M-8}px;"></div>`):(m.style.cssText="position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:280px;width:calc(100% - 32px);",m.innerHTML=x);document.body.appendChild(m),v()}function v(){const l=document.getElementById("tourNextBtn"),p=document.getElementById("tourSkipBtn");l&&(l.onclick=()=>{r(),i++,localStorage.setItem("settingsTourStep",String(i)),d()}),p&&(p.onclick=()=>{n()})}function S(){typeof invokeLoadEditRooms=="function"&&invokeLoadEditRooms();const l=document.createElement("div");l.id="tourBlurOverlay",l.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(l),document.body.style.overflow="hidden";const p=s.activeHotelName||"Your Hotel",y=p.trim().charAt(0).toUpperCase(),u=p.length>10?p.slice(0,10):p,m="width:32px;display:flex;flex-direction:column;align-items:center;gap:5px;",b="width:32px;height:32px;border-radius:9px;box-sizing:border-box;",w="height:8px;max-width:46px;line-height:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;",x=`<div style="${m}"><div style="${b}background:rgba(255,255,255,0.22);"></div><div style="${w}"></div></div>`,f=s.activeHotelAppIcon||"",k=f?`<img src="${f}" alt="" style="width:100%;height:100%;object-fit:contain;">`:y,T=f?`${b}background:#fff;padding:5px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);`:`${b}background:#fff;color:#2E7D5B;font-weight:800;font-size:15px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);`,E=`<div style="${m}"><div style="${T}">${k}</div><div style="${w}font-size:7.5px;color:#fff;font-weight:700;">${u}</div></div>`,I=[x,x,x,x,E,x,x,x].join(""),B=document.createElement("div");if(B.id="tourTooltip",B.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:20px 16px;",B.innerHTML=`
      ...
      </div>`,document.body.appendChild(B),!document.getElementById("tourModalAnimStyle")){const M=document.createElement("style");M.id="tourModalAnimStyle",M.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(M)}document.getElementById("tourNextBtn").onclick=()=>{r(),i++,localStorage.setItem("settingsTourStep",String(i)),d()},document.getElementById("tourSkipBtn").onclick=()=>{n()}}function te(){const l=document.createElement("div");l.id="tourBlurOverlay",l.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(l),document.body.style.overflow="hidden";let p=0;const y=[`<div style="padding:20px 18px 0;">
    ...
    `;e.innerHTML=S,O(),typeof lucide<"u"&&lucide.createIcons()}catch{e.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load your page</div><div class="empty-sub">Check your connection and refresh.</div></div>'}})();try{await s.editRoomsLoadPromise}finally{s.editRoomsLoadPromise=null}}}function ge(){O()}function O(){const e=document.getElementById("editRoomsCards");if(e){if(!s.editRooms.length){e.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>';return}e.innerHTML=s.editRooms.map(t=>{const o=(t.amenities||"").split("•").map(n=>n.trim()).filter(Boolean),i=(t.images||[]).filter(n=>n&&n.url),r=jsStr(t.id);return`
    <div class="booking-card" style="margin-bottom:14px;" id="edit-card-${t.id}">
      ...
      <div class="room-edit-photo" data-photo-index="0">
        ${i.length?`
          <img class="room-edit-main-img" src="${esc(i[0].url)}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='https://suitestay.clickinns.com/kingbedsuitestay.webp';">
          ${i.length>1?`
            ...
            <div class="room-edit-photo-count">1 / ${i.length}</div>
      ...
      ${i.length>1?'<div class="room-edit-thumbs">'+i.map((n,a)=>`<div class="room-edit-thumb-wrap"><button type="button" class="room-edit-thumb ${a===0?"active":""}" aria-label="Show photo ${a+1}" ${a===0?'aria-current="true"':""} onclick="showEditRoomPhoto('${r}', ${a})"><img src="${esc(n.url)}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='https://suitestay.clickinns.com/kingbedsuitestay.webp';"></button><button type="button" onclick="event.stopPropagation();deleteEditImage('${r}','${jsStr(n.id)}')" style="position:absolute;top:-4px;right:-4px;width:18px;height:18px;border-radius:50%;background:var(--red);color:white;border:none;font-size:11px;cursor:pointer;display:flex;align-items:center;justify-content:center;">×</button></div>`).join("")+"</div>":""}
      </div>
      <div class="room-edit-fields" style="padding:18px;">
        ...
        <div style="margin-bottom:12px;">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;" id="edit-amenity-pills-${t.id}">
            ${o.map(n=>`<span style="display:inline-flex;align-items:center;gap:4px;background:var(--green-pale);color:var(--green);padding:5px 10px;border-radius:8px;font-size:12px;font-weight:600;">${Ae(n)} ${n} <button onclick="removeAmenity('${t.id}','${n.replace(/'/g,"\\'")}')" style="background:none;border:none;color:var(--green);cursor:pointer;font-size:14px;margin-left:2px;">×</button></span>`).join("")}
          </div>
    ...
    </div>`}).join(""),typeof lucide<"u"&&lucide.createIcons()}}function Ie(e){const t=s.editRooms.find(o=>String(o.id)===String(e));return(t&&t.images||[]).filter(o=>o&&o.url)}function Be(e,t){const o=Ie(e);if(!o.length)return;const i=document.getElementById("edit-card-"+e);if(!i)return;const r=o.length,n=((Number(t)||0)%r+r)%r,a=i.querySelector(".room-edit-main-img");a&&(a.src=o[n].url),i.querySelector(".room-edit-photo")?.setAttribute("data-photo-index",String(n));const d=i.querySelector(".room-edit-photo-count");d&&(d.textContent=n+1+" / "+r),i.querySelectorAll(".room-edit-image-dot").forEach((c,g)=>{c.classList.toggle("active",g===n),g===n?c.setAttribute("aria-current","true"):c.removeAttribute("aria-current")}),i.querySelectorAll(".room-edit-thumb").forEach((c,g)=>{c.classList.toggle("active",g===n),g===n?c.setAttribute("aria-current","true"):c.removeAttribute("aria-current")})}function ot(e,t){const i=document.getElementById("edit-card-"+e)?.querySelector(".room-edit-photo"),r=parseInt(i?.getAttribute("data-photo-index")||"0",10)||0;Be(e,r+t)}function Ae(e){const t=e.toLowerCase();return t.includes("wifi")?'<i data-lucide="wifi" style="width:14px;height:14px;"></i>':t.includes("tv")||t.includes("television")?'<i data-lucide="tv" style="width:14px;height:14px;"></i>':t.includes("fridge")||t.includes("refrigerator")?'<i data-lucide="thermometer-snowflake" style="width:14px;height:14px;"></i>':t.includes("parking")?'<i data-lucide="car" style="width:14px;height:14px;"></i>':t.includes("housekeeping")||t.includes("cleaning")?'<i data-lucide="sparkles" style="width:14px;height:14px;"></i>':t.includes("bath")||t.includes("shower")?'<i data-lucide="bath" style="width:14px;height:14px;"></i>':t.includes("work")||t.includes("desk")?'<i data-lucide="laptop" style="width:14px;height:14px;"></i>':t.includes("pet")||t.includes("dog")?'<i data-lucide="paw-print" style="width:14px;height:14px;"></i>':t.includes("pool")?'<i data-lucide="waves" style="width:14px;height:14px;"></i>':t.includes("kitchen")||t.includes("microwave")?'<i data-lucide="cooking-pot" style="width:14px;height:14px;"></i>':'<i data-lucide="check" style="width:14px;height:14px;"></i>'}const Pe=[{key:"wifi",label:"Free WiFi",icon:"wifi"},{key:"tv",label:"Smart TV",icon:"tv"},{key:"fridge",label:"Fridge",icon:"thermometer-snowflake"},{key:"parking",label:"Free Parking",icon:"car"},{key:"housekeeping",label:"Weekly Housekeeping",icon:"sparkles"},{key:"bath",label:"Bath",icon:"bath"},{key:"workstation",label:"Workstation",icon:"laptop"},{key:"pet",label:"Pet Friendly",icon:"paw-print"},{key:"pool",label:"Pool",icon:"waves"},{key:"kitchen",label:"Kitchenette",icon:"cooking-pot"},{key:"ac",label:"Air Conditioning",icon:"wind"},{key:"laundry",label:"Laundry",icon:"shirt"}];let me=null;function Ce(e){me=e;const o=(s.editRooms.find(n=>n.id===e)?.amenities||"").split("•").map(n=>n.trim().toLowerCase()).filter(Boolean);let i=document.getElementById("amenityPickerModal");i||(document.body.insertAdjacentHTML("beforeend",`
    ...
    `),document.getElementById("amenityPickerModal").addEventListener("click",K),i=document.getElementById("amenityPickerModal"));const r=document.getElementById("amenityPickerGrid");r.innerHTML=Pe.map(n=>{const a=o.some(d=>d.includes(n.key));return`<button onclick="toggleAmenityPreset(this,'${n.key}')" data-key="${n.key}" style="display:inline-flex;align-items:center;gap:5px;padding:7px 12px;border-radius:8px;border:1.5px solid ${a?"#2E7D5B":"#e5e7eb"};background:${a?"#E8F5EE":"white"};color:${a?"#2E7D5B":"#1a1a2e"};font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;"><i data-lucide="${n.icon}" style="width:14px;height:14px;"></i> ${n.label}</button>`}).join(""),document.getElementById("amenityCustomInput").value="",i.style.display="flex",typeof lucide<"u"&&lucide.createIcons()}function it(e,t){const o=e.style.borderColor==="rgb(46, 125, 91)";e.style.borderColor=o?"#e5e7eb":"#2E7D5B",e.style.background=o?"white":"#E8F5EE",e.style.color=o?"#1a1a2e":"#2E7D5B"}function K(){document.getElementById("amenityPickerModal").style.display="none",me=null}function nt(){const e=s.editRooms.find(r=>r.id===me);if(!e){K();return}const t=document.getElementById("amenityPickerGrid"),o=[];t.querySelectorAll("button").forEach(r=>{if(r.style.background==="rgb(232, 245, 238)"){const n=Pe.find(a=>a.key===r.dataset.key);n&&o.push(n.label)}});const i=document.getElementById("amenityCustomInput").value.trim();i&&o.push(i),e.amenities=o.join(" • "),K(),ge(),typeof lucide<"u"&&lucide.createIcons()}function rt(e){Ce(e)}function at(e,t){const o=s.editRooms.find(r=>r.id===e);if(!o)return;const i=(o.amenities||"").split("•").map(r=>r.trim()).filter(Boolean);o.amenities=i.filter(r=>r!==t).join(" • "),ge(),typeof lucide<"u"&&lucide.createIcons()}async function st(){const e=document.getElementById("edit-hotel-name")?.value.trim(),t=document.getElementById("edit-hotel-subtitle")?.value.trim(),o=document.getElementById("edit-hotel-address")?.value.trim(),i=document.getElementById("edit-hotel-phone")?.value.trim(),r=document.getElementById("edit-hotel-policy")?.value.trim();try{await api("POST","/api/crm/hotel-info",{name:e,subtitle:t,address:o,phone:i,cancellationPolicy:r}),toast("Hotel info saved!","success")}catch{toast("Failed to save","error")}}async function dt(){const e=parseFloat(document.getElementById("edit-rate-nightly")?.value)||69,t=parseFloat(document.getElementById("edit-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("edit-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:e,weekly:t,monthly:o}),localStorage.setItem("ratesChanged","1"),s.launchStatus=null,N(),toast("Rates saved!","success")}catch{toast("Failed to save rates","error")}}async function lt(){const e=document.getElementById("edit-new-pin")?.value.trim();if(!e||e.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:e}),s.token=e;try{localStorage.setItem("crmToken",s.token)}catch{}toast("PIN updated!","success")}catch{toast("Failed to change PIN","error")}}function pt(e){navigator.clipboard.writeText(e).then(()=>{toast("Booking link copied!","success")}).catch(()=>{toast("Failed to copy","error")})}function ct(e){const t=e.nextElementSibling,o=e.querySelector(".accordion-arrow");t.style.display==="none"?(t.style.display="block",o&&(o.style.transform="rotate(90deg)")):(t.style.display="none",o&&(o.style.transform="rotate(0deg)"))}let G=!1;function $e(){if(document.getElementById("goLiveOverlay"))return;const e=document.createElement("div");e.id="goLiveOverlay",e.style.cssText="position:fixed;inset:0;z-index:99999;background:rgba(255,255,255,0.94);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;",e.innerHTML='<div class="logo-sprite-bounce"></div><div style="font-size:14px;font-weight:700;color:#1a5c3f;">Opening secure checkout…</div><div style="font-size:12px;color:#6b7280;">Taking you to Stripe — one moment</div>',document.body.appendChild(e)}function se(){const e=document.getElementById("goLiveOverlay");e&&e.remove()}async function ut(){if(!G){G=!0,$e();try{const e=await api("POST","/api/crm/go-live");if(e.success&&e.url){window.location.href=e.url;return}se(),G=!1,toast(e.message||"Failed to start checkout","error")}catch{se(),G=!1,toast("Failed to start checkout. Try again.","error")}}}async function gt(){try{const e=await api("GET","/api/crm/billing-portal");e.success&&e.url?window.location.href=e.url:toast(e.message||"Contact support@bookmarketel.com to manage your subscription.","error")}catch{toast("Contact support@bookmarketel.com to manage your subscription.","error")}}async function mt(){const e=document.getElementById("supportMessage")?.value.trim();if(!e){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:e}),document.getElementById("supportMessage").value="",toast("Message sent! We'll reply to your email.","success")}catch{toast("Failed to send. Email support@bookmarketel.com directly.","error")}}async function xt(e){const t=s.editRooms.find(d=>d.id===e);if(!t){toast("Room not found — try refreshing","error");return}const o=document.getElementById("edit-name-"+e)?.value.trim(),i=document.getElementById("edit-desc-"+e)?.value.trim(),r=parseInt(document.getElementById("edit-occ-"+e)?.value)||4,n=parseInt(document.getElementById("edit-units-"+e)?.value)||1,a={id:e,name:o||t.name,description:i||"",amenities:t.amenities||"",maxOccupancy:r,totalUnits:n};try{const d=await api("POST","/api/crm/rooms",a);if(d&&d.success===!1){toast(d.message||"Failed to save","error");return}t.name=a.name,t.description=a.description,t.maxOccupancy=r,t.totalUnits=n,toast("Room saved!","success")}catch(d){toast("Failed to save: "+(d.message||""),"error")}}async function ft(e,t){const o=Array.from(e.target.files);if(!o.length)return;const r=document.getElementById("edit-card-"+t)?.querySelector("div:first-child");r&&(r.style.position="relative",r.insertAdjacentHTML("beforeend",'<div id="upload-spinner-'+t+'" style="position:absolute;inset:0;background:rgba(255,255,255,0.8);display:flex;align-items:center;justify-content:center;z-index:5;flex-direction:column;gap:6px;"><div style="width:24px;height:24px;border:3px solid var(--border);border-top-color:var(--green);border-radius:50%;animation:spin 0.7s linear infinite;"></div><div id="upload-progress-'+t+'" style="font-size:12px;color:var(--text-muted);font-weight:600;">0 / '+o.length+"</div></div>"));let n=0,a="";for(const c of o){try{const h=await pe(t,c);if(h.image){const v=s.editRooms.find(S=>S.id===t);v&&(v.images||(v.images=[]),v.images.push(h.image),v.imageUrl||(v.imageUrl=h.image.url)),n++}}catch(h){a=h.message||"Upload failed"}const g=document.getElementById("upload-progress-"+t);g&&(g.textContent=n+" / "+o.length)}const d=document.getElementById("upload-spinner-"+t);d&&d.remove(),O(),n>0&&(s.launchStatus=null),N(),n>0?toast(n+" photo"+(n!==1?"s":"")+" added. Check the Bookings tab to continue your launch checklist!","success"):toast(a||"Upload failed","error")}function Me(e,t=512){return new Promise((o,i)=>{const r=new Image,n=URL.createObjectURL(e);r.onload=()=>{try{const a=Math.min(r.naturalWidth,r.naturalHeight),d=(r.naturalWidth-a)/2,c=(r.naturalHeight-a)/2,g=document.createElement("canvas");g.width=t,g.height=t;const h=g.getContext("2d");h.imageSmoothingQuality="high",h.drawImage(r,d,c,a,a,0,0,t,t),URL.revokeObjectURL(n),g.toBlob(v=>v?o(v):i(new Error("crop failed")),"image/png",.92)}catch(a){URL.revokeObjectURL(n),i(a)}},r.onerror=()=>{URL.revokeObjectURL(n),i(new Error("load failed"))},r.src=n})}function Le(){const e=document.getElementById("appsAppIconPreview");e&&(e.innerHTML='<div style="width:24px;height:24px;border:3px solid rgba(255,255,255,0.5);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;"></div>')}function xe(e){const t=document.getElementById("appsAppIconPreview");t&&(t.style.background="#fff",t.style.border="1px solid var(--border)",t.style.padding="0",t.innerHTML='<img src="'+e+'" alt="App icon" style="width:100%;height:100%;object-fit:contain;">')}function de(){const e=document.getElementById("appsAppIconPreview");if(!e)return;if(s.activeHotelAppIcon){xe(s.activeHotelAppIcon);return}const t=(s.activeHotelName||"H").trim().charAt(0).toUpperCase()||"🏨";e.style.background="transparent",e.style.border="none",e.style.padding="0",e.innerHTML='<span style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:var(--green);color:#fff;border-radius:14px;font-size:24px;font-weight:800;">'+t+"</span>"}async function yt(e){const t=e.files&&e.files[0];if(!t)return;Le();const o=new FormData;try{const i=await Me(t,512);o.append("icon",i,"app-icon.png")}catch{o.append("icon",t)}try{const i=le(),r=new URLSearchParams;s.activeHotelId&&r.set("hotelId",s.activeHotelId),i&&r.set("token",i);const a=await(await fetch(`/api/crm/hotel-app-icon?${r}`,{method:"POST",headers:{"x-crm-token":i},body:o})).json();if(a.success&&a.appIconUrl){s.activeHotelAppIcon=a.appIconUrl,xe(a.appIconUrl);const d=document.getElementById("appsView");d&&(d.dataset.appsKey=(s.activeHotelId||"")+"|"+a.appIconUrl+"|"+(s.activeHotelDomain||"")),typeof updateFrontdeskManifestLink=="function"&&updateFrontdeskManifestLink(),toast("Logo updated! Guests will see it on their phone.","success")}else toast(a.message||"Failed to upload icon","error"),de()}catch{toast("Failed to upload icon","error"),de()}e.value=""}async function ht(e,t){if(confirm("Delete this photo?"))try{await api("DELETE",`/api/crm/rooms/${e}/images/${t}`);const o=s.editRooms.find(i=>i.id===e);o&&o.images&&(o.images=o.images.filter(i=>i.id!==t),o.imageUrl=o.images[0]?.url||null),O(),toast("Photo deleted","success")}catch{toast("Failed to delete","error")}}async function bt(e){if(confirm("Delete this room type?"))try{await api("DELETE",`/api/crm/rooms/${e}`),toast("Room deleted","success"),ue()}catch{toast("Failed to delete","error")}}function vt(){const e=document.getElementById("editRoomsList");document.getElementById("editAddForm")||(e.insertAdjacentHTML("beforeend",`
  ...
  `),document.getElementById("editNewRoomName").focus())}function wt(){const e=document.getElementById("editNewRoomName").value.trim();e&&api("POST","/api/crm/rooms",{name:e,maxOccupancy:4,totalUnits:5}).then(()=>{toast("Room added","success"),ue()}).catch(()=>toast("Failed to add","error"))}const Re={addAmenityPrompt:rt,advanceTourIfNeeded:N,bookingsFrontdeskNudgeHtml:ke,changePin:lt,checklistGoTo:je,checklistGoToRates:Ne,cleanupPostActivationTourUi:J,cleanupSettingsTourUi:ee,closeAmenityPicker:K,confirmAmenityPicker:nt,confirmEditAddRoom:wt,copyBookingLink:pt,copyBookingLinkFromChecklist:Oe,deleteEditImage:ht,deleteEditRoom:bt,ensureTourBlurOverlay:H,finishPostActivationTour:Y,frontdeskInstallCardHtml:ne,getAmenityIcon:Ae,getCrmAuthToken:le,getEditRoomImages:Ie,goLive:ut,guestBookingEngineUrl:ve,handleInstallFrontdesk:Xe,handoffToGuestAppsTour:tt,hideGoLiveOverlay:se,loadEditRooms:ue,loadSettings:be,maybePromptInstalledNotifications:et,openAmenityPicker:Ce,openBillingPortal:gt,openEditAddRoom:vt,openGuestBookingEngine:Ye,openPreviewSite:Ge,openTourAccordion:L,postRoomImageUpload:pe,queryTourSelector:C,refreshAppsInstallSection:Z,refreshFrontdeskInstallCard:X,removeAmenity:at,renderEditRooms:ge,renderEditRoomsCards:O,replayWalkthrough:Je,resolveLiveTourElement:_,resolveTourHighlightEl:F,restoreAppIconPreview:de,saveEditRoom:xt,saveHotelInfo:st,saveRates:dt,scrollTourTargetIntoView:Te,sendSupportMessage:mt,setAppIconPreviewImage:xe,setAppIconPreviewLoading:Le,settingsChangePin:Ve,settingsCopyLink:De,settingsSaveRates:Ue,settingsSendSupport:We,settingsUploadPhoto:qe,showActivatedModal:Ke,showEditRoomPhoto:Be,showFinaleMockModal:D,showGoLiveOverlay:$e,showIosInstallSheet:Se,showNotifPromptModal:Ee,showOnboardingQuestions:Qe,showTestDriveModal:ze,showWelcomeModal:we,squareCropImage:Me,startPostActivationTabTour:ce,startSettingsTour:ae,stepEditRoomPhoto:ot,toggleAmenityPreset:it,toggleAppNotifications:Ze,toggleSection:ct,tourAnchorRect:Q,tourElementRect:re,updatePreviewSiteBar:_e,uploadAppIcon:yt,uploadEditImages:ft};function kt(){Fe(Re)}const Tt=Object.freeze(Object.defineProperty({__proto__:null,default:Re,install:kt},Symbol.toStringTag,{value:"Module"}));export{Et as a,Tt as b,s as c,Fe as e,St as s};
