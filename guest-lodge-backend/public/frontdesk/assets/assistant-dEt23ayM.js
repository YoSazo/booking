import{e as O,c as i,f as w}from"./settings-Bc7IRK4y.js";let I=!1,m=null;const H=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function D(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function _(t,e,a){let n=null;const o=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,o]).finally(()=>clearTimeout(n))}function s(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function A(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function S(){return(i.assistantData?.recipients||[]).filter(t=>t.active)}function F(){return S().filter(t=>t.verified)}function N(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function C(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function j(){if(!C())return!1;try{const t=window.webkit?.messageHandlers?.marketelShell;return!t||typeof t.postMessage!="function"?!1:(t.postMessage({type:"openAssistant"}),!0)}catch{return!1}}function V(){const t=[i.editRooms,i.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const a=String(e[0]?.name||"").trim();if(a)return a}return"Queen Room"}function R(){if(i.assistantData?.latestResult)return i.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(i.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function U(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const n=Math.floor(a/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const o=Math.floor(n/24);return o<7?`${o} day${o===1?"":"s"} ago`:E(t)}function Y(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(i.nativeNotificationState||""),a=e==="registered",n=e==="authorized",o=e==="unavailable";return`<div class="fda-section">
    <div class="fda-section-title">${a?"Booking alerts are on":n?"Connecting this iPhone":o?"Booking alerts need attention":"Turn on booking alerts"}</div>
    <div class="fda-section-sub">${a?"This iPhone can receive new-booking and room-check alerts even when Front Desk is closed.":n?"Notification access is allowed. Refresh once to finish connecting this iPhone.":o?"Front Desk could not register this iPhone for alerts. Check notification settings, then refresh.":"Allow notifications so Front Desk can warn you when a booking or room check needs attention."}</div>
  ...
  </div>`}function E(t,e={}){if(!t)return"";const a=new Date(t);if(!Number.isFinite(a.getTime()))return"";const n=Date.now(),o=Math.round((a.getTime()-n)/6e4);if(e.relative&&o>0&&o<60)return`in ${o} min`;if(e.relative&&o>=60&&o<1440){const d=Math.round(o/60);return`in ${d} hr${d===1?"":"s"}`}return a.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function W(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function z(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const a=Number(e[1]);if(!Number.isInteger(a)||a<0||a>23)return"Not set";const n=a>=12?"PM":"AM";return`${a%12||12}:${e[2]} ${n}`}function $(t,e,a=""){const n=s(e||"");return`<div class="fda-time-control${a?" is-disabled":""}">
    <span class="fda-time-value">${s(z(e))}</span>
    <input id="${s(t)}" type="time" value="${n}" ${a} oninput="updateAssistantTimeDisplay(this)">
  ...
  `,document.head.appendChild(t)}async function f({force:t=!1}={}){if(i.assistantData&&!t)return i.assistantData;if(m&&!t)return m;const e=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",k(),m=_(l("GET","/api/crm/frontdesk-assistant"),H,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==e?null:(i.assistantData=a.data,a.data)}).catch(a=>{throw i.activeHotelId===e&&(i.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{i.assistantLoading=!1,m=null,k(),document.getElementById("frontDeskAssistantOverlay")&&g()}),m}const M="marketelAssistantConfigured";function G(){try{return localStorage.getItem(M+":"+(i.activeHotelId||""))==="1"}catch{return!1}}function Z(t){try{const e=M+":"+(i.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function J(){const t=i.assistantData,e=S(),a=t?!!t.config?.enabled&&e.length>0:G();t&&Z(a);const n=R(),o=!!n&&(n.type==="availability_warning"||n.status==="attention");window.marketelAssistantPillLabel=a?o?"Needs your review":"Front Desk":"Set up Front Desk",window.syncNativeShellState?.(),!t&&!i.assistantLoading&&!i.assistantError&&f().catch(()=>{})}function Q(){window.marketelAssistantPillLabel="Front Desk"}function k(){T();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(C()){const r=R();if(!i.assistantData||!r){!i.assistantData&&!i.assistantLoading&&!i.assistantError&&f().catch(()=>{}),t.innerHTML="",t.style.display="none";return}const p=r.type==="availability_warning"||r.status==="attention";t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result${p?" attention":""}" onclick="openFrontDeskAssistant()" aria-label="Open Front Desk Assistant activity">
      <span class="fda-native-result-icon" aria-hidden="true">${p?"!":"✓"}</span>
      <span class="fda-native-result-copy">
        <span class="fda-native-result-label">${p?"Front Desk needs your review":"Front Desk handled this"}</span>
        <span class="fda-native-result-title">${s(r.summary||"Assistant updated your property")}</span>
        <span class="fda-native-result-time">${s(U(r.createdAt))} · View activity</span>
      </span>
      ...
      <div class="fda-card-row">
        <div class="fda-card-icon fda-card-app-icon"><img src="${w}" width="42" height="42" alt=""></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${r?s(i.assistantError):"Tell it when a walk-in or another channel takes a room. It updates Availability and your direct booking page."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${r?"Retry":"Open"}</button>
      </div>
    </div>`,i.assistantLoading||f().catch(()=>{});return}const a=F(),n=i.assistantData.config||{},o=i.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${E(n.nextCheckAt,{relative:!0})}`:"",p=o.enabled?` · no reply ${o.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon fda-card-app-icon"><img src="${w}" width="42" height="42" alt=""></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${s(W(n.checkFrequency))}${s(r)}${s(p)}</div>
        </div>
  ...
  </div>`}function K(t){const e=s((t.name||"?").charAt(0).toUpperCase()),a=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      <div class="fda-avatar">${e}</div>
      <div class="fda-person-copy">
        <div class="fda-person-name">${s(t.name)}</div>
        <div class="fda-person-meta">${s(a)}</div>
      </div>
    ...
    ${t.verified?"":`<div class="fda-verify">
      <input id="assistant-code-${s(t.id)}" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="6-digit code" aria-label="Verification code">
  ...
  </div>`}function X(){const t=(i.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
    <div class="fda-activity-dot"></div>
    <div class="fda-activity-copy">
      ${s(e.summary||"Front Desk activity")}
      <div class="fda-activity-time">${s(E(e.createdAt))}${e.status?` · ${s(e.status)}`:""}</div>
    </div>
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function tt(){const t=i.assistantData;if(!t)return i.assistantError&&!i.assistantLoading?`<div class="fda-section">
        <div class="fda-section-title">Assistant could not connect</div>
        <div class="fda-section-sub">${s(i.assistantError)}</div>
        <button type="button" class="fda-btn primary full" onclick="retryFrontDeskAssistant()">Retry</button>
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},a=t.bookingApproval||{},n=S(),o=t.capabilities||{},d=N(),r=Number(o.maxRecipients||3),p=n.length<r,u=d?"":"disabled",v=d&&o.manualAvailability?"":"disabled",B=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",L=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),y=[];o.smsConfigured||y.push("Marketel's texting number still needs its server credentials."),o.manualAvailability||y.push("The Assistant works with properties whose Availability is managed in Marketel.");const q=y.length?`<div class="fda-section fda-lock">
        <div class="fda-section-title">Not ready on this property yet</div>
        <div class="fda-section-sub" style="margin:0;">${y.map(h=>s(h)).join(" ")}</div>
      </div>`:"";return`
    ${!!e.enabled&&n.length>0?"":`<div class="fda-section fda-story">
      ...
      <div class="fda-bubble assistant"><strong>Front Desk</strong><br>New booking: ${s(V())}, tonight. Is it still free?</div>
    ...
    ${!d&&!L?`<div class="fda-section fda-lock">
      <div class="fda-section-title">${i.marketelTrialEligible!==!1?"Included in your 14-day free trial":"Included when you reactivate"}</div>
      ...
      <button type="button" class="fda-btn primary full" onclick="activateFromAssistant()">${i.marketelTrialEligible!==!1?"Start 14 days free":"Reactivate Marketel"}</button>
    </div>`:""}
    ${q}
    ${Y()}
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Review new bookings before confirmation">
          <input id="assistant-approval-enabled" type="checkbox" ${a.enabled?"checked":""} ${v} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
          <span class="fda-switch-track" aria-hidden="true"></span>
        </label>
      </div>
    ...
    <div class="fda-section">
      ...
      ${n.length?n.map(K).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
      ${p?`<div class="fda-grid" style="margin-top:8px;">
      ...
      <button type="button" class="fda-btn secondary full" onclick="addAssistantRecipient()" ${d&&o.smsConfigured?"":"disabled"}>Send verification code</button>`:""}
      ${o.assistantPhone?'<button type="button" class="fda-icon-btn" style="margin-top:10px;" onclick="saveAssistantContact()">Save “Marketel Front Desk” to contacts</button>':""}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label>${$("assistant-check-time",e.dailyCheckTime||"18:00",u)}</div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${s(B)}" ${u}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label>${$("assistant-quiet-start",e.quietHoursStart||"",u)}</div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label>${$("assistant-quiet-end",e.quietHoursEnd||"",u)}</div>
      </div>
      ...
      <button type="button" class="fda-btn primary full" style="margin-top:14px;" onclick="saveAssistantSettings()" ${d&&o.smsConfigured&&o.manualAvailability?"":"disabled"}>Save assistant settings</button>
      <div class="fda-actions">
        <button type="button" class="fda-btn secondary" onclick="sendAssistantTest()" ${F().length&&d&&o.smsConfigured?"":"disabled"}>Send test text</button>
        <button type="button" class="fda-btn secondary" onclick="runAssistantCheckNow()" ${F().length&&d&&o.smsConfigured?"":"disabled"}>Ask for an update now</button>
      </div>
    </div>
  ...
  `}function g(){const t=document.getElementById("frontDeskAssistantSheet");t&&(t.innerHTML=`<div class="fda-sheet-head">
  ...
  <div class="fda-sheet-body">${tt()}</div>`,P())}function et(){document.getElementById("frontDeskAssistantSheet")&&g()}function P(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,a=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${a} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${a} minutes keeps the booking and emails the guest a confirmation.`}function at(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=z(t.value))}function it(){D(!1),T();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&x()}),document.body.appendChild(t)),document.body.style.overflow="hidden",g(),requestAnimationFrame(()=>D(!1)),i.assistantData?f({force:!0}).catch(()=>{}):f().catch(()=>{})}function nt(){j()||it()}function x(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",D(!0)}function ot(){i.assistantError="",g(),f({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(i.assistantData=t.data),k(),g(),e&&c(e,"success"),t}async function st(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),a=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!a)return c("Enter a name and mobile number.","error");try{const n=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:a});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function rt(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(a){c(a.message||"That code could not be verified.","error")}}async function dt(t){try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function ct(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function lt(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await l("PUT","/api/crm/frontdesk-assistant",t));const a=await l("POST","/api/crm/booking-approval",e);if(!a?.success)throw new Error(a?.message||"Could not save the booking rule.");await f({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(a){await f({force:!0}).catch(()=>{}),c(a.message||"Could not save the assistant.","error")}}async function ft(){try{return b(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function pt(){try{b(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function ut(){const t=i.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const o=window.webkit?.messageHandlers?.marketelShell;if(o&&typeof o.postMessage=="function"){o.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),a=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=a,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(a),2e3)}async function bt(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){x(),c("Front Desk app access is managed with your Marketel account.","info");return}x();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const ht={activateFromAssistant:bt,addAssistantRecipient:st,closeFrontDeskAssistant:x,loadFrontDeskAssistant:f,openFrontDeskAssistant:nt,removeAssistantRecipient:ct,refreshFrontDeskAssistantSheet:et,renderFrontDeskAssistantCard:k,renderAssistantPill:J,hideAssistantPill:Q,retryFrontDeskAssistant:ot,resendAssistantCode:dt,runAssistantCheckNow:pt,saveAssistantContact:ut,saveAssistantSettings:lt,sendAssistantTest:ft,updateAssistantPolicySummary:P,updateAssistantTimeDisplay:at,verifyAssistantRecipient:rt};function yt(){I||(I=!0,T(),O(ht))}export{bt as activateFromAssistant,st as addAssistantRecipient,x as closeFrontDeskAssistant,Q as hideAssistantPill,yt as install,f as loadFrontDeskAssistant,nt as openFrontDeskAssistant,et as refreshFrontDeskAssistantSheet,ct as removeAssistantRecipient,J as renderAssistantPill,k as renderFrontDeskAssistantCard,dt as resendAssistantCode,ot as retryFrontDeskAssistant,pt as runAssistantCheckNow,ut as saveAssistantContact,lt as saveAssistantSettings,ft as sendAssistantTest,P as updateAssistantPolicySummary,at as updateAssistantTimeDisplay,rt as verifyAssistantRecipient};
