import{e as L,c as i}from"./settings-B4WoF5xs.js";let S=!1,h=null;const q=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function A(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function P(t,e,a){let n=null;const s=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function k(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(i.assistantData?.recipients||[]).filter(t=>t.active)}function D(){return E().filter(t=>t.verified)}function I(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function $(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function O(){if(i.assistantData?.latestResult)return i.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(i.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function H(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const n=Math.floor(a/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:F(t)}function _(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(i.nativeNotificationState||""),a=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function F(t,e={}){if(!t)return"";const a=new Date(t);if(!Number.isFinite(a.getTime()))return"";const n=Date.now(),s=Math.round((a.getTime()-n)/6e4);if(e.relative&&s>0&&s<60)return`in ${s} min`;if(e.relative&&s>=60&&s<1440){const d=Math.round(s/60);return`in ${d} hr${d===1?"":"s"}`}return a.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function j(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function N(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const a=Number(e[1]);if(!Number.isInteger(a)||a<0||a>23)return"Not set";const n=a>=12?"PM":"AM";return`${a%12||12}:${e[2]} ${n}`}function w(t,e,a=""){const n=o(e||"");return`<div class="fda-time-control${a?" is-disabled":""}">
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(i.assistantData&&!t)return i.assistantData;if(h&&!t)return h;const e=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",g(),h=P(l("GET","/api/crm/frontdesk-assistant"),q,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==e?null:(i.assistantData=a.data,a.data)}).catch(a=>{throw i.activeHotelId===e&&(i.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{i.assistantLoading=!1,h=null,g(),document.getElementById("frontDeskAssistantOverlay")&&v()}),h}function g(){T();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!$()){t.innerHTML=`<div class="fda-card is-off">
    ...
    </div>`;return}if($()){const r=O();if(!i.assistantData||!r){if(!i.assistantData&&!i.assistantLoading&&!i.assistantError&&u().catch(()=>{}),i.assistantLoading){t.innerHTML="",t.style.display="none";return}t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result is-intro" onclick="openFrontDeskAssistant()" aria-label="Set up Front Desk Assistant">
    ...
    </div>`,i.assistantLoading||u().catch(()=>{});return}const a=D(),n=i.assistantData.config||{},s=i.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${F(n.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
  ...
  </div>`}function V(t){const e=o((t.name||"?").charAt(0).toUpperCase()),a=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    ...
    <div class="fda-activity-copy">
      ...
      <div class="fda-activity-time">${o(F(e.createdAt))}${e.status?` · ${o(e.status)}`:""}</div>
    </div>
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},a=t.bookingApproval||{},n=E(),s=t.capabilities||{},d=I(),r=Number(s.maxRecipients||3),f=n.length<r,p=d?"":"disabled",m=d&&s.manualAvailability?"":"disabled",M=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",z=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),R=s.smsConfigured?"":`<div class="fda-section fda-lock"><div class="fda-section-title">Messaging is being connected</div><div class="fda-section-sub" style="margin:0;">The interface is ready, but Marketel's texting number still needs its server credentials before it can send.</div></div>`,B=s.manualAvailability?"":'<div class="fda-section fda-lock"><div class="fda-section-title">Availability updates are not available here yet</div><div class="fda-section-sub" style="margin:0;">The Assistant currently works with properties whose Availability is managed directly in Marketel.</div></div>';return`
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Review new bookings before confirmation">
          <input id="assistant-approval-enabled" type="checkbox" ${a.enabled?"checked":""} ${m} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
          <span class="fda-switch-track" aria-hidden="true"></span>
        </label>
      </div>
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        ...
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${o(M)}" ${p}></div>
      </div>
      <div class="fda-grid">
