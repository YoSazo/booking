import{e as L,c as a}from"./settings-CP4ZY9Ws.js";let S=!1,h=null;const P=12e3;function l(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function A(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function q(t,e,i){let n=null;const s=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function k(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function $(){return E().filter(t=>t.verified)}function N(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function D(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function O(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function H(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:F(t)}function _(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(h&&!t)return h;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",g(),h=q(l("GET","/api/crm/frontdesk-assistant"),P,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,h=null,g(),document.getElementById("frontDeskAssistantOverlay")&&m()}),h}function g(){T();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!D()){t.innerHTML=`<div class="fda-card is-off">
    ...
    </div>`,a.assistantLoading||u().catch(()=>{});return}const i=$(),n=a.assistantData.config||{},s=a.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${F(n.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
    ...
    <div class="fda-activity-copy">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},n=E(),s=t.capabilities||{},d=N(),r=Number(s.maxRecipients||3),f=n.length<r,p=d?"":"disabled",v=d&&s.manualAvailability?"":"disabled",z=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",M=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),R=s.smsConfigured?"":`<div class="fda-section fda-lock"><div class="fda-section-title">Messaging is being connected</div><div class="fda-section-sub" style="margin:0;">The interface is ready, but Marketel's texting number still needs its server credentials before it can send.</div></div>`,B=s.manualAvailability?"":'<div class="fda-section fda-lock"><div class="fda-section-title">Availability updates are not available here yet</div><div class="fda-section-sub" style="margin:0;">The Assistant currently works with properties whose Availability is managed directly in Marketel.</div></div>';return`
