import{e as B,c as a}from"./settings-C-XMJ1IW.js";let T=!1,h=null;const P=12e3;function l(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function A(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function q(t,e,i){let n=null;const s=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function k(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function S(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function $(){return S().filter(t=>t.verified)}function E(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function L(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function O(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function H(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:D(t)}function _(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function D(t,e={}){if(!t)return"";const i=new Date(t);if(!Number.isFinite(i.getTime()))return"";const n=Date.now(),s=Math.round((i.getTime()-n)/6e4);if(e.relative&&s>0&&s<60)return`in ${s} min`;if(e.relative&&s>=60&&s<1440){const d=Math.round(s/60);return`in ${d} hr${d===1?"":"s"}`}return i.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function j(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function N(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const i=Number(e[1]);if(!Number.isInteger(i)||i<0||i>23)return"Not set";const n=i>=12?"PM":"AM";return`${i%12||12}:${e[2]} ${n}`}function w(t,e,i=""){const n=o(e||"");return`<div class="fda-time-control${i?" is-disabled":""}">
    <span class="fda-time-value">${o(N(e))}</span>
    <input id="${o(t)}" type="time" value="${n}" ${i} oninput="updateAssistantTimeDisplay(this)">
  </div>`}function F(){if(document.getElementById("frontDeskAssistantStyles"))return;const t=document.createElement("style");t.id="frontDeskAssistantStyles",t.textContent=`
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(h&&!t)return h;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",v(),h=q(l("GET","/api/crm/frontdesk-assistant"),P,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,h=null,v(),document.getElementById("frontDeskAssistantOverlay")&&m()}),h}function v(){F();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(L()){const r=O();if(!a.assistantData||!r){t.innerHTML="",t.style.display="none",!a.assistantData&&!a.assistantLoading&&!a.assistantError&&u().catch(()=>{});return}const f=r.type==="availability_warning"||r.status==="attention";t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result${f?" attention":""}" onclick="openFrontDeskAssistant()" aria-label="Open Front Desk Assistant activity">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${o(H(r.createdAt))} · View activity</span>
      </span>
      ...
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${r?o(a.assistantError):"It updates availability for you and helps prevent outside bookings from colliding."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${r?"Retry":"Open"}</button>
      </div>
    </div>`,a.assistantLoading||u().catch(()=>{});return}const i=$(),n=a.assistantData.config||{},s=a.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${D(n.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(j(n.checkFrequency))}${o(r)}${o(f)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const d=!E();t.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon">💬</div>
      <div class="fda-card-copy">
  ...
  </div>`}function V(t){const e=o((t.name||"?").charAt(0).toUpperCase()),i=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
  ...
  </div>`}function U(){const t=(a.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
    <div class="fda-activity-dot"></div>
    <div class="fda-activity-copy">
      ${o(e.summary||"Front Desk activity")}
      <div class="fda-activity-time">${o(D(e.createdAt))}${e.status?` · ${o(e.status)}`:""}</div>
    </div>
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function Y(){const t=a.assistantData;if(!t)return a.assistantError&&!a.assistantLoading?`<div class="fda-section">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},n=S(),s=t.capabilities||{},d=E(),r=Number(s.maxRecipients||3),f=n.length<r,p=d?"":"disabled",g=d&&s.manualAvailability?"":"disabled",I=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",z=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),M=s.smsConfigured?"":`<div class="fda-section fda-lock"><div class="fda-section-title">Messaging is being connected</div><div class="fda-section-sub" style="margin:0;">The interface is ready, but Marketel's texting number still needs its server credentials before it can send.</div></div>`,R=s.manualAvailability?"":'<div class="fda-section fda-lock"><div class="fda-section-title">Availability updates are not available here yet</div><div class="fda-section-sub" style="margin:0;">The Assistant currently works with properties whose Availability is managed directly in Marketel.</div></div>';return`
    ...
    ${!d&&!z?`<div class="fda-section fda-lock">
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Review new bookings before confirmation">
          <input id="assistant-approval-enabled" type="checkbox" ${i.enabled?"checked":""} ${g} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
          <span class="fda-switch-track" aria-hidden="true"></span>
        </label>
      </div>
      <div class="fda-field" style="margin-top:13px;">
        ...
        <select id="assistant-approval-window" ${g} onchange="updateAssistantPolicySummary()">
          ${[5,10,15,20,30,45,60].map(x=>`<option value="${x}" ${Number(i.windowMinutes||20)===x?"selected":""}>${x} minutes</option>`).join("")}
        </select>
      </div>
      ...
      <div class="fda-policy-options">
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="confirm" ${i.noResponseAction!=="release"?"checked":""} ${g} onchange="updateAssistantPolicySummary()">
        ...
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="release" ${i.noResponseAction==="release"?"checked":""} ${g} onchange="updateAssistantPolicySummary()">
