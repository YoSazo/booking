const l={token:"",bookings:[],guestMessages:[],currentFilter:"settings",bookingCallFilter:"all",manualAvailability:{rooms:[],overrides:{}},manualSelectedRoom:"",availabilityYear:new Date().getFullYear(),availabilityMonth:new Date().getMonth(),availabilityEditingDay:"",availabilityDaySaving:!1,editingRoomName:"",pendingDeleteRoomName:"",currentHotelPms:"",revenueEnabled:!1,hotelSubscribed:!1,revenuePeriod:"30d",revenueCache:{},revenueLoading:!1,revenueError:"",blockedDemand:{total:0,today:0,recent:[]},bookingsSubview:"bookings",launchStatus:null,growthFunnel:null,growthChecklist:{},growthPeriod:"30d",ALLOWED_REVENUE_PERIODS:new Set(["today","7d","30d","all"]),OTA_COMMISSION_RATE:.25,activeHotelId:"",activeHotelName:"",activeHotelAppIcon:"",appsViewPlatform:"ios",activeHotelDomain:"",activeHotelContext:null,settingsTourActive:!1,bootInFlight:!1,CRM_HOTEL_BY_HOST:{"guestlodgeminot.clickinns.com":"guest-lodge-minot","booking-kappa-nine.vercel.app":"guest-lodge-minot","stcroix.clickinns.com":"st-croix-wisconsin","homeplacesuites.clickinns.com":"home-place-suites","myhomeplacesuites.com":"home-place-suites","www.myhomeplacesuites.com":"home-place-suites","suitestay.clickinns.com":"suite-stay","clickinns.com":"suite-stay","www.clickinns.com":"suite-stay"},CRM_HOTEL_LABELS:{"guest-lodge-minot":"Guest Lodge Minot","st-croix-wisconsin":"St. Croix Wisconsin","home-place-suites":"Home Place Suites","suite-stay":"Suite Stay"},deferredInstallPrompt:null,frontdeskInstalled:!1,_magicLoginPending:!1,editRooms:[],editRoomsLoadPromise:null,messageUnreadCount:0,messagesInboxOpen:!1,messagesThreadPickerOpen:!1,selectedMessageThread:"",bookingsVirtualList:[],bookingsVirtualRaf:0};let G=null;function me(){return typeof lucide<"u"?Promise.resolve():G||(G=new Promise((t,e)=>{const o=document.createElement("script");o.src="https://unpkg.com/lucide@0.469.0/dist/umd/lucide.min.js",o.async=!0,o.onload=()=>t(),o.onerror=()=>e(new Error("lucide load failed")),document.head.appendChild(o)}),G)}async function Pt(t){if(!t||!t.type.startsWith("image/")||t.type==="image/webp"&&t.size<4e5)return t;try{const e=await createImageBitmap(t),o=1600,i=1200;let n=e.width,a=e.height;const r=Math.min(1,o/n,i/a);n=Math.round(n*r),a=Math.round(a*r);const s=document.createElement("canvas");s.width=n,s.height=a,s.getContext("2d").drawImage(e,0,0,n,a),e.close();const p=await new Promise((h,v)=>{s.toBlob(S=>S?h(S):v(new Error("encode failed")),"image/webp",.82)}),g=(t.name||"room-photo").replace(/\.[^.]+$/,"")||"room-photo";return new File([p],g+".webp",{type:"image/webp"})}catch{return t}}function xe(){const t=()=>{l.currentFilter==="bookings"?loadMessages():loadMessageBadges()};"requestIdleCallback"in window?requestIdleCallback(t,{timeout:2500}):setTimeout(t,600)}function Ct(t){Object.assign(window,t)}async function ft(){const t=document.getElementById("settingsList");if(t){t.innerHTML='<div class="loading"><div class="logo-sprite-bounce"></div> Loading…</div>';try{const e=await api("GET","/api/crm/verify"),i="https://"+(e?.domain||l.activeHotelId+".mktel.co"),n="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(i),a=await api("GET","/api/crm/rooms");let r={nightly:69,weekly:299,monthly:999};a?.rates&&(r=a.rates);const s=a?.rooms||[];let p="";e?.subscribed||(p+=goLiveInlineCardHtml()),s.length?s.forEach(g=>{const h=g.images&&g.images.length>0;p+=`
    ...
    `,t.innerHTML=p}catch{t.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load settings</div></div>'}}}function $t(){const t=document.getElementById("settings-booking-url");t&&navigator.clipboard.writeText(t.value).then(()=>{localStorage.setItem("linkCopied","1"),N(),toast("Link copied!","success")}).catch(()=>toast("Copy failed","error"))}function Mt(){localStorage.setItem("settingsTourDone","1");const t=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",t);let e=0;const o=setInterval(()=>{e++;const i=document.getElementById("edit-rate-nightly");if(i||e>20){if(clearInterval(o),!i)return;const n=i.closest(".accordion-body");if(n&&n.style.display==="none"){n.style.display="block";const a=n.previousElementSibling?.querySelector(".accordion-arrow");a&&(a.style.transform="rotate(90deg)")}setTimeout(()=>{i.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const a=document.getElementById("checklistPointer");a&&a.remove();const r=i.getBoundingClientRect(),s=document.createElement("div");s.id="checklistPointer",s.style.cssText=`position:fixed;z-index:100000;left:50%;transform:translateX(-50%);top:${r.bottom+12}px;max-width:240px;width:calc(100% - 40px);`,s.innerHTML=`
          ...
          `,document.body.appendChild(s),setTimeout(()=>{const p=document.getElementById("checklistPointer");p&&p.remove()},6e3)},1e3)},100)}},200)}function Lt(){const e="https://"+(l.activeHotelDomain||l.activeHotelId+".mktel.co");navigator.clipboard.writeText(e).then(()=>{localStorage.setItem("linkCopied","1"),N(),toast("Link copied!","success"),loadBookings()}).catch(()=>toast("Copy failed","error"))}function Rt(t,e){localStorage.setItem("settingsTourDone","1");const o=document.querySelector('[data-nav-filter="settings"]');setFilter("settings",o);let i=0;const n=setInterval(()=>{i++;const a=document.querySelector(t);if(a||i>20){if(clearInterval(n),!a)return;a.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{const r=document.getElementById("checklistPointer");r&&r.remove();const s=a.getBoundingClientRect(),p=document.createElement("div");p.id="checklistPointer",p.style.cssText=`
        ...
        `,document.body.appendChild(p),setTimeout(()=>{const g=document.getElementById("checklistPointer");g&&g.remove()},6e3)},1e3)}},200)}function at(){const t=String(l.token||localStorage.getItem("crmToken")||"").trim();return t&&(l.token=t),t}async function st(t,e){const o=at();if(!o)throw new Error("Not logged in");const i=await Pt(e),n=new FormData;n.append("image",i,i.name||"room.webp");const a=new URLSearchParams;l.activeHotelId&&a.set("hotelId",l.activeHotelId),a.set("token",o);const r=await fetch(`/api/crm/rooms/${t}/images?${a}`,{method:"POST",headers:{"x-crm-token":o},body:n}),s=await r.json().catch(()=>({}));if(!r.ok||!s.success)throw new Error(s.message||s.error||`Upload failed (${r.status})`);return s}async function Ht(t,e){const o=t.target.files[0];if(o)try{await st(e,o),toast("Photo uploaded!","success"),ft()}catch(i){toast(i.message||"Upload failed","error")}}async function Ft(){const t=parseFloat(document.getElementById("settings-rate-nightly")?.value)||69,e=parseFloat(document.getElementById("settings-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("settings-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:t,weekly:e,monthly:o}),toast("Rates saved","success")}catch{toast("Failed to save rates","error")}}async function Dt(){const t=document.getElementById("settings-new-pin")?.value.trim();if(!t||t.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:t}),l.token=t;try{localStorage.setItem("crmToken",l.token)}catch{}toast("PIN updated!","success"),document.getElementById("settings-new-pin").value=""}catch{toast("Failed to change PIN","error")}}async function Nt(){const t=document.getElementById("settings-support-msg")?.value.trim();if(!t){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:t}),toast("Message sent!","success"),document.getElementById("settings-support-msg").value=""}catch{toast("Failed to send","error")}}function Ot(){const t=l.activeHotelDomain||l.activeHotelId+".mktel.co",o=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:5173/?hotelId="+encodeURIComponent(l.activeHotelId)+"&preview=1":"https://"+t+"?preview=1";window.open(o,"_blank")}function yt(){if((window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1")&&l.activeHotelId)return"http://localhost:5173/?hotelId="+encodeURIComponent(l.activeHotelId);const e=l.activeHotelDomain||"";return e?"https://"+e+"/":""}function qt(){const t=yt();if(!t){toast("Your booking domain is still setting up.","info");return}window.open(t,"_blank")}function jt(){const t=document.getElementById("previewSiteBar");t&&(t.style.display=l.currentFilter==="settings"?"block":"none")}function N(){if(localStorage.getItem("settingsTourDone"))return;const t=parseInt(localStorage.getItem("settingsTourStep")||"0"),e=l.editRooms.some(r=>r.images&&r.images.length>0),o=!!localStorage.getItem("ratesChanged"),i=!!localStorage.getItem("linkCopied");t===1&&e&&localStorage.setItem("settingsTourStep","2"),t===2&&o&&localStorage.setItem("settingsTourStep","3"),t===3&&i&&localStorage.setItem("settingsTourStep","4");const n=document.getElementById("tourTooltip");n&&n.remove();const a=document.getElementById("tourBlurOverlay");a&&a.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(r=>{r.style.position=r.dataset.tourOrigPosition||"",r.style.zIndex="",r.style.isolation="",r.style.boxShadow="",r.removeAttribute("data-tour-highlighted"),delete r.dataset.tourOrigPosition}),document.body.style.overflow=""}function Ut(){let t=0;const e={},o=[{title:"Why do you want a booking engine?",key:"why",type:"text",placeholder:"e.g. I want guests to book directly instead of calling me..."},{title:"How do guests currently book with you?",key:"currentBooking",type:"choice",options:[{label:"They call me or walk in",value:"phone_walkin"},{label:"Through Booking.com / Expedia",value:"ota"},{label:"I have a website but no booking system",value:"website_no_booking"},{label:"I don't take bookings online yet",value:"no_online"}]},{title:"How many rooms do you have?",key:"roomCount",type:"choice",options:[{label:"1–5 rooms",value:"1-5"},{label:"6–15 rooms",value:"6-15"},{label:"16–50 rooms",value:"16-50"},{label:"50+ rooms",value:"50+"}]},{title:"What's most important to you?",key:"priority",type:"choice",options:[{label:"Stop paying OTA commissions",value:"no_commission"},{label:"Get more direct bookings",value:"more_bookings"},{label:"Have a professional online presence",value:"professional"},{label:"Make it easier for guests to book",value:"easier_booking"}]}];function i(){let n=document.getElementById("onboardingOverlay");if(n&&n.remove(),t>=o.length){localStorage.setItem("onboardingDone","1");try{api("POST","/api/crm/onboarding-answers",e).catch(()=>{})}catch{}ht();return}const a=o[t],r=document.createElement("div");r.id="onboardingOverlay",r.style.cssText="position:fixed;inset:0;z-index:100001;background:linear-gradient(135deg, #1a2b22 0%, #2E7D5B 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;",a.type==="text"?(r.innerHTML=`
      ...
      <div style="background:white;border-radius:20px;padding:28px 24px;max-width:340px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
      ...
      </div>`,document.getElementById("welcomeModalNext").onclick=e}document.body.appendChild(t),o(),typeof invokeLoadEditRooms=="function"&&invokeLoadEditRooms()}function J(){const t=document.getElementById("postActivationTourTooltip");t&&t.remove();const e=document.getElementById("postActivationTourOverlay");e&&e.remove(),document.querySelectorAll("[data-post-activation-highlight]").forEach(o=>{o.style.boxShadow="",o.style.position="",o.style.zIndex="",o.removeAttribute("data-post-activation-highlight")}),document.body.style.overflow=""}function Y(){J(),localStorage.setItem("postActivationTourDone","1");const t=document.querySelector('.tab[data-nav-filter="apps"]')||document.querySelector('.mobile-nav-item[data-nav-filter="apps"]');try{setFilter("apps",t)}catch{}}function dt(){if(localStorage.getItem("postActivationTourDone")){Y();return}J();const t=[{tab:"bookings",navFilter:"bookings",text:"<strong>Bookings</strong> — live reservations land here. You'll get a push alert for each new one."},{tab:"apps",navFilter:"apps",text:"<strong>Phones</strong> — put your guest booking app on their home screen and send install reminders."}];let e=0;function o(){if(J(),e>=t.length){Y();return}const i=t[e],n=document.querySelector(`.tab[data-nav-filter="${i.navFilter}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${i.navFilter}"]`);n&&setFilter(i.tab,n);const a=document.createElement("div");a.id="postActivationTourOverlay",a.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.55);",document.body.appendChild(a),document.body.style.overflow="hidden",setTimeout(()=>{const r=document.querySelector(`.tab[data-nav-filter="${i.navFilter}"]`)||document.querySelector(`.mobile-nav-item[data-nav-filter="${i.navFilter}"]`);r&&(r.setAttribute("data-post-activation-highlight","1"),r.style.position="relative",r.style.zIndex="100003",r.style.boxShadow="0 0 0 3px #fff, 0 0 0 6px #2E7D5B",r.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"}));const s=r?r.getBoundingClientRect():{left:24,bottom:80,width:200},p=document.createElement("div");p.id="postActivationTourTooltip";const g=Math.min(300,window.innerWidth-32),h=Math.max(16,Math.min(s.left+s.width/2-g/2,window.innerWidth-g-16)),v=Math.min(s.bottom+14,window.innerHeight-180);p.style.cssText=`position:fixed;z-index:100004;left:${h}px;top:${v}px;max-width:${g}px;width:${g}px;`;const S=e>=t.length-1;p.innerHTML=`
        ...
        </div>`,document.body.appendChild(p),document.getElementById("postActivationTourNext").onclick=()=>{e+=1,o()},document.getElementById("postActivationTourSkip").onclick=()=>{Y()}},i.tab==="apps"?80:0)}o()}window.startPostActivationTabTour=dt;function Gt(){if(document.getElementById("activatedModalOverlay"))return;const t=l.activeHotelDomain||(l.activeHotelId?l.activeHotelId+".mktel.co":""),e="Bookings and Phones",o=document.createElement("div");o.id="activatedModalOverlay",o.style.cssText="position:fixed;inset:0;z-index:100002;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px;",o.innerHTML=`
  ...
  `,document.body.appendChild(o),document.getElementById("activatedModalTour").onclick=()=>{o.remove(),dt()},document.getElementById("activatedModalSkip").onclick=()=>{o.remove(),localStorage.setItem("postActivationTourDone","1");try{setFilter("bookings")}catch{}}}function H(){let t=document.getElementById("tourBlurOverlay");return t||(t=document.createElement("div"),t.id="tourBlurOverlay",t.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.6);",document.body.appendChild(t),document.body.style.overflow="hidden",t)}function X(){const t=document.getElementById("tourTooltip");t&&t.remove();const e=document.getElementById("tourBlurOverlay");e&&e.remove(),document.querySelectorAll("[data-tour-highlighted]").forEach(i=>{i.style.position=i.dataset.tourOrigPosition||"",i.style.zIndex="",i.style.isolation="",i.style.boxShadow="",i.removeAttribute("data-tour-highlighted"),delete i.dataset.tourOrigPosition});const o=document.getElementById("goLiveBanner");o&&o.dataset.tourHidden&&(delete o.dataset.tourHidden,typeof updateGoLiveBanner=="function"&&updateGoLiveBanner()),document.body.style.overflow=""}function L(t,e){if(!e.openAccordion)return;const o=e.accordionCard?document.querySelector(e.accordionCard):t&&t.closest?t.closest(".booking-card"):null;if(!o)return;const i=o.querySelector(".accordion-body");if(!i)return;if(i.style.display==="none"||getComputedStyle(i).display==="none"){i.style.display="block";const a=o.querySelector(".accordion-arrow");a&&(a.style.transform="rotate(90deg)")}}function C(t){if(!t)return null;for(const e of String(t).split(",").map(o=>o.trim()).filter(Boolean)){const o=document.querySelector(e);if(o&&o.isConnected)return o}return null}function F(t,e){if(e.highlightSelector){const o=C(e.highlightSelector);if(o)return o}if(e.highlightCard){const o=e.accordionCard?document.querySelector(e.accordionCard):t&&t.closest?t.closest(".booking-card"):null;if(o)return o}return e.targetParent&&(t.closest(".booking-card")||t.closest(".accordion-body"))||t}function _(t,e){if(!e)return t;const o=String(e.target||"").split(",").map(i=>i.trim()).filter(Boolean);for(const i of o){const n=document.querySelector(i);if(n&&n.isConnected)return n}if(e.accordionCard){const i=document.querySelector(e.accordionCard);if(i&&i.isConnected)return i}return t&&t.isConnected?t:null}function ot(t,e){if(!t||!t.isConnected)return null;const o=t.getBoundingClientRect();return o.width<2||o.height<2||!e&&(o.bottom<8||o.top>window.innerHeight-8)?null:o}function Q(t,e){const o=C(t.anchorSelector);if(o){const i=ot(o,!0);if(i)return i}return ot(e,!0)}function bt(t,e){const o=e.scrollTarget||e.accordionCard,i=(o?C(o):null)||t;if(!i&&!e.scrollToTop)return Promise.resolve();const n=e.scrollBlock||"center",a=window.matchMedia("(prefers-reduced-motion: reduce)").matches,r=l.settingsTourActive||a?"auto":"smooth";return new Promise(s=>{const p=()=>{const $=e.scrollPadTop??80,d=e.scrollPadBottom??130,c=C(e.anchorSelector)||(i&&i.isConnected?i:null)||(t&&t.isConnected?t:null);if(!c){s();return}const y=c.getBoundingClientRect();y.top<$&&window.scrollBy({top:y.top-$,left:0,behavior:"auto"}),y.bottom>window.innerHeight-d&&window.scrollBy({top:y.bottom-window.innerHeight+d,left:0,behavior:"auto"}),requestAnimationFrame(()=>requestAnimationFrame(s))},g=()=>{i&&i.scrollIntoView({behavior:e.scrollToTop?"auto":r,block:n,inline:"nearest"}),p()};if(e.scrollToTop){if(window.scrollTo({top:0,left:0,behavior:r}),document.documentElement.scrollTop=0,document.body.scrollTop=0,e.scrollToTopOnly){requestAnimationFrame(()=>requestAnimationFrame(s));return}if(r==="auto"){g();return}let $=!1;const d=()=>{$||($=!0,window.removeEventListener("scrollend",c),clearTimeout(y),g())},c=()=>d();"onscrollend"in window&&window.addEventListener("scrollend",c,{once:!0});const y=setTimeout(d,520);return}if(!i){s();return}if(i.scrollIntoView({behavior:r,block:n,inline:"nearest"}),r==="auto"){p();return}let h=!1;const v=()=>{h||(h=!0,window.removeEventListener("scrollend",S),clearTimeout(Z),p())},S=()=>v();"onscrollend"in window&&window.addEventListener("scrollend",S,{once:!0});const Z=setTimeout(v,620)})}function Vt(){X(),localStorage.setItem("settingsTourStep","handoff");const t=()=>{const o=document.querySelector('.tab[data-nav-filter="apps"]')||document.querySelector('.mobile-nav-item[data-nav-filter="apps"]');o&&setFilter("apps",o);const i=typeof ensureAppsViewRendered=="function"?ensureAppsViewRendered:window.ensureAppsViewRendered;typeof i=="function"&&i(!0);const n=typeof startAppsTour=="function"?startAppsTour:window.startAppsTour;typeof n=="function"&&n({chainFromSettingsTour:!0})},e=typeof loadAppsModule=="function"?loadAppsModule:window.loadAppsModule;typeof e=="function"?e().then(t).catch(t):t()}function D(){l.settingsTourActive=!1,updateGoLiveBanner();const t=document.createElement("div");t.id="tourBlurOverlay",t.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(t),document.body.style.overflow="hidden";const e=document.createElement("div");if(e.id="tourTooltip",e.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;",e.innerHTML=`
    ...
    </div>`,document.body.appendChild(e),!document.getElementById("tourModalAnimStyle")){const o=document.createElement("style");o.id="tourModalAnimStyle",o.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(o)}typeof lucide<"u"&&setTimeout(()=>lucide.createIcons(),0),document.getElementById("tourNextBtn").onclick=()=>{const i="https://"+(l.activeHotelDomain||l.activeHotelId+".mktel.co");navigator.clipboard.writeText(i).catch(()=>{}),X(),l.settingsTourActive=!1,localStorage.setItem("settingsTourDone","1"),localStorage.setItem("linkCopied","1"),localStorage.removeItem("settingsTourStep"),toast("Booking link copied!","success"),finishTourHydration(),vt()}}function vt(t){const e=document.createElement("div");e.id="testDriveOverlay",e.style.cssText="position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px 16px;",e.innerHTML=`
    <div style="background:white;border-radius:20px;max-width:340px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
      ...
      <div style="background:#1a1a2e;border-radius:10px;padding:14px 18px;color:white;font-size:13px;line-height:1.5;font-weight:500;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.25);">
        ...
        ${b}
      </div>`;if(c.tooltipAnchor==="card-bottom"||c.tooltipAnchor==="card-top"){const q=C(c.highlightSelector)||d,gt=C(c.anchorSelector);if(q){const P=q.getBoundingClientRect(),j=gt?gt.getBoundingClientRect():null,z=Math.min(280,window.innerWidth-32),mt=P.left+P.width/2,R=Math.max(16,Math.min(mt-z/2,window.innerWidth-z-16)),U=Math.max(20,Math.min(mt-R,z-20)),W=132;if(c.tooltipAnchor==="card-bottom"){const tt=window.innerHeight-P.bottom,xt=Math.min(P.bottom+12,window.innerHeight-W-16);if(tt>=W+20)m.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${xt}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ...
              ${x}`;else{const et=j?j.top:P.top;m.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-et+10}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`}}else{const tt=P.top;if(P.top-16>=W+24)m.style.cssText=`position:fixed;z-index:100000;left:${R}px;bottom:${window.innerHeight-tt+10}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              ${x}
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${U-8}px;"></div>`;else{const et=Math.min(P.top+(j?j.height:48)+12,window.innerHeight-W-16);m.style.cssText=`position:fixed;z-index:100000;left:${R}px;top:${et}px;max-width:${z}px;width:${z}px;`,m.innerHTML=`
              <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:8px solid #1a1a2e;margin-left:${U-8}px;"></div>
              ${x}`}}document.body.appendChild(m),v();return}}const f=y||Q(c,d);if(!f){m.style.cssText="position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:280px;width:calc(100% - 32px);",m.innerHTML=x,document.body.appendChild(m),v();return}const k=window.innerHeight-f.bottom,T=f.top,E=Math.min(280,window.innerWidth-32),B=f.left+f.width/2,I=Math.max(16,Math.min(B-E/2,window.innerWidth-E-16)),M=Math.max(20,Math.min(B-I,E-20));let A=c.tooltipPosition||(k>150?"below":T>150?"above":"center");if(A==="below"&&k<130&&T>k&&(A="above"),A==="above"&&T<130&&k>T&&(A="below"),A==="below"&&f.bottom+150>window.innerHeight&&(A="above"),A==="below"){const q=Math.min(f.bottom+8,window.innerHeight-24);m.style.cssText=`position:fixed;z-index:100000;left:${I}px;top:${q}px;max-width:${E}px;width:${E}px;`,m.innerHTML=`
        ...
        ${x}
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #1a1a2e;margin-left:${M-8}px;"></div>`):(m.style.cssText="position:fixed;z-index:100000;left:50%;top:50%;transform:translate(-50%,-50%);max-width:280px;width:calc(100% - 32px);",m.innerHTML=x);document.body.appendChild(m),v()}function v(){const d=document.getElementById("tourNextBtn"),c=document.getElementById("tourSkipBtn");d&&(d.onclick=()=>{n(),i++,localStorage.setItem("settingsTourStep",String(i)),s()}),c&&(c.onclick=()=>{a()})}function S(){typeof invokeLoadEditRooms=="function"&&invokeLoadEditRooms();const d=document.createElement("div");d.id="tourBlurOverlay",d.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(d),document.body.style.overflow="hidden";const c=l.activeHotelName||"Your Hotel",y=c.trim().charAt(0).toUpperCase(),u=c.length>10?c.slice(0,10):c,m="width:32px;display:flex;flex-direction:column;align-items:center;gap:5px;",b="width:32px;height:32px;border-radius:9px;box-sizing:border-box;",w="height:8px;max-width:46px;line-height:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;",x=`<div style="${m}"><div style="${b}background:rgba(255,255,255,0.22);"></div><div style="${w}"></div></div>`,f=l.activeHotelAppIcon||"",k=f?`<img src="${f}" alt="" style="width:100%;height:100%;object-fit:contain;">`:y,T=f?`${b}background:#fff;padding:5px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);`:`${b}background:#fff;color:#2E7D5B;font-weight:800;font-size:15px;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.95);box-shadow:0 6px 14px rgba(0,0,0,0.28);`,E=`<div style="${m}"><div style="${T}">${k}</div><div style="${w}font-size:7.5px;color:#fff;font-weight:700;">${u}</div></div>`,B=[x,x,x,x,E,x,x,x].join(""),I=document.createElement("div");if(I.id="tourTooltip",I.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:20px 16px;",I.innerHTML=`
      ...
      </div>`,document.body.appendChild(I),!document.getElementById("tourModalAnimStyle")){const M=document.createElement("style");M.id="tourModalAnimStyle",M.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(M)}document.getElementById("tourNextBtn").onclick=()=>{n(),i++,localStorage.setItem("settingsTourStep",String(i)),s()},document.getElementById("tourSkipBtn").onclick=()=>{a()}}function Z(){const d=document.createElement("div");d.id="tourBlurOverlay",d.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(d),document.body.style.overflow="hidden";let c=0;const y=[`<div style="padding:20px 18px 0;">
      ...
      <div style="padding:0 14px 14px;">
        ...
        </div>`,document.getElementById("tourNextBtn").onclick=()=>{c<y.length-1?(c++,m()):(n(),i++,localStorage.setItem("settingsTourStep",String(i)),s())},document.getElementById("tourSkipBtn").onclick=()=>{a()}}if(document.body.appendChild(u),m(),!document.getElementById("tourModalAnimStyle")){const b=document.createElement("style");b.id="tourModalAnimStyle",b.textContent="@keyframes tourModalSlideUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(b)}}function $(){const d=document.createElement("div");d.id="tourBlurOverlay",d.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.7);",document.body.appendChild(d),document.body.style.overflow="hidden";const c=document.createElement("div");if(c.id="tourTooltip",c.style.cssText="position:fixed;z-index:100000;inset:0;display:flex;align-items:center;justify-content:center;padding:24px 16px;",c.innerHTML=`
      <div style="background:white;border-radius:20px;max-width:340px;width:100%;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:tourModalSlideUp 0.3s ease;">
    ...
    `;t.innerHTML=S,O(),typeof lucide<"u"&&lucide.createIcons()}catch{t.innerHTML='<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-text">Failed to load your page</div><div class="empty-sub">Check your connection and refresh.</div></div>'}})();try{await l.editRoomsLoadPromise}finally{l.editRoomsLoadPromise=null}}}function ct(){O()}function O(){const t=document.getElementById("editRoomsCards");if(t){if(!l.editRooms.length){t.innerHTML='<div class="empty-state"><div class="empty-icon">🛏️</div><div class="empty-text">No rooms yet</div><div class="empty-sub">Add your first room type below.</div></div>';return}t.innerHTML=l.editRooms.map((e,o)=>{const i=(e.amenities||"").split("•").map(r=>r.trim()).filter(Boolean),n=(e.images||[]).filter(r=>r&&r.url),a=jsStr(e.id);return`
      ...
      <div class="room-edit-fields" style="padding:18px;">
        ...
        <div style="margin-bottom:12px;">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;" id="edit-amenity-pills-${e.id}">
            ${i.map(r=>`<span style="display:inline-flex;align-items:center;gap:4px;background:var(--green-pale);color:var(--green);padding:5px 10px;border-radius:8px;font-size:12px;font-weight:600;">${Et(r)} ${r} <button onclick="removeAmenity('${e.id}','${r.replace(/'/g,"\\'")}')" style="background:none;border:none;color:var(--green);cursor:pointer;font-size:14px;margin-left:2px;">×</button></span>`).join("")}
          </div>
    ...
    </div>`}).join(""),typeof lucide<"u"&&lucide.createIcons()}}function wt(t){const e=l.editRooms.find(o=>String(o.id)===String(t));return(e&&e.images||[]).filter(o=>o&&o.url)}function kt(t,e){const o=wt(t);if(!o.length)return;const i=document.getElementById("edit-card-"+t);if(!i)return;const n=o.length,a=((Number(e)||0)%n+n)%n,r=i.querySelector(".room-edit-main-img");r&&(r.src=o[a].url),i.querySelector(".room-edit-photo")?.setAttribute("data-photo-index",String(a));const s=i.querySelector(".room-edit-photo-count");s&&(s.textContent=a+1+" / "+n),i.querySelectorAll(".room-edit-image-dot").forEach((p,g)=>{p.classList.toggle("active",g===a),g===a?p.setAttribute("aria-current","true"):p.removeAttribute("aria-current")}),i.querySelectorAll(".room-edit-thumb").forEach((p,g)=>{p.classList.toggle("active",g===a),g===a?p.setAttribute("aria-current","true"):p.removeAttribute("aria-current")})}function Yt(t,e){const i=document.getElementById("edit-card-"+t)?.querySelector(".room-edit-photo"),n=parseInt(i?.getAttribute("data-photo-index")||"0",10)||0;kt(t,n+e)}function Et(t){const e=t.toLowerCase();return e.includes("wifi")?'<i data-lucide="wifi" style="width:14px;height:14px;"></i>':e.includes("tv")||e.includes("television")?'<i data-lucide="tv" style="width:14px;height:14px;"></i>':e.includes("fridge")||e.includes("refrigerator")?'<i data-lucide="thermometer-snowflake" style="width:14px;height:14px;"></i>':e.includes("parking")?'<i data-lucide="car" style="width:14px;height:14px;"></i>':e.includes("housekeeping")||e.includes("cleaning")?'<i data-lucide="sparkles" style="width:14px;height:14px;"></i>':e.includes("bath")||e.includes("shower")?'<i data-lucide="bath" style="width:14px;height:14px;"></i>':e.includes("work")||e.includes("desk")?'<i data-lucide="laptop" style="width:14px;height:14px;"></i>':e.includes("pet")||e.includes("dog")?'<i data-lucide="paw-print" style="width:14px;height:14px;"></i>':e.includes("pool")?'<i data-lucide="waves" style="width:14px;height:14px;"></i>':e.includes("kitchen")||e.includes("microwave")?'<i data-lucide="cooking-pot" style="width:14px;height:14px;"></i>':'<i data-lucide="check" style="width:14px;height:14px;"></i>'}const St=[{key:"wifi",label:"Free WiFi",icon:"wifi"},{key:"tv",label:"Smart TV",icon:"tv"},{key:"fridge",label:"Fridge",icon:"thermometer-snowflake"},{key:"parking",label:"Free Parking",icon:"car"},{key:"housekeeping",label:"Weekly Housekeeping",icon:"sparkles"},{key:"bath",label:"Bath",icon:"bath"},{key:"workstation",label:"Workstation",icon:"laptop"},{key:"pet",label:"Pet Friendly",icon:"paw-print"},{key:"pool",label:"Pool",icon:"waves"},{key:"kitchen",label:"Kitchenette",icon:"cooking-pot"},{key:"ac",label:"Air Conditioning",icon:"wind"},{key:"laundry",label:"Laundry",icon:"shirt"}];let pt=null;function Tt(t){pt=t;const o=(l.editRooms.find(a=>a.id===t)?.amenities||"").split("•").map(a=>a.trim().toLowerCase()).filter(Boolean);let i=document.getElementById("amenityPickerModal");i||(document.body.insertAdjacentHTML("beforeend",`
    ...
    `),document.getElementById("amenityPickerModal").addEventListener("click",K),i=document.getElementById("amenityPickerModal"));const n=document.getElementById("amenityPickerGrid");n.innerHTML=St.map(a=>{const r=o.some(s=>s.includes(a.key));return`<button onclick="toggleAmenityPreset(this,'${a.key}')" data-key="${a.key}" style="display:inline-flex;align-items:center;gap:5px;padding:7px 12px;border-radius:8px;border:1.5px solid ${r?"#2E7D5B":"#e5e7eb"};background:${r?"#E8F5EE":"white"};color:${r?"#2E7D5B":"#1a1a2e"};font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;"><i data-lucide="${a.icon}" style="width:14px;height:14px;"></i> ${a.label}</button>`}).join(""),document.getElementById("amenityCustomInput").value="",i.style.display="flex",typeof lucide<"u"&&lucide.createIcons()}function _t(t,e){const o=t.style.borderColor==="rgb(46, 125, 91)";t.style.borderColor=o?"#e5e7eb":"#2E7D5B",t.style.background=o?"white":"#E8F5EE",t.style.color=o?"#1a1a2e":"#2E7D5B"}function K(){document.getElementById("amenityPickerModal").style.display="none",pt=null}function Qt(){const t=l.editRooms.find(n=>n.id===pt);if(!t){K();return}const e=document.getElementById("amenityPickerGrid"),o=[];e.querySelectorAll("button").forEach(n=>{if(n.style.background==="rgb(232, 245, 238)"){const a=St.find(r=>r.key===n.dataset.key);a&&o.push(a.label)}});const i=document.getElementById("amenityCustomInput").value.trim();i&&o.push(i),t.amenities=o.join(" • "),K(),ct(),typeof lucide<"u"&&lucide.createIcons()}function Jt(t){Tt(t)}function Kt(t,e){const o=l.editRooms.find(n=>n.id===t);if(!o)return;const i=(o.amenities||"").split("•").map(n=>n.trim()).filter(Boolean);o.amenities=i.filter(n=>n!==e).join(" • "),ct(),typeof lucide<"u"&&lucide.createIcons()}async function Xt(){const t=document.getElementById("edit-hotel-name")?.value.trim(),e=document.getElementById("edit-hotel-subtitle")?.value.trim(),o=document.getElementById("edit-hotel-address")?.value.trim(),i=document.getElementById("edit-hotel-phone")?.value.trim(),n=document.getElementById("edit-hotel-policy")?.value.trim();try{await api("POST","/api/crm/hotel-info",{name:t,subtitle:e,address:o,phone:i,cancellationPolicy:n}),toast("Hotel info saved!","success")}catch{toast("Failed to save","error")}}async function Zt(){const t=parseFloat(document.getElementById("edit-rate-nightly")?.value)||69,e=parseFloat(document.getElementById("edit-rate-weekly")?.value)||299,o=parseFloat(document.getElementById("edit-rate-monthly")?.value)||999;try{await api("POST","/api/crm/rates",{nightly:t,weekly:e,monthly:o}),localStorage.setItem("ratesChanged","1"),l.launchStatus=null,N(),toast("Rates saved!","success")}catch{toast("Failed to save rates","error")}}async function te(){const t=document.getElementById("edit-new-pin")?.value.trim();if(!t||t.length<4){toast("PIN must be at least 4 characters","error");return}try{await api("POST","/api/crm/change-pin",{newPin:t}),l.token=t;try{localStorage.setItem("crmToken",l.token)}catch{}toast("PIN updated!","success")}catch{toast("Failed to change PIN","error")}}function ee(t){navigator.clipboard.writeText(t).then(()=>{toast("Booking link copied!","success")}).catch(()=>{toast("Failed to copy","error")})}function oe(t){const e=t.nextElementSibling,o=t.querySelector(".accordion-arrow");e.style.display==="none"?(e.style.display="block",o&&(o.style.transform="rotate(90deg)")):(e.style.display="none",o&&(o.style.transform="rotate(0deg)"))}let V=!1;function zt(){if(document.getElementById("goLiveOverlay"))return;const t=document.createElement("div");t.id="goLiveOverlay",t.style.cssText="position:fixed;inset:0;z-index:99999;background:rgba(255,255,255,0.94);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;",t.innerHTML='<div class="logo-sprite-bounce"></div><div style="font-size:14px;font-weight:700;color:#1a5c3f;">Opening secure checkout…</div><div style="font-size:12px;color:#6b7280;">Taking you to Stripe — one moment</div>',document.body.appendChild(t)}function nt(){const t=document.getElementById("goLiveOverlay");t&&t.remove()}async function ie(){if(!V){V=!0,zt();try{const t=await api("POST","/api/crm/go-live");if(t.success&&t.url){window.location.href=t.url;return}nt(),V=!1,toast(t.message||"Failed to start checkout","error")}catch{nt(),V=!1,toast("Failed to start checkout. Try again.","error")}}}async function ne(){try{const t=await api("GET","/api/crm/billing-portal");t.success&&t.url?window.location.href=t.url:toast(t.message||"Contact support@bookmarketel.com to manage your subscription.","error")}catch{toast("Contact support@bookmarketel.com to manage your subscription.","error")}}async function re(){const t=document.getElementById("supportMessage")?.value.trim();if(!t){toast("Please enter a message","error");return}try{await api("POST","/api/crm/support",{message:t}),document.getElementById("supportMessage").value="",toast("Message sent! We'll reply to your email.","success")}catch{toast("Failed to send. Email support@bookmarketel.com directly.","error")}}async function ae(t){const e=l.editRooms.find(s=>s.id===t);if(!e){toast("Room not found — try refreshing","error");return}const o=document.getElementById("edit-name-"+t)?.value.trim(),i=document.getElementById("edit-desc-"+t)?.value.trim(),n=parseInt(document.getElementById("edit-occ-"+t)?.value)||4,a=parseInt(document.getElementById("edit-units-"+t)?.value)||1,r={id:t,name:o||e.name,description:i||"",amenities:e.amenities||"",maxOccupancy:n,totalUnits:a};try{const s=await api("POST","/api/crm/rooms",r);if(s&&s.success===!1){toast(s.message||"Failed to save","error");return}e.name=r.name,e.description=r.description,e.maxOccupancy=n,e.totalUnits=a,toast("Room saved!","success")}catch(s){toast("Failed to save: "+(s.message||""),"error")}}async function se(t,e){const o=Array.from(t.target.files);if(!o.length)return;const n=document.getElementById("edit-card-"+e)?.querySelector("div:first-child");n&&(n.style.position="relative",n.insertAdjacentHTML("beforeend",'<div id="upload-spinner-'+e+'" style="position:absolute;inset:0;background:rgba(255,255,255,0.8);display:flex;align-items:center;justify-content:center;z-index:5;flex-direction:column;gap:6px;"><div style="width:24px;height:24px;border:3px solid var(--border);border-top-color:var(--green);border-radius:50%;animation:spin 0.7s linear infinite;"></div><div id="upload-progress-'+e+'" style="font-size:12px;color:var(--text-muted);font-weight:600;">0 / '+o.length+"</div></div>"));let a=0,r="";for(const p of o){try{const h=await st(e,p);if(h.image){const v=l.editRooms.find(S=>S.id===e);v&&(v.images||(v.images=[]),v.images.push(h.image),v.imageUrl||(v.imageUrl=h.image.url)),a++}}catch(h){r=h.message||"Upload failed"}const g=document.getElementById("upload-progress-"+e);g&&(g.textContent=a+" / "+o.length)}const s=document.getElementById("upload-spinner-"+e);s&&s.remove(),O(),a>0&&(l.launchStatus=null),N(),a>0?toast(a+" photo"+(a!==1?"s":"")+" added. Check the Bookings tab to continue your launch checklist!","success"):toast(r||"Upload failed","error")}function Bt(t,e=512){return new Promise((o,i)=>{const n=new Image,a=URL.createObjectURL(t);n.onload=()=>{try{const r=Math.min(n.naturalWidth,n.naturalHeight),s=(n.naturalWidth-r)/2,p=(n.naturalHeight-r)/2,g=document.createElement("canvas");g.width=e,g.height=e;const h=g.getContext("2d");h.imageSmoothingQuality="high",h.drawImage(n,s,p,r,r,0,0,e,e),URL.revokeObjectURL(a),g.toBlob(v=>v?o(v):i(new Error("crop failed")),"image/png",.92)}catch(r){URL.revokeObjectURL(a),i(r)}},n.onerror=()=>{URL.revokeObjectURL(a),i(new Error("load failed"))},n.src=a})}function It(){const t=document.getElementById("appsAppIconPreview");t&&(t.innerHTML='<div style="width:24px;height:24px;border:3px solid rgba(255,255,255,0.5);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;"></div>')}function ut(t){const e=document.getElementById("appsAppIconPreview");e&&(e.style.background="#fff",e.style.border="1px solid var(--border)",e.style.padding="0",e.innerHTML='<img src="'+t+'" alt="App icon" style="width:100%;height:100%;object-fit:contain;">')}function rt(){const t=document.getElementById("appsAppIconPreview");if(!t)return;if(l.activeHotelAppIcon){ut(l.activeHotelAppIcon);return}const e=(l.activeHotelName||"H").trim().charAt(0).toUpperCase()||"🏨";t.style.background="transparent",t.style.border="none",t.style.padding="0",t.innerHTML='<span style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:var(--green);color:#fff;border-radius:14px;font-size:24px;font-weight:800;">'+e+"</span>"}async function de(t){const e=t.files&&t.files[0];if(!e)return;It();const o=new FormData;try{const i=await Bt(e,512);o.append("icon",i,"app-icon.png")}catch{o.append("icon",e)}try{const i=at(),n=new URLSearchParams;l.activeHotelId&&n.set("hotelId",l.activeHotelId),i&&n.set("token",i);const r=await(await fetch(`/api/crm/hotel-app-icon?${n}`,{method:"POST",headers:{"x-crm-token":i},body:o})).json();if(r.success&&r.appIconUrl){l.activeHotelAppIcon=r.appIconUrl,ut(r.appIconUrl);const s=document.getElementById("appsView");s&&(s.dataset.appsKey=(l.activeHotelId||"")+"|"+r.appIconUrl+"|"+(l.activeHotelDomain||"")),typeof updateFrontdeskManifestLink=="function"&&updateFrontdeskManifestLink(),toast("Logo updated! Guests will see it on their phone.","success")}else toast(r.message||"Failed to upload icon","error"),rt()}catch{toast("Failed to upload icon","error"),rt()}t.value=""}async function le(t,e){if(confirm("Delete this photo?"))try{await api("DELETE",`/api/crm/rooms/${t}/images/${e}`);const o=l.editRooms.find(i=>i.id===t);o&&o.images&&(o.images=o.images.filter(i=>i.id!==e),o.imageUrl=o.images[0]?.url||null),O(),toast("Photo deleted","success")}catch{toast("Failed to delete","error")}}async function ce(t){if(confirm("Delete this room type?"))try{await api("DELETE",`/api/crm/rooms/${t}`),toast("Room deleted","success"),lt()}catch{toast("Failed to delete","error")}}function pe(){const t=document.getElementById("editRoomsList");document.getElementById("editAddForm")||(t.insertAdjacentHTML("beforeend",`
  ...
  `),document.getElementById("editNewRoomName").focus())}function ue(){const t=document.getElementById("editNewRoomName").value.trim();t&&api("POST","/api/crm/rooms",{name:t,maxOccupancy:4,totalUnits:5}).then(()=>{toast("Room added","success"),lt()}).catch(()=>toast("Failed to add","error"))}const At={addAmenityPrompt:Jt,advanceTourIfNeeded:N,changePin:te,checklistGoTo:Rt,checklistGoToRates:Mt,cleanupPostActivationTourUi:J,cleanupSettingsTourUi:X,closeAmenityPicker:K,confirmAmenityPicker:Qt,confirmEditAddRoom:ue,copyBookingLink:ee,copyBookingLinkFromChecklist:Lt,deleteEditImage:le,deleteEditRoom:ce,ensureTourBlurOverlay:H,finishPostActivationTour:Y,getAmenityIcon:Et,getCrmAuthToken:at,getEditRoomImages:wt,goLive:ie,guestBookingEngineUrl:yt,handoffToGuestAppsTour:Vt,hideGoLiveOverlay:nt,loadEditRooms:lt,loadSettings:ft,openAmenityPicker:Tt,openBillingPortal:ne,openEditAddRoom:pe,openGuestBookingEngine:qt,openPreviewSite:Ot,openTourAccordion:L,postRoomImageUpload:st,queryTourSelector:C,removeAmenity:Kt,renderEditRooms:ct,renderEditRoomsCards:O,replayWalkthrough:Wt,resolveLiveTourElement:_,resolveTourHighlightEl:F,restoreAppIconPreview:rt,saveEditRoom:ae,saveHotelInfo:Xt,saveRates:Zt,scrollTourTargetIntoView:bt,sendSupportMessage:re,setAppIconPreviewImage:ut,setAppIconPreviewLoading:It,settingsChangePin:Dt,settingsCopyLink:$t,settingsSaveRates:Ft,settingsSendSupport:Nt,settingsUploadPhoto:Ht,showActivatedModal:Gt,showEditRoomPhoto:kt,showFinaleMockModal:D,showGoLiveOverlay:zt,showOnboardingQuestions:Ut,showTestDriveModal:vt,showWelcomeModal:ht,squareCropImage:Bt,startPostActivationTabTour:dt,startSettingsTour:it,stepEditRoomPhoto:Yt,toggleAmenityPreset:_t,toggleSection:oe,tourAnchorRect:Q,tourElementRect:ot,updatePreviewSiteBar:jt,uploadAppIcon:de,uploadEditImages:se};function ge(){Ct(At)}const fe=Object.freeze(Object.defineProperty({__proto__:null,default:At,install:ge},Symbol.toStringTag,{value:"Module"}));export{me as a,fe as b,l as c,Ct as e,xe as s};
