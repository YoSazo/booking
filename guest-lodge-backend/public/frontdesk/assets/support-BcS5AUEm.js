import{e as f,c as o}from"./settings-BB-2eeJ9.js";let n=null,p=null,s=!1;function i(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function h(e){const t=new Date(e);if(!Number.isFinite(t.getTime()))return"";const r=t.toDateString()===new Date().toDateString();return t.toLocaleString([],r?{hour:"numeric",minute:"2-digit"}:{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function d(){if(document.getElementById("marketelSupportStyles"))return;const e=document.createElement("style");e.id="marketelSupportStyles",e.textContent=`
  ...
  `,document.head.appendChild(e)}function u(){const e=Math.max(0,Number(o.supportUnreadCount||0));document.querySelectorAll(".marketel-support-unread").forEach(t=>{t.textContent=e>99?"99+":String(e),t.classList.toggle("visible",e>0)})}function k(){const e=o.supportThread?.messages||[];return e.length?e.map(t=>{const r=t.sender==="owner";return`<div class="marketel-support-row ${r?"owner":"support"}">
      <div class="marketel-support-label">${r?"You":"Marketel"} · ${i(h(t.createdAt))}</div>
      <div class="marketel-support-bubble">${i(t.body)}</div>
    ...
    </div>`}function m(){const e=document.getElementById("marketelSupportMessages");if(!e)return;e.innerHTML=k();const t=document.getElementById("marketelSupportStatus");t&&(t.textContent=o.supportThread?.status==="resolved"?"Resolved":"Replies here"),requestAnimationFrame(()=>{e.scrollTop=e.scrollHeight}),u()}async function l({markRead:e=!1,silent:t=!1}={}){if(p)return p;p=(async()=>{try{const r=await window.api("GET","/api/crm/support");if(!r?.success)throw new Error(r?.message||"Could not load support.");return o.supportThread=r.thread||null,o.supportUnreadCount=Number(r.thread?.unread||0),e&&o.supportUnreadCount>0&&(await window.api("POST","/api/crm/support/read",{}).catch(()=>null),o.supportUnreadCount=0),document.getElementById("marketelSupportOverlay")?m():u(),o.supportThread}catch(r){if(!t){const a=document.getElementById("marketelSupportMessages");a&&(a.innerHTML=`<div class="marketel-support-empty"><strong>Could not load this conversation.</strong><p>${i(r.message||"Check your connection and try again.")}</p></div>`)}throw r}})();try{return await p}finally{p=null}}function b(){c(),n=window.setInterval(()=>{document.hidden||!document.getElementById("marketelSupportOverlay")||l({markRead:!0,silent:!0}).catch(()=>{})},15e3)}function c(){n&&window.clearInterval(n),n=null}function y(){d();let e=document.getElementById("marketelSupportOverlay");if(e)return e;e=document.createElement("div"),e.id="marketelSupportOverlay",e.className="marketel-support-overlay",e.setAttribute("role","presentation"),e.onclick=r=>{r.target===e&&g()},e.innerHTML=`<section class="marketel-support-dialog" role="dialog" aria-modal="true" aria-labelledby="marketelSupportTitle">
  ...
  </section>`,document.body.appendChild(e);const t=document.getElementById("marketelSupportInput");return t&&(t.addEventListener("input",()=>{t.style.height="auto",t.style.height=`${Math.min(130,t.scrollHeight)}px`}),t.addEventListener("keydown",r=>{r.key==="Enter"&&(r.metaKey||r.ctrlKey)&&(r.preventDefault(),x())})),e}async function w(){y(),document.body.classList.add("marketel-support-open"),window.setNativeModalOpen?.("marketel-support",!0),b(),await l({markRead:!0}).catch(()=>null),document.getElementById("marketelSupportInput")?.focus({preventScroll:!0})}function g(){c(),document.getElementById("marketelSupportOverlay")?.remove(),document.body.classList.remove("marketel-support-open"),window.setNativeModalOpen?.("marketel-support",!1)}async function x(){if(s)return;const e=document.getElementById("marketelSupportInput"),t=document.getElementById("marketelSupportSend"),r=String(e?.value||"").trim();if(!r){window.toast?.("Write a message first.","error"),e?.focus();return}s=!0,t&&(t.disabled=!0);try{const a=await window.api("POST","/api/crm/support",{message:r,surface:`frontdesk-${o.currentFilter||"unknown"}`,pagePath:`${window.location.pathname}${window.location.search}`});if(!a?.success)throw new Error(a?.message||"Could not send that message.");o.supportThread=a.thread||o.supportThread,o.supportUnreadCount=0,e&&(e.value="",e.style.height="auto"),m()}catch(a){window.toast?.(a.message||"Could not send. Try again.","error")}finally{s=!1,t&&(t.disabled=!1),e?.focus()}}async function S(){return l({markRead:!1,silent:!0})}function E(){d(),f({closeSupportConversation:g,openSupportConversation:w,sendMarketelSupportMessage:x})}export{g as closeSupportConversation,E as install,S as loadSupportSummary,w as openSupportConversation,x as sendMarketelSupportMessage};
