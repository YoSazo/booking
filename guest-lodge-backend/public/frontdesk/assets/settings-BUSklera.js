  ...
  `,document.body.appendChild(o),document.getElementById("activatedModalTour").onclick=()=>{o.remove(),ce()},document.getElementById("activatedModalSkip").onclick=()=>{o.remove(),localStorage.setItem("postActivationTourDone","1");try{setFilter("bookings")}catch{}}}function ke(){return isStandaloneApp()||s.frontdeskInstalled?pushSupported()?typeof Notification<"u"&&Notification.permission==="granted"?"":ne():"":ne()}function ne(){const e=isStandaloneApp()||s.frontdeskInstalled,t=typeof Notification<"u"&&Notification.permission==="granted";let o;return e&&pushSupported()?o=t?`<p style="font-size:12px;color:var(--text-muted);margin:0 0 14px;line-height:1.5;">You're all set — you'll get an alert on this device every time a guest books.</p>
    ...
    <div class="booking-card" style="margin-bottom:14px;" id="edit-card-${t.id}">
      ...
      <div class="room-edit-photo" style="position:relative;">
        ${i.length?`<img src="${i[0].url}" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:contain;display:block;background:var(--bg);" onerror="this.onerror=null;this.src='https://suitestay.clickinns.com/kingbedsuitestay.webp';">`:'<div style="width:100%;height:100%;min-height:320px;background:var(--bg);display:flex;align-items:center;justify-content:center;color:var(--text-muted);font-size:14px;">No photos yet</div>'}
