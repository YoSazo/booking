import{c as n,e as Z}from"./settings-CBZ0R7Zu.js";const R="marketelValueRevealPendingV1",A="marketelValueRevealStepV1",U="marketelBillingIntervalV1";let i=0,c="guest",p=!1,b={rooms:[],rates:null},$=null,o={ready:!1,checking:!0,reason:"",attempts:0,domain:""},y=0,k=0,w=null,l=0,B=0,f=0,g="month",v=null;const ee="https://is1-ssl.mzstatic.com/image/thumb/Purple221/v4/46/2a/e1/462ae1c9-9347-efd0-5e99-41e7f636e3f7/phone-0-0-1x_U007epad-0-1-0-sRGB-85-220.png/512x512bb.jpg",te="https://is1-ssl.mzstatic.com/image/thumb/Purple221/v4/23/4c/cb/234ccbb4-e65a-bb94-f877-3d230743e9e3/safari-0-0-1x_U007epad-0-1-0-sRGB-85-220.png/512x512bb.jpg";function W(){const e=window.location.hostname;return e==="localhost"||e==="127.0.0.1"||e==="0.0.0.0"||e==="::1"||e.endsWith(".local")||/^10\./.test(e)||/^192\.168\./.test(e)||/^172\.(1[6-9]|2\d|3[01])\./.test(e)}function d(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function ae(e){const t=Number(e);return Number.isFinite(t)?new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:Number.isInteger(t)?0:2}).format(t):"$99"}function h(){return n.activeHotelName||"Your Property"}function F(){return b.rooms[0]||n.editRooms[0]||{name:"Your first room",totalUnits:1,images:[]}}function re(){const e=F();return e.images?.[0]?.url||e.imageUrl||""}function ie(){return b.rates?.nightly||99}function P(){if(W()&&n.activeHotelId){const a=new URL(window.location.href);return a.port="5173",a.pathname="/",a.search="",a.hash="",a.searchParams.set("hotelId",n.activeHotelId),a.searchParams.set("preview","1"),a.toString()}const e=o.domain||n.activeHotelDomain||"";if(!e)return"";const t=new URL(`https://${e}/`);return n.activeHotelId&&t.searchParams.set("hotelId",n.activeHotelId),t.searchParams.set("preview","1"),t.toString()}function C(){const e=String(o.domain||n.activeHotelDomain||"").trim().replace(/^https?:\/\//i,"").replace(/\/.*$/,"").toLowerCase();return e?e.endsWith(".bookmarketel.com")?e.replace(/\.bookmarketel\.com$/,".mktel.co"):e:`${h().normalize("NFKD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,50)||"your-property"}.mktel.co`}function oe(){const e=new URL(window.location.href);return e.search="",e.hash="",n.activeHotelId&&e.searchParams.set("hotelId",n.activeHotelId),e.searchParams.set("previewEditor","1"),e.toString()}function L(e=""){const t=n.activeHotelAppIcon||re(),a=h().trim().charAt(0).toUpperCase()||"M";return t?`<img class="${e}" src="${d(t)}" alt="">`:`<span class="${e}">${d(a)}</span>`}function ne(){if(!n.hotelSubscribed)try{localStorage.setItem(R,"1"),localStorage.setItem(A,String(i))}catch{}}function m(e,t=""){typeof window.api=="function"&&window.api("POST","/api/crm/value-reveal-event",{eventName:e,contentName:t,...window.MarketelJourney?.linkage?.()||{}}).catch(()=>{})}function s(e,t={},a={}){return window.MarketelJourney?.track(e,{revealStep:i,stageName:["booking-page","guest-app","front-desk-assistant","activation"][i]||"unknown",...t},a)}function se(){try{const e=new URL(window.location.href);e.searchParams.delete("welcome"),e.searchParams.delete("reveal"),window.history.replaceState({},"",e.pathname+e.search+e.hash)}catch{}}function z(e){typeof window.setNativeShellVisible=="function"&&window.setNativeShellVisible(e)}function T(e){const t=Math.max(0,Math.floor(Number(e||0)/1e3)),a=Math.floor(t/60),r=String(t%60).padStart(2,"0");return`${a}:${r}`}function E(e){e?.layer&&(e.layer.classList.remove("is-visible"),e.layer.setAttribute("aria-hidden","true"),e.layer.innerHTML="")}function D(e="",t=!1){const a=v;if(a){if(a.timerId&&(window.clearInterval(a.timerId),a.timerId=0),t&&a.status==="running"){const r=Date.now()-a.startedAt;m("BookingChallengeAbandoned",e),s("JourneyBookingChallengeAbandoned",{reason:e,elapsedMs:r},{durationMs:r})}a.timer&&(a.timer.hidden=!0),a.status==="running"&&(a.status="abandoned"),E(a)}}function _(e){if(!e||e.status!=="running"||!e.timer)return;const t=Date.now()-e.startedAt,a=e.timer.querySelector("[data-challenge-time]");a&&(a.textContent=`${T(t)} / 1:00`),e.timer.classList.toggle("is-over-minute",t>=6e4)}function le(e){!e||e!==v||e.status!=="prompted"||(e.status="running",e.startedAt=Date.now(),E(e),e.timer.hidden=!1,_(e),e.timerId=window.setInterval(()=>_(e),500),m("BookingChallengeStarted"),s("JourneyBookingChallengeStarted",{targetSeconds:60,bookingDomain:C()}))}function de(e){!e||e!==v||e.hasPrompted||c!=="guest"||(e.hasPrompted=!0,e.status="prompted",e.layer.innerHTML=`<section class="mvr-challenge-card" role="dialog" aria-labelledby="mvrChallengeTitle">
    ...
    <div class="mvr-copy">
      ...
      <p>Guests install <strong>${d(h())}</strong> from your booking page. After that, they can book direct in one tap and receive notifications you send from Front Desk.</p>
              ...
              <div class="mvr-install-demo-stage">
                <div class="mvr-install-entry">
                  ...
                  <div class="mvr-install-card">
                    <div class="mvr-install-property-icon">${L()}</div>
                    <div>
                      <strong>Get the ${d(h())} app</strong>
                  ...
                  <div class="mvr-ios-crop">
                    <div class="mvr-ios-dock">
                      <div class="mvr-dock-icon mvr-dock-property">${L()}</div>
            ...
            <div class="mvr-app-carousel-slide mvr-app-carousel-value" aria-hidden="${l===1?"false":"true"}">
              <div class="mvr-installed-value">
                <div class="mvr-installed-value-head">
                  <div class="mvr-installed-app-icon">${L()}</div>
                  <div>
                    <strong>From their Home Screen</strong>
                    <span>${d(h())} stays one tap away.</span>
                  </div>
                  <b>✓</b>
                </div>
                ...
                <div class="mvr-app-push-preview">
                  <div class="mvr-app-push-meta">
                    <span class="mvr-app-push-icon">${L()}</span>
                    <strong>${d(h())}</strong>
                    <span>now</span>
                  </div>
  ...
  </section>`}function he(){const e=F().name||"King Suite";return`<section class="mvr-stage mvr-stage-assistant">
    <div class="mvr-copy">
  ...
  </section>`}function fe(){const e=n.hotelSubscribed,t=g==="year",a=t?"$1,990":"$199",r=t?"/year":"/month",u=t?"Activate Marketel — $1,990/year":"Activate Marketel — $199/month";return`<section class="mvr-stage mvr-stage-finale">
    <div class="mvr-finale-card">
      <div class="mvr-finale-mark">✓</div>
      <div class="mvr-eyebrow">${e?"Your Marketel system":"Ready to activate"}</div>
      <h1>${e?`${d(h())} is ready.`:`Marketel is ready for ${d(h())}.`}</h1>
  ...
  </section>`}function be(){return i===0?pe():i===1?ge():i===2?he():fe()}function ye(){if(i===0)return"";if(i===3)return`<div class="mvr-footer mvr-footer-final">
    ...
    ${ye()}
  </div>`,Ie())}function we(){const e=P();if(!e||document.getElementById("mvrLivePreview"))return;c="guest";const t=Date.now(),a=document.createElement("div");a.id="mvrLivePreview",a.className="mvr-live-preview",a.innerHTML=`<div class="mvr-live-toolbar">
    <div class="mvr-live-topline">
      ...
      <div class="mvr-live-address" id="mvrLiveLocation" aria-label="Your live booking address">
        ...
        <strong data-live-location-text>${d(C())}</strong>
      </div>
