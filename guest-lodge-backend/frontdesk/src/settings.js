...
function ensureTourBlurOverlay(options) {
  ...
  overlay.style.cssText = `position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.48);pointer-events:${opts.blockPointer ? 'auto' : 'none'};`;
  document.body.appendChild(overlay);
...
function startSettingsTour() {
  ...
  const steps = [
    ...
    },
    {
      ...
      text: 'Tap + Add Photos to upload a real room photo. Guests make faster decisions when they can see the room before they book.',
      openAccordion: false,
      tab: 'settings',
      scrollBlock: 'start',
      scrollPadTop: 80,
    ...
    },
    {
      ...
      text: 'Then edit the room name, description, maximum guests, amenities, and units. These details show on your booking page.',
      openAccordion: false,
      tab: 'settings',
      scrollBlock: 'start',
      scrollPadTop: 80,
    ...
    },
    {
      ...
      text: 'Set nightly, weekly, and monthly prices here. Guests book from these rates on your direct booking page.',
      openAccordion: true,
    ...
    },
    {
      ...
      text: 'Revenue shows direct bookings and estimated OTA commission savings. Guest cards are verified to reduce no-shows, but you still collect payment at check-in.',
      openAccordion: false,
      tab: 'revenue',
      waitForVisible: true,
      scrollBlock: 'start',
  ...
  function positionTooltip(el, s, measuredRect) {
    ...
    const title = s.title || 'Quick setup';
    ...
    tooltip.innerHTML = `
      <div style="pointer-events:auto;width:100%;max-width:560px;background:#fff;color:#1A2B22;border:1.5px solid #D8E4DC;border-radius:16px;box-shadow:0 18px 50px rgba(26,43,34,0.24);padding:14px 14px 13px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
          <div style="font-size:11px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:#6B7D72;white-space:nowrap;">${current} of ${steps.length}</div>
          <div style="height:6px;flex:1;border-radius:999px;background:#E6EEE9;overflow:hidden;">
            <div style="height:100%;width:${progress}%;border-radius:999px;background:#2E7D5B;"></div>
          </div>
        </div>
        <div style="font-size:16px;font-weight:800;line-height:1.25;margin-bottom:5px;">${title}</div>
        <p style="font-size:13px;color:#4B5D52;line-height:1.45;margin:0 0 12px;">${s.text}</p>
        <div style="display:flex;align-items:center;gap:8px;">
          <button id="tourBackBtn" type="button" ${backDisabled ? 'disabled' : ''} style="min-height:40px;padding:9px 12px;border-radius:10px;border:1.5px solid #D8E4DC;background:#fff;color:${backDisabled ? '#A8B5AD' : '#1A2B22'};font-family:inherit;font-size:13px;font-weight:700;cursor:${backDisabled ? 'default' : 'pointer'};">Back</button>
          ...
          <button id="tourNextBtn" type="button" style="min-height:40px;margin-left:auto;padding:10px 18px;border-radius:10px;border:none;background:#2E7D5B;color:#fff;font-family:inherit;font-size:14px;font-weight:800;cursor:pointer;">${nextLabel}</button>
        </div>
      </div>`;
    document.body.appendChild(tooltip);
