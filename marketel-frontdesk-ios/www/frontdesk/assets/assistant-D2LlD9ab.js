import{e as q,c as a}from"./settings-EK7KiGyG.js";let I=!1,m=null;const O=12e3;function f(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function F(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function _(t,e,i){let n=null;const s=new Promise((r,d)=>{n=setTimeout(()=>d(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function D(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function S(){return E().filter(t=>t.verified)}function C(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function x(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function j(){const t=[a.editRooms,a.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const i=String(e[0]?.name||"").trim();if(i)return i}return"Queen Room"}function N(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function V(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:T(t)}function U(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(m&&!t)return m;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",k(),m=_(f("GET","/api/crm/frontdesk-assistant"),O,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,m=null,k(),document.getElementById("frontDeskAssistantOverlay")&&g()}),m}const z="marketelAssistantConfigured";function W(){try{return localStorage.getItem(z+":"+(a.activeHotelId||""))==="1"}catch{return!1}}function G(t){try{const e=z+":"+(a.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function Z(){A();let t=document.getElementById("frontDeskAssistantPill");if(!(x()&&a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive)){t?.classList.remove("is-visible");return}t||(t=document.createElement("button"),t.id="frontDeskAssistantPill",t.type="button",t.className="fda-pill",t.addEventListener("click",()=>L()),document.body.appendChild(t));const i=a.assistantData,n=E(),s=i?!!i.config?.enabled&&n.length>0:W();i&&G(s);const r=N(),d=!!r&&(r.type==="availability_warning"||r.status==="attention"),l=s?d?"Front Desk needs your review":"Front Desk Assistant":"Set up Front Desk Assistant";t.innerHTML=`<span class="fda-pill-mark" aria-hidden="true"><i data-lucide="phone" style="width:14px;height:14px;"></i></span>${d?'<span class="fda-pill-dot" aria-hidden="true"></span>':""}<span>${o(l)}</span>`,t.setAttribute("aria-label",l),t.classList.add("is-visible");try{window.lucide?.createIcons()}catch{}!i&&!a.assistantLoading&&!a.assistantError&&u().catch(()=>{})}function J(){document.getElementById("frontDeskAssistantPill")?.classList.remove("is-visible")}function k(){A();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!x()){t.innerHTML=`<div class="fda-card is-off">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${o(V(d.createdAt))} · View activity</span>
      </span>
      ...
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(Y(n.checkFrequency))}${o(d)}${o(l)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const r=!C();t.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
      <div class="fda-card-copy">
    ...
    <div class="fda-activity-copy">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},n=E(),s=t.capabilities||{},r=C(),d=Number(s.maxRecipients||3),l=n.length<d,p=r?"":"disabled",v=r&&s.manualAvailability?"":"disabled",P=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",B=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),y=[];s.smsConfigured||y.push("Marketel's texting number still needs its server credentials."),s.manualAvailability||y.push("The Assistant works with properties whose Availability is managed in Marketel.");const H=y.length?`<div class="fda-section fda-lock">
    ...
    ${!r&&!B?`<div class="fda-section fda-lock">
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        ...
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${o(P)}" ${p}></div>
      </div>
      <div class="fda-grid">
  ...
  <div class="fda-sheet-body">${X()}</div>`,M())}function tt(){document.getElementById("frontDeskAssistantSheet")&&g()}function M(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,i=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${i} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${i} minutes keeps the booking and emails the guest a confirmation.`}function et(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=R(t.value))}function L(){if(!x()){window.openFrontdeskAppDownload?.();return}F(!1),A();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&w()}),document.body.appendChild(t)),document.body.style.overflow="hidden",g(),requestAnimationFrame(()=>F(!1)),a.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function w(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",F(!0)}function it(){a.assistantError="",g(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(a.assistantData=t.data),k(),g(),e&&c(e,"success"),t}async function at(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),i=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!i)return c("Enter a name and mobile number.","error");try{const n=await f("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:i});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function nt(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await f("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(i){c(i.message||"That code could not be verified.","error")}}async function st(t){try{b(await f("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function ot(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await f("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function rt(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await f("PUT","/api/crm/frontdesk-assistant",t)),await f("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(i){c(i.message||"Could not save the assistant.","error")}}async function dt(){try{return b(await f("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function ct(){try{b(await f("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function lt(){const t=a.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),i=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=i,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(i),2e3)}async function ft(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){w(),c("Front Desk app access is managed with your Marketel account.","info");return}w();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const pt={activateFromAssistant:ft,addAssistantRecipient:at,closeFrontDeskAssistant:w,loadFrontDeskAssistant:u,openFrontDeskAssistant:L,removeAssistantRecipient:ot,refreshFrontDeskAssistantSheet:tt,renderFrontDeskAssistantCard:k,renderAssistantPill:Z,hideAssistantPill:J,retryFrontDeskAssistant:it,resendAssistantCode:st,runAssistantCheckNow:ct,saveAssistantContact:lt,saveAssistantSettings:rt,sendAssistantTest:dt,updateAssistantPolicySummary:M,updateAssistantTimeDisplay:et,verifyAssistantRecipient:nt};function mt(){I||(I=!0,A(),q(pt))}export{ft as activateFromAssistant,at as addAssistantRecipient,w as closeFrontDeskAssistant,J as hideAssistantPill,mt as install,u as loadFrontDeskAssistant,L as openFrontDeskAssistant,tt as refreshFrontDeskAssistantSheet,ot as removeAssistantRecipient,Z as renderAssistantPill,k as renderFrontDeskAssistantCard,st as resendAssistantCode,it as retryFrontDeskAssistant,ct as runAssistantCheckNow,lt as saveAssistantContact,rt as saveAssistantSettings,dt as sendAssistantTest,M as updateAssistantPolicySummary,et as updateAssistantTimeDisplay,nt as verifyAssistantRecipient};
