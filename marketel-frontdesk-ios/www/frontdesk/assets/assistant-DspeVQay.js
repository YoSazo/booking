import{e as O,c as n}from"./settings-EK7KiGyG.js";let I=!1,h=null;const _=12e3;function f(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function F(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function V(t,e,i){let a=null;const s=new Promise((r,d)=>{a=setTimeout(()=>d(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(a))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function D(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function T(){return(n.assistantData?.recipients||[]).filter(t=>t.active)}function S(){return T().filter(t=>t.verified)}function N(){return!!n.assistantData?.hotel?.subscribed||!!n.isMasterPin}function g(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function j(){const t=[n.editRooms,n.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const i=String(e[0]?.name||"").trim();if(i)return i}return"Queen Room"}function C(){if(n.assistantData?.latestResult)return n.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(n.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function U(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const a=Math.floor(i/60);if(a<24)return`${a} hr${a===1?"":"s"} ago`;const s=Math.floor(a/24);return s<7?`${s} day${s===1?"":"s"} ago`:E(t)}function Y(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(n.nativeNotificationState||""),i=e==="registered",a=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
    <div class="fda-section-title">${i?"Booking alerts are on":a?"Connecting this iPhone":s?"Booking alerts need attention":"Turn on booking alerts"}</div>
    <div class="fda-section-sub">${i?"This iPhone can receive new-booking and room-check alerts even when Front Desk is closed.":a?"Notification access is allowed. Refresh once to finish connecting this iPhone.":s?"Front Desk could not register this iPhone for alerts. Check notification settings, then refresh.":"Allow notifications so Front Desk can warn you when a booking or room check needs attention."}</div>
    ${i?'<button type="button" class="fda-btn secondary full" onclick="toggleAppNotifications()">Send a test booking alert</button>':a?'<button type="button" class="fda-btn primary full" onclick="window.location.reload()">Finish connecting</button>':'<button type="button" class="fda-btn primary full" onclick="openNativeNotificationSettings()">Open iPhone notification settings</button>'}
  </div>`}function E(t,e={}){if(!t)return"";const i=new Date(t);if(!Number.isFinite(i.getTime()))return"";const a=Date.now(),s=Math.round((i.getTime()-a)/6e4);if(e.relative&&s>0&&s<60)return`in ${s} min`;if(e.relative&&s>=60&&s<1440){const r=Math.round(s/60);return`in ${r} hr${r===1?"":"s"}`}return i.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function W(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function M(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const i=Number(e[1]);if(!Number.isInteger(i)||i<0||i>23)return"Not set";const a=i>=12?"PM":"AM";return`${i%12||12}:${e[2]} ${a}`}function $(t,e,i=""){const a=o(e||"");return`<div class="fda-time-control${i?" is-disabled":""}">
    <span class="fda-time-value">${o(M(e))}</span>
    <input id="${o(t)}" type="time" value="${a}" ${i} oninput="updateAssistantTimeDisplay(this)">
  </div>`}function A(){if(document.getElementById("frontDeskAssistantStyles"))return;const t=document.createElement("style");t.id="frontDeskAssistantStyles",t.textContent=`
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(n.assistantData&&!t)return n.assistantData;if(h&&!t)return h;const e=n.activeHotelId;return n.assistantLoading=!0,n.assistantError="",k(),h=V(f("GET","/api/crm/frontdesk-assistant"),_,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return n.activeHotelId!==e?null:(n.assistantData=i.data,i.data)}).catch(i=>{throw n.activeHotelId===e&&(n.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{n.assistantLoading=!1,h=null,k(),document.getElementById("frontDeskAssistantOverlay")&&v()}),h}const R="marketelAssistantConfigured";function G(){try{return localStorage.getItem(R+":"+(n.activeHotelId||""))==="1"}catch{return!1}}function Z(t){try{const e=R+":"+(n.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function J(){A();let t=document.getElementById("frontDeskAssistantPill");if(!(g()&&n.currentFilter==="bookings"&&n.bookingsSubview==="bookings"&&!n.settingsTourActive)){t?.classList.remove("is-visible");return}t||(t=document.createElement("button"),t.id="frontDeskAssistantPill",t.type="button",t.className="fda-pill",t.addEventListener("click",()=>P()),document.body.appendChild(t));const i=n.assistantData,a=T(),s=i?!!i.config?.enabled&&a.length>0:G();i&&Z(s);const r=C(),d=!!r&&(r.type==="availability_warning"||r.status==="attention"),l=s?d?"Front Desk needs your review":"Front Desk Assistant":"Set up Front Desk Assistant";t.innerHTML=`<span class="fda-pill-mark" aria-hidden="true"><i data-lucide="phone" style="width:14px;height:14px;"></i></span>${d?'<span class="fda-pill-dot" aria-hidden="true"></span>':""}<span>${o(l)}</span>`,t.setAttribute("aria-label",l),t.classList.add("is-visible");try{window.lucide?.createIcons()}catch{}!i&&!n.assistantLoading&&!n.assistantError&&u().catch(()=>{})}function Q(){document.getElementById("frontDeskAssistantPill")?.classList.remove("is-visible")}function k(){A();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=n.currentFilter==="bookings"&&n.bookingsSubview==="bookings"&&!n.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!g()){t.innerHTML=`<div class="fda-card is-off">
    ...
    </div>`;return}if(g()){const d=C();if(!n.assistantData||!d){!n.assistantData&&!n.assistantLoading&&!n.assistantError&&u().catch(()=>{}),t.innerHTML="",t.style.display="none";return}const l=d.type==="availability_warning"||d.status==="attention";t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result${l?" attention":""}" onclick="openFrontDeskAssistant()" aria-label="Open Front Desk Assistant activity">
      <span class="fda-native-result-icon" aria-hidden="true">${l?"!":"✓"}</span>
      <span class="fda-native-result-copy">
        <span class="fda-native-result-label">${l?"Front Desk needs your review":"Front Desk handled this"}</span>
        <span class="fda-native-result-title">${o(d.summary||"Assistant updated your property")}</span>
        <span class="fda-native-result-time">${o(U(d.createdAt))} · View activity</span>
      </span>
      <span class="fda-native-result-arrow" aria-hidden="true">›</span>
    </button>`;return}if(!n.assistantData){const d=!!n.assistantError&&!n.assistantLoading;t.innerHTML=`<div class="fda-card is-off">
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-title">${n.assistantLoading?"Connecting your assistant…":d?"Assistant could not connect":"Tell Front Desk when a room is taken"}</div>
          <div class="fda-card-sub">${d?o(n.assistantError):"Tell it when a walk-in or another channel takes a room. It updates Availability and your direct booking page."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${d?"Retry":"Open"}</button>
      </div>
    </div>`,n.assistantLoading||u().catch(()=>{});return}const i=S(),a=n.assistantData.config||{},s=n.assistantData.bookingApproval||{};if(a.enabled){const d=a.nextCheckAt?` · next ${E(a.nextCheckAt,{relative:!0})}`:"",l=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(W(a.checkFrequency))}${o(d)}${o(l)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const r=!N();t.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
      <div class="fda-card-copy">
        <div class="fda-eyebrow">${r?"Included when activated":"Front Desk Assistant"}</div>
        <div class="fda-card-title">Text Front Desk. It handles availability.</div>
        <div class="fda-card-sub">${r?"See how it protects your direct booking page before you go live.":"If a walk-in or another channel takes a room, text Front Desk what happened. It updates Availability for you."}</div>
      </div>
      <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${r?"See it":"Set up"}</button>
    </div>
  </div>`}function K(t){const e=o((t.name||"?").charAt(0).toUpperCase()),i=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      ...
      <button type="button" class="fda-icon-btn danger" onclick='removeAssistantRecipient(${D(t.id)})'>Remove</button>
    </div>
    ${t.verified?"":`<div class="fda-verify">
      ...
      <button type="button" class="fda-btn primary" onclick='verifyAssistantRecipient(${D(t.id)})'>Verify</button>
      <button type="button" class="fda-icon-btn" onclick='resendAssistantCode(${D(t.id)})'>Resend</button>
    </div>`}
  </div>`}function X(){const t=(n.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
    <div class="fda-activity-dot"></div>
    <div class="fda-activity-copy">
      ${o(e.summary||"Front Desk activity")}
      <div class="fda-activity-time">${o(E(e.createdAt))}${e.status?` · ${o(e.status)}`:""}</div>
    </div>
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function tt(){const t=n.assistantData;if(!t)return n.assistantError&&!n.assistantLoading?`<div class="fda-section">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},a=T(),s=t.capabilities||{},r=N(),d=Number(s.maxRecipients||3),l=a.length<d,p=r?"":"disabled",y=r&&s.manualAvailability?"":"disabled",L=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",H=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),w=[];s.smsConfigured||w.push("Marketel's texting number still needs its server credentials."),s.manualAvailability||w.push("The Assistant works with properties whose Availability is managed in Marketel.");const q=w.length?`<div class="fda-section fda-lock">
        <div class="fda-section-title">Not ready on this property yet</div>
        <div class="fda-section-sub" style="margin:0;">${w.map(m=>o(m)).join(" ")}</div>
      </div>`:"";return`
    ${!!e.enabled&&a.length>0?"":`<div class="fda-section fda-story">
    ...
    ${!r&&!H?`<div class="fda-section fda-lock">
    ...
    <div class="fda-section">
      ...
      <div class="fda-field" style="margin-top:13px;">
        ...
        <select id="assistant-approval-window" ${y} onchange="updateAssistantPolicySummary()">
          ${[5,10,15,20,30,45,60].map(m=>`<option value="${m}" ${Number(i.windowMinutes||20)===m?"selected":""}>${m} minutes</option>`).join("")}
        </select>
      </div>
    ...
    <div class="fda-section">
      ...
      ${a.length?a.map(K).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
      ${l?`<div class="fda-grid" style="margin-top:8px;">
      ...
      <button type="button" class="fda-btn secondary full" onclick="addAssistantRecipient()" ${r&&s.smsConfigured?"":"disabled"}>Send verification code</button>`:""}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label>${$("assistant-check-time",e.dailyCheckTime||"18:00",p)}</div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${o(L)}" ${p}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label>${$("assistant-quiet-start",e.quietHoursStart||"",p)}</div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label>${$("assistant-quiet-end",e.quietHoursEnd||"",p)}</div>
      </div>
      ...
      <button type="button" class="fda-btn primary full" style="margin-top:14px;" onclick="saveAssistantSettings()" ${r&&s.smsConfigured&&s.manualAvailability?"":"disabled"}>Save assistant settings</button>
      <div class="fda-actions">
        <button type="button" class="fda-btn secondary" onclick="sendAssistantTest()" ${S().length&&r&&s.smsConfigured?"":"disabled"}>Send test text</button>
        <button type="button" class="fda-btn secondary" onclick="runAssistantCheckNow()" ${S().length&&r&&s.smsConfigured?"":"disabled"}>Ask for an update now</button>
      </div>
    </div>
  ...
  <div class="fda-sheet-body">${tt()}</div>`,z())}function et(){document.getElementById("frontDeskAssistantSheet")&&v()}function z(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,i=Number(document.getElementById("assistant-approval-window")?.value||20),a=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=a==="release"?`<strong>Your rule:</strong> no answer after ${i} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${i} minutes keeps the booking and emails the guest a confirmation.`}function it(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=M(t.value))}function B(){return typeof document.startViewTransition=="function"&&!(window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches)}function at(t){const e=document.getElementById("frontDeskAssistantPill");if(!B()){t();return}e&&(e.style.viewTransitionName="fda-morph"),document.startViewTransition(()=>{t();const a=document.getElementById("frontDeskAssistantSheet");a&&(a.style.viewTransitionName="fda-morph"),e&&(e.style.viewTransitionName="")}).finished.finally(()=>{const a=document.getElementById("frontDeskAssistantSheet");a&&(a.style.viewTransitionName=""),e&&(e.style.viewTransitionName="")}).catch(()=>{})}function nt(){if(!g()){window.openFrontdeskAppDownload?.();return}F(!1),A();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&x()}),document.body.appendChild(t)),document.body.style.overflow="hidden",v(),requestAnimationFrame(()=>F(!1)),n.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function P(){if(!g()){window.openFrontdeskAppDownload?.();return}at(()=>nt())}function x(){const t=()=>{document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",F(!0)},e=document.getElementById("frontDeskAssistantSheet");if(!B()||!e){t();return}e.style.viewTransitionName="fda-morph",document.startViewTransition(()=>{t();const a=document.getElementById("frontDeskAssistantPill");a&&(a.style.viewTransitionName="fda-morph")}).finished.finally(()=>{const a=document.getElementById("frontDeskAssistantPill");a&&(a.style.viewTransitionName="")}).catch(()=>{})}function st(){n.assistantError="",v(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(n.assistantData=t.data),k(),v(),e&&c(e,"success"),t}async function ot(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),i=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!i)return c("Enter a name and mobile number.","error");try{const a=await f("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:i});b(a,a.verificationSent?"Verification code sent.":"Phone connected.")}catch(a){c(a.message||"Could not add that phone.","error")}}async function rt(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await f("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(i){c(i.message||"That code could not be verified.","error")}}async function dt(t){try{b(await f("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function ct(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await f("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function lt(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await f("PUT","/api/crm/frontdesk-assistant",t)),await f("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(i){c(i.message||"Could not save the assistant.","error")}}async function ft(){try{return b(await f("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function pt(){try{b(await f("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function ut(){const t=n.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),i=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),a=document.createElement("a");a.href=i,a.download="Marketel-Front-Desk.vcf",document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(i),2e3)}async function bt(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){x(),c("Front Desk app access is managed with your Marketel account.","info");return}x();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const mt={activateFromAssistant:bt,addAssistantRecipient:ot,closeFrontDeskAssistant:x,loadFrontDeskAssistant:u,openFrontDeskAssistant:P,removeAssistantRecipient:ct,refreshFrontDeskAssistantSheet:et,renderFrontDeskAssistantCard:k,renderAssistantPill:J,hideAssistantPill:Q,retryFrontDeskAssistant:st,resendAssistantCode:dt,runAssistantCheckNow:pt,saveAssistantContact:ut,saveAssistantSettings:lt,sendAssistantTest:ft,updateAssistantPolicySummary:z,updateAssistantTimeDisplay:it,verifyAssistantRecipient:rt};function yt(){I||(I=!0,A(),O(mt))}export{bt as activateFromAssistant,ot as addAssistantRecipient,x as closeFrontDeskAssistant,Q as hideAssistantPill,yt as install,u as loadFrontDeskAssistant,P as openFrontDeskAssistant,et as refreshFrontDeskAssistantSheet,ct as removeAssistantRecipient,J as renderAssistantPill,k as renderFrontDeskAssistantCard,dt as resendAssistantCode,st as retryFrontDeskAssistant,pt as runAssistantCheckNow,ut as saveAssistantContact,lt as saveAssistantSettings,ft as sendAssistantTest,z as updateAssistantPolicySummary,it as updateAssistantTimeDisplay,rt as verifyAssistantRecipient};
