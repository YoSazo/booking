import{e as L,c as i}from"./settings-B4WoF5xs.js";let T=!1,h=null;const P=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function A(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function q(t,e,a){let n=null;const o=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,o]).finally(()=>clearTimeout(n))}function s(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function k(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(i.assistantData?.recipients||[]).filter(t=>t.active)}function D(){return E().filter(t=>t.verified)}function I(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function $(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function O(){if(i.assistantData?.latestResult)return i.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(i.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function H(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const n=Math.floor(a/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const o=Math.floor(n/24);return o<7?`${o} day${o===1?"":"s"} ago`:F(t)}function _(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(i.nativeNotificationState||""),a=e==="registered",n=e==="authorized",o=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function F(t,e={}){if(!t)return"";const a=new Date(t);if(!Number.isFinite(a.getTime()))return"";const n=Date.now(),o=Math.round((a.getTime()-n)/6e4);if(e.relative&&o>0&&o<60)return`in ${o} min`;if(e.relative&&o>=60&&o<1440){const d=Math.round(o/60);return`in ${d} hr${d===1?"":"s"}`}return a.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function j(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function N(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const a=Number(e[1]);if(!Number.isInteger(a)||a<0||a>23)return"Not set";const n=a>=12?"PM":"AM";return`${a%12||12}:${e[2]} ${n}`}function w(t,e,a=""){const n=s(e||"");return`<div class="fda-time-control${a?" is-disabled":""}">
    <span class="fda-time-value">${s(N(e))}</span>
    <input id="${s(t)}" type="time" value="${n}" ${a} oninput="updateAssistantTimeDisplay(this)">
  </div>`}function S(){if(document.getElementById("frontDeskAssistantStyles"))return;const t=document.createElement("style");t.id="frontDeskAssistantStyles",t.textContent=`
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(i.assistantData&&!t)return i.assistantData;if(h&&!t)return h;const e=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",g(),h=q(l("GET","/api/crm/frontdesk-assistant"),P,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==e?null:(i.assistantData=a.data,a.data)}).catch(a=>{throw i.activeHotelId===e&&(i.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{i.assistantLoading=!1,h=null,g(),document.getElementById("frontDeskAssistantOverlay")&&m()}),h}function g(){S();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!$()){t.innerHTML=`<div class="fda-card is-off">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${s(H(r.createdAt))} · View activity</span>
      </span>
    ...
    </div>`,i.assistantLoading||u().catch(()=>{});return}const a=D(),n=i.assistantData.config||{},o=i.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${F(n.nextCheckAt,{relative:!0})}`:"",f=o.enabled?` · no reply ${o.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${s(j(n.checkFrequency))}${s(r)}${s(f)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const d=!I();t.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon">💬</div>
      <div class="fda-card-copy">
  ...
  </div>`}function V(t){const e=s((t.name||"?").charAt(0).toUpperCase()),a=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      ...
      <button type="button" class="fda-icon-btn danger" onclick='removeAssistantRecipient(${k(t.id)})'>Remove</button>
    </div>
    ${t.verified?"":`<div class="fda-verify">
      ...
      <button type="button" class="fda-btn primary" onclick='verifyAssistantRecipient(${k(t.id)})'>Verify</button>
      <button type="button" class="fda-icon-btn" onclick='resendAssistantCode(${k(t.id)})'>Resend</button>
    </div>`}
  </div>`}function U(){const t=(i.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
    <div class="fda-activity-dot"></div>
    <div class="fda-activity-copy">
      ${s(e.summary||"Front Desk activity")}
      <div class="fda-activity-time">${s(F(e.createdAt))}${e.status?` · ${s(e.status)}`:""}</div>
    </div>
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function Y(){const t=i.assistantData;if(!t)return i.assistantError&&!i.assistantLoading?`<div class="fda-section">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},a=t.bookingApproval||{},n=E(),o=t.capabilities||{},d=I(),r=Number(o.maxRecipients||3),f=n.length<r,p=d?"":"disabled",v=d&&o.manualAvailability?"":"disabled",z=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",M=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),R=o.smsConfigured?"":`<div class="fda-section fda-lock"><div class="fda-section-title">Messaging is being connected</div><div class="fda-section-sub" style="margin:0;">The interface is ready, but Marketel's texting number still needs its server credentials before it can send.</div></div>`,B=o.manualAvailability?"":'<div class="fda-section fda-lock"><div class="fda-section-title">Availability updates are not available here yet</div><div class="fda-section-sub" style="margin:0;">The Assistant currently works with properties whose Availability is managed directly in Marketel.</div></div>';return`
    ...
    <div class="fda-section">
      ...
      <div class="fda-field" style="margin-top:13px;">
        ...
        <select id="assistant-approval-window" ${v} onchange="updateAssistantPolicySummary()">
          ${[5,10,15,20,30,45,60].map(x=>`<option value="${x}" ${Number(a.windowMinutes||20)===x?"selected":""}>${x} minutes</option>`).join("")}
        </select>
      </div>
    ...
    <div class="fda-section">
      ...
      ${n.length?n.map(V).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label>${w("assistant-check-time",e.dailyCheckTime||"18:00",p)}</div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${s(z)}" ${p}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label>${w("assistant-quiet-start",e.quietHoursStart||"",p)}</div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label>${w("assistant-quiet-end",e.quietHoursEnd||"",p)}</div>
      </div>
      ...
      <div class="fda-actions">
        <button type="button" class="fda-btn secondary" onclick="sendAssistantTest()" ${D().length&&d&&o.smsConfigured?"":"disabled"}>Send test text</button>
        <button type="button" class="fda-btn secondary" onclick="runAssistantCheckNow()" ${D().length&&d&&o.smsConfigured?"":"disabled"}>Ask for an update now</button>
      </div>
    </div>
  ...
  `}function m(){const t=document.getElementById("frontDeskAssistantSheet");t&&(t.innerHTML=`<div class="fda-sheet-head">
  ...
  <div class="fda-sheet-body">${Y()}</div>`,C())}function W(){document.getElementById("frontDeskAssistantSheet")&&m()}function C(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,a=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${a} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${a} minutes keeps the booking and emails the guest a confirmation.`}function Z(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=N(t.value))}function G(){if(!$()){window.openFrontdeskAppDownload?.();return}A(!1),S();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&y()}),document.body.appendChild(t)),document.body.style.overflow="hidden",m(),requestAnimationFrame(()=>A(!1)),i.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function y(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",A(!0)}function J(){i.assistantError="",m(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(i.assistantData=t.data),g(),m(),e&&c(e,"success"),t}async function Q(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),a=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!a)return c("Enter a name and mobile number.","error");try{const n=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:a});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function K(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(a){c(a.message||"That code could not be verified.","error")}}async function X(t){try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function tt(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function et(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await l("PUT","/api/crm/frontdesk-assistant",t)),await l("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(a){c(a.message||"Could not save the assistant.","error")}}async function at(){try{return b(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function it(){try{b(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function nt(){const t=i.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const o=window.webkit?.messageHandlers?.marketelShell;if(o&&typeof o.postMessage=="function"){o.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),a=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=a,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(a),2e3)}async function ot(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){y(),c("Front Desk app access is managed with your Marketel account.","info");return}y();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const st={activateFromAssistant:ot,addAssistantRecipient:Q,closeFrontDeskAssistant:y,loadFrontDeskAssistant:u,openFrontDeskAssistant:G,removeAssistantRecipient:tt,refreshFrontDeskAssistantSheet:W,renderFrontDeskAssistantCard:g,retryFrontDeskAssistant:J,resendAssistantCode:X,runAssistantCheckNow:it,saveAssistantContact:nt,saveAssistantSettings:et,sendAssistantTest:at,updateAssistantPolicySummary:C,updateAssistantTimeDisplay:Z,verifyAssistantRecipient:K};function dt(){T||(T=!0,S(),L(st))}export{ot as activateFromAssistant,Q as addAssistantRecipient,y as closeFrontDeskAssistant,dt as install,u as loadFrontDeskAssistant,G as openFrontDeskAssistant,W as refreshFrontDeskAssistantSheet,tt as removeAssistantRecipient,g as renderFrontDeskAssistantCard,X as resendAssistantCode,J as retryFrontDeskAssistant,it as runAssistantCheckNow,nt as saveAssistantContact,et as saveAssistantSettings,at as sendAssistantTest,C as updateAssistantPolicySummary,Z as updateAssistantTimeDisplay,K as verifyAssistantRecipient};
