import{e as q,c as i,f as H}from"./settings-DtcQd-dF.js";let I=!1,m=null;const O=12e3;function l(t,e,a){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,a)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function $(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function _(t,e,a){let n=null;const o=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(a)),e)});return Promise.race([t,o]).finally(()=>clearTimeout(n))}function s(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function A(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function S(){return(i.assistantData?.recipients||[]).filter(t=>t.active)}function F(){return S().filter(t=>t.verified)}function N(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function g(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function j(){if(!g())return!1;try{const t=window.webkit?.messageHandlers?.marketelShell;return!t||typeof t.postMessage!="function"?!1:(t.postMessage({type:"openAssistant"}),!0)}catch{return!1}}function V(){const t=[i.editRooms,i.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const a=String(e[0]?.name||"").trim();if(a)return a}return"Queen Room"}function C(){if(i.assistantData?.latestResult)return i.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(i.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function U(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const a=Math.max(0,Math.floor((Date.now()-e)/6e4));if(a<1)return"Just now";if(a<60)return`${a} min ago`;const n=Math.floor(a/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const o=Math.floor(n/24);return o<7?`${o} day${o===1?"":"s"} ago`:T(t)}function Y(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(i.nativeNotificationState||""),a=e==="registered",n=e==="authorized",o=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function T(t,e={}){if(!t)return"";const a=new Date(t);if(!Number.isFinite(a.getTime()))return"";const n=Date.now(),o=Math.round((a.getTime()-n)/6e4);if(e.relative&&o>0&&o<60)return`in ${o} min`;if(e.relative&&o>=60&&o<1440){const d=Math.round(o/60);return`in ${d} hr${d===1?"":"s"}`}return a.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function W(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function R(t){const e=String(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return"Not set";const a=Number(e[1]);if(!Number.isInteger(a)||a<0||a>23)return"Not set";const n=a>=12?"PM":"AM";return`${a%12||12}:${e[2]} ${n}`}function D(t,e,a=""){const n=s(e||"");return`<div class="fda-time-control${a?" is-disabled":""}">
    <span class="fda-time-value">${s(R(e))}</span>
  ...
  `,document.head.appendChild(t)}async function f({force:t=!1}={}){if(i.assistantData&&!t)return i.assistantData;if(m&&!t)return m;const e=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",k(),m=_(l("GET","/api/crm/frontdesk-assistant"),O,"Front Desk Assistant took too long to respond. Tap retry.").then(a=>{if(!a?.success)throw new Error(a?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==e?null:(i.assistantData=a.data,a.data)}).catch(a=>{throw i.activeHotelId===e&&(i.assistantError=a?.message||"Could not load Front Desk Assistant."),a}).finally(()=>{i.assistantLoading=!1,m=null,k(),document.getElementById("frontDeskAssistantOverlay")&&v()}),m}const M="marketelAssistantConfigured";function G(){try{return localStorage.getItem(M+":"+(i.activeHotelId||""))==="1"}catch{return!1}}function Z(t){try{const e=M+":"+(i.activeHotelId||"");t?localStorage.setItem(e,"1"):localStorage.removeItem(e)}catch{}}function J(){const t=i.assistantData,e=S(),a=t?!!t.config?.enabled&&e.length>0:G();t&&Z(a);const n=C(),o=!!n&&(n.type==="availability_warning"||n.status==="attention");window.marketelAssistantPillLabel=a?o?"Needs your review":"Front Desk":"Set up Front Desk",window.syncNativeShellState?.(),!t&&!i.assistantLoading&&!i.assistantError&&f().catch(()=>{})}function Q(){window.marketelAssistantPillLabel="Front Desk"}function k(){E();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!g()){t.innerHTML=`<div class="fda-card is-off">
      <div class="fda-card-row">
        <div class="fda-card-icon fda-card-app-icon"><img src="${H}" width="42" height="42" alt=""></div>
    ...
    </div>`,i.assistantLoading||f().catch(()=>{});return}const a=F(),n=i.assistantData.config||{},o=i.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${T(n.nextCheckAt,{relative:!0})}`:"",p=o.enabled?` · no reply ${o.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
        <div class="fda-card-copy">
    ...
    <div class="fda-activity-copy">
      ...
      <div class="fda-activity-time">${s(T(e.createdAt))}${e.status?` · ${s(e.status)}`:""}</div>
    </div>
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},a=t.bookingApproval||{},n=S(),o=t.capabilities||{},d=N(),r=Number(o.maxRecipients||3),p=n.length<r,u=d?"":"disabled",y=d&&o.manualAvailability?"":"disabled",P=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",L=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),w=[];o.smsConfigured||w.push("Marketel's texting number still needs its server credentials."),o.manualAvailability||w.push("The Assistant works with properties whose Availability is managed in Marketel.");const B=w.length?`<div class="fda-section fda-lock">
        <div class="fda-section-title">Not ready on this property yet</div>
        <div class="fda-section-sub" style="margin:0;">${w.map(h=>s(h)).join(" ")}</div>
      </div>`:"";return`
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Review new bookings before confirmation">
          <input id="assistant-approval-enabled" type="checkbox" ${a.enabled?"checked":""} ${y} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
          <span class="fda-switch-track" aria-hidden="true"></span>
        </label>
      </div>
      ...
      <div class="fda-policy-options">
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="confirm" ${a.noResponseAction!=="release"?"checked":""} ${y} onchange="updateAssistantPolicySummary()">
        ...
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="release" ${a.noResponseAction==="release"?"checked":""} ${y} onchange="updateAssistantPolicySummary()">
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        ...
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${s(P)}" ${u}></div>
      </div>
      <div class="fda-grid">
