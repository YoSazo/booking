import{e as z,c as i}from"./settings-CsnKIiEZ.js";let D=!1,b=null;const B=12e3;function l(e,t,n){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(e,t,n)}function r(e,t="info"){typeof window.toast=="function"&&window.toast(e,t)}function w(e){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(e);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!e})}catch{}}function M(e,t,n){let a=null;const s=new Promise((o,c)=>{a=setTimeout(()=>c(new Error(n)),t)});return Promise.race([e,s]).finally(()=>clearTimeout(a))}function d(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function x(e){return JSON.stringify(String(e??"")).replace(/</g,"\\u003c")}function F(){return(i.assistantData?.recipients||[]).filter(e=>e.active)}function A(){return F().filter(e=>e.verified)}function E(){return!!i.assistantData?.hotel?.subscribed||!!i.isMasterPin}function P(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const t=String(i.nativeNotificationState||""),n=t==="registered",a=t==="authorized",s=t==="unavailable";return`<div class="fda-section">
    <div class="fda-section-title">${n?"Booking alerts are on":a?"Connecting this iPhone":s?"Booking alerts need attention":"Turn on booking alerts"}</div>
    <div class="fda-section-sub">${n?"This iPhone can receive new-booking and room-check alerts even when Front Desk is closed.":a?"Notification access is allowed. Refresh once to finish connecting this iPhone.":s?"Front Desk could not register this iPhone for alerts. Check notification settings, then refresh.":"Allow notifications so Front Desk can warn you when a booking or room check needs attention."}</div>
    ${n?'<button type="button" class="fda-btn secondary full" onclick="toggleAppNotifications()">Send a test booking alert</button>':a?'<button type="button" class="fda-btn primary full" onclick="window.location.reload()">Finish connecting</button>':'<button type="button" class="fda-btn primary full" onclick="openNativeNotificationSettings()">Open iPhone notification settings</button>'}
  </div>`}function S(e,t={}){if(!e)return"";const n=new Date(e);if(!Number.isFinite(n.getTime()))return"";const a=Date.now(),s=Math.round((n.getTime()-a)/6e4);if(t.relative&&s>0&&s<60)return`in ${s} min`;if(t.relative&&s>=60&&s<1440){const o=Math.round(s/60);return`in ${o} hr${o===1?"":"s"}`}return n.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function q(e){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[e]||"Smart daily check"}function $(){if(document.getElementById("frontDeskAssistantStyles"))return;const e=document.createElement("style");e.id="frontDeskAssistantStyles",e.textContent=`
  ...
  `,document.head.appendChild(e)}async function u({force:e=!1}={}){if(i.assistantData&&!e)return i.assistantData;if(b&&!e)return b;const t=i.activeHotelId;return i.assistantLoading=!0,i.assistantError="",h(),b=M(l("GET","/api/crm/frontdesk-assistant"),B,"Front Desk Assistant took too long to respond. Tap retry.").then(n=>{if(!n?.success)throw new Error(n?.message||"Could not load Front Desk Assistant.");return i.activeHotelId!==t?null:(i.assistantData=n.data,n.data)}).catch(n=>{throw i.activeHotelId===t&&(i.assistantError=n?.message||"Could not load Front Desk Assistant."),n}).finally(()=>{i.assistantLoading=!1,b=null,h(),document.getElementById("frontDeskAssistantOverlay")&&v()}),b}function h(){$();const e=document.getElementById("frontDeskAssistantPanel");if(!e)return;const t=i.currentFilter==="bookings"&&i.bookingsSubview==="bookings"&&!i.settingsTourActive;if(e.style.display=t?"block":"none",!t)return;if(!i.assistantData){const c=!!i.assistantError&&!i.assistantLoading;e.innerHTML=`<div class="fda-card is-off">
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${c?d(i.assistantError):"It updates availability for you and helps prevent outside bookings from colliding."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${c?"Retry":"Open"}</button>
      </div>
    </div>`,i.assistantLoading||u().catch(()=>{});return}const n=A(),a=i.assistantData.config||{},s=i.assistantData.bookingApproval||{};if(a.enabled){const c=a.nextCheckAt?` · next ${S(a.nextCheckAt,{relative:!0})}`:"",g=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";e.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
          <div class="fda-eyebrow fda-live">Assistant on</div>
          <div class="fda-card-title">Front Desk is watching ${n.length} phone${n.length===1?"":"s"}</div>
          <div class="fda-card-sub">${d(q(a.checkFrequency))}${d(c)}${d(g)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const o=!E();e.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon">💬</div>
      <div class="fda-card-copy">
        <div class="fda-eyebrow">${o?"Included when activated":"Front Desk Assistant"}</div>
        <div class="fda-card-title">Text Front Desk. It handles availability.</div>
        <div class="fda-card-sub">${o?"See how it protects your direct booking page before you go live.":"Connect your phone and tell it when a walk-in takes a room."}</div>
      </div>
      <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${o?"See it":"Set up"}</button>
    </div>
  </div>`}function O(e){const t=d((e.name||"?").charAt(0).toUpperCase()),n=[e.role,e.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      <div class="fda-avatar">${t}</div>
      <div class="fda-person-copy">
        <div class="fda-person-name">${d(e.name)}</div>
        <div class="fda-person-meta">${d(n)}</div>
      </div>
    ...
    ${e.verified?"":`<div class="fda-verify">
      <input id="assistant-code-${d(e.id)}" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="6-digit code" aria-label="Verification code">
    ...
    <div class="fda-activity-copy">
      ${d(t.summary||"Front Desk activity")}
      <div class="fda-activity-time">${d(S(t.createdAt))}${t.status?` · ${d(t.status)}`:""}</div>
    </div>
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function H(){const e=i.assistantData;if(!e)return i.assistantError&&!i.assistantLoading?`<div class="fda-section">
        <div class="fda-section-title">Assistant could not connect</div>
        <div class="fda-section-sub">${d(i.assistantError)}</div>
        <button type="button" class="fda-btn primary full" onclick="retryFrontDeskAssistant()">Retry</button>
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const t=e.config||{},n=e.bookingApproval||{},a=F(),s=e.capabilities||{},o=E(),c=Number(s.maxRecipients||3),g=a.length<c,f=o?"":"disabled",m=o&&s.manualAvailability?"":"disabled",I=t.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",C=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),N=s.smsConfigured?"":`<div class="fda-section fda-lock"><div class="fda-section-title">Messaging is being connected</div><div class="fda-section-sub" style="margin:0;">The interface is ready, but Marketel's texting number still needs its server credentials before it can send.</div></div>`,R=s.manualAvailability?"":'<div class="fda-section fda-lock"><div class="fda-section-title">Availability updates are not available here yet</div><div class="fda-section-sub" style="margin:0;">The Assistant currently works with properties whose Availability is managed directly in Marketel.</div></div>';return`
    ...
    ${!o&&!C?`<div class="fda-section fda-lock">
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <input class="fda-toggle" id="assistant-approval-enabled" type="checkbox" ${n.enabled?"checked":""} ${m} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
      </div>
      <div class="fda-field" style="margin-top:13px;">
        ...
        <select id="assistant-approval-window" ${m} onchange="updateAssistantPolicySummary()">
          ${[5,10,15,20,30,45,60].map(k=>`<option value="${k}" ${Number(n.windowMinutes||20)===k?"selected":""}>${k} minutes</option>`).join("")}
        </select>
      </div>
      ...
      <div class="fda-policy-options">
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="confirm" ${n.noResponseAction!=="release"?"checked":""} ${m} onchange="updateAssistantPolicySummary()">
        ...
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="release" ${n.noResponseAction==="release"?"checked":""} ${m} onchange="updateAssistantPolicySummary()">
    ...
    <div class="fda-section">
      ...
      ${a.length?a.map(O).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
      ...
      <button type="button" class="fda-btn secondary full" onclick="addAssistantRecipient()" ${o&&s.smsConfigured?"":"disabled"}>Send verification code</button>`:""}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label><input id="assistant-check-time" type="time" value="${d(t.dailyCheckTime||"18:00")}" ${f}></div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${d(I)}" ${f}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label><input id="assistant-quiet-start" type="time" value="${d(t.quietHoursStart||"")}" ${f}></div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label><input id="assistant-quiet-end" type="time" value="${d(t.quietHoursEnd||"")}" ${f}></div>
      </div>
      ...
      <button type="button" class="fda-btn primary full" style="margin-top:14px;" onclick="saveAssistantSettings()" ${o&&s.smsConfigured&&s.manualAvailability?"":"disabled"}>Save assistant settings</button>
      <div class="fda-actions">
        <button type="button" class="fda-btn secondary" onclick="sendAssistantTest()" ${A().length&&o&&s.smsConfigured?"":"disabled"}>Send test text</button>
        <button type="button" class="fda-btn secondary" onclick="runAssistantCheckNow()" ${A().length&&o&&s.smsConfigured?"":"disabled"}>Ask for an update now</button>
      </div>
    </div>
  ...
  <div class="fda-sheet-body">${H()}</div>`,T())}function j(){document.getElementById("frontDeskAssistantSheet")&&v()}function T(){const e=document.getElementById("assistant-policy-result");if(!e)return;const t=!!document.getElementById("assistant-approval-enabled")?.checked,n=Number(document.getElementById("assistant-approval-window")?.value||20),a=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!t){e.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}e.innerHTML=a==="release"?`<strong>Your rule:</strong> no answer after ${n} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${n} minutes keeps the booking and emails the guest a confirmation.`}function V(){w(!1),$();let e=document.getElementById("frontDeskAssistantOverlay");e||(e=document.createElement("div"),e.id="frontDeskAssistantOverlay",e.className="fda-overlay",e.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',e.addEventListener("click",t=>{t.target===e&&y()}),document.body.appendChild(e)),document.body.style.overflow="hidden",v(),requestAnimationFrame(()=>w(!1)),i.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function y(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",w(!0)}function U(){i.assistantError="",v(),u({force:!0}).catch(()=>{})}function p(e,t){if(!e?.success)throw new Error(e?.message||"That did not save.");return e.data&&(i.assistantData=e.data),h(),v(),t&&r(t,"success"),e}async function _(){const e=document.getElementById("assistant-person-name")?.value.trim(),t=document.getElementById("assistant-person-role")?.value.trim(),n=document.getElementById("assistant-person-phone")?.value.trim();if(!e||!n)return r("Enter a name and mobile number.","error");try{const a=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:e,role:t,phone:n});p(a,a.verificationSent?"Verification code sent.":"Phone connected.")}catch(a){r(a.message||"Could not add that phone.","error")}}async function Y(e){const t=document.getElementById(`assistant-code-${e}`)?.value.trim();if(!t||t.length!==6)return r("Enter the 6-digit code.","error");try{p(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(e)}/verify`,{code:t}),"Phone connected to Front Desk.")}catch(n){r(n.message||"That code could not be verified.","error")}}async function W(e){try{p(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(e)}/resend`,{}),"A new code was sent.")}catch(t){r(t.message||"Could not resend the code.","error")}}async function Z(e){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{p(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(e)}`),"Phone removed.")}catch(t){r(t.message||"Could not remove that phone.","error")}}async function G(){const e={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},t={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{p(await l("PUT","/api/crm/frontdesk-assistant",e)),await l("POST","/api/crm/booking-approval",t),await u({force:!0}),r("Front Desk Assistant and booking rule saved.","success")}catch(n){r(n.message||"Could not save the assistant.","error")}}async function Q(){try{return p(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(e){r(e.message||"Could not send a test text.","error")}}async function J(){try{p(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(e){r(e.message||"Could not send that check.","error")}}function K(){const e=i.assistantData?.capabilities?.assistantPhone||"";if(!e)return r("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:e});return}}catch{}const t=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${e}`,"NOTE:Text this contact when a walk-in or outside booking changes your availability.","END:VCARD"].join(`\r
`),n=URL.createObjectURL(new Blob([t],{type:"text/vcard;charset=utf-8"})),a=document.createElement("a");a.href=n,a.download="Marketel-Front-Desk.vcf",document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(n),2e3)}async function X(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){y(),r("Front Desk app access is managed with your Marketel account.","info");return}y();try{const e=await window.loadSettingsModule?.(),t=window.goLive||e?.goLive;if(typeof t=="function"){t();return}}catch{}r("Open Your page to activate Marketel.","info")}const ee={activateFromAssistant:X,addAssistantRecipient:_,closeFrontDeskAssistant:y,loadFrontDeskAssistant:u,openFrontDeskAssistant:V,removeAssistantRecipient:Z,refreshFrontDeskAssistantSheet:j,renderFrontDeskAssistantCard:h,retryFrontDeskAssistant:U,resendAssistantCode:W,runAssistantCheckNow:J,saveAssistantContact:K,saveAssistantSettings:G,sendAssistantTest:Q,updateAssistantPolicySummary:T,verifyAssistantRecipient:Y};function ne(){D||(D=!0,$(),z(ee))}export{X as activateFromAssistant,_ as addAssistantRecipient,y as closeFrontDeskAssistant,ne as install,u as loadFrontDeskAssistant,V as openFrontDeskAssistant,j as refreshFrontDeskAssistantSheet,Z as removeAssistantRecipient,h as renderFrontDeskAssistantCard,W as resendAssistantCode,U as retryFrontDeskAssistant,J as runAssistantCheckNow,K as saveAssistantContact,G as saveAssistantSettings,Q as sendAssistantTest,T as updateAssistantPolicySummary,Y as verifyAssistantRecipient};
