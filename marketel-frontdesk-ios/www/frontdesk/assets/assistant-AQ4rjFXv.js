import{e as R,c as a}from"./settings-BScQqWYt.js";let F=!1,v=null;const M=12e3;function l(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function w(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function B(t,e,i){let n=null;const s=new Promise((d,r)=>{n=setTimeout(()=>r(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function x(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function A(){return E().filter(t=>t.verified)}function S(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function P(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function q(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function L(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:$(t)}function O(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  </div>`}function $(t,e={}){if(!t)return"";const i=new Date(t);if(!Number.isFinite(i.getTime()))return"";const n=Date.now(),s=Math.round((i.getTime()-n)/6e4);if(e.relative&&s>0&&s<60)return`in ${s} min`;if(e.relative&&s>=60&&s<1440){const d=Math.round(s/60);return`in ${d} hr${d===1?"":"s"}`}return i.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function H(t){return{smart:"Evening availability check","2h":"Every 2 hours","4h":"Every 4 hours",daily:"Once daily",booking_only:"New bookings only",off:"No check-ins"}[t]||"Smart daily check"}function D(){if(document.getElementById("frontDeskAssistantStyles"))return;const t=document.createElement("style");t.id="frontDeskAssistantStyles",t.textContent=`
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(v&&!t)return v;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",m(),v=B(l("GET","/api/crm/frontdesk-assistant"),M,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,v=null,m(),document.getElementById("frontDeskAssistantOverlay")&&h()}),v}function m(){D();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(P()){const r=q();if(!a.assistantData||!r){t.innerHTML="",t.style.display="none",!a.assistantData&&!a.assistantLoading&&!a.assistantError&&u().catch(()=>{});return}const f=r.type==="availability_warning"||r.status==="attention";t.style.display="block",t.innerHTML=`<button type="button" class="fda-native-result${f?" attention":""}" onclick="openFrontDeskAssistant()" aria-label="Open Front Desk Assistant activity">
    ...
    </div>`,a.assistantLoading||u().catch(()=>{});return}const i=A(),n=a.assistantData.config||{},s=a.assistantData.bookingApproval||{};if(n.enabled){const r=n.nextCheckAt?` · next ${$(n.nextCheckAt,{relative:!0})}`:"",f=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon">💬</div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(H(n.checkFrequency))}${o(r)}${o(f)}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">Manage</button>
      </div>
    </div>`;return}const d=!S();t.innerHTML=`<div class="fda-card is-off">
    <div class="fda-card-row">
      <div class="fda-card-icon">💬</div>
      <div class="fda-card-copy">
  ...
  </div>`}function _(t){const e=o((t.name||"?").charAt(0).toUpperCase()),i=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
    <div class="fda-person">
      ...
      <button type="button" class="fda-icon-btn danger" onclick='removeAssistantRecipient(${x(t.id)})'>Remove</button>
    </div>
    ${t.verified?"":`<div class="fda-verify">
      ...
      <button type="button" class="fda-btn primary" onclick='verifyAssistantRecipient(${x(t.id)})'>Verify</button>
      <button type="button" class="fda-icon-btn" onclick='resendAssistantCode(${x(t.id)})'>Resend</button>
    </div>`}
  </div>`}function j(){const t=(a.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
  ...
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function V(){const t=a.assistantData;if(!t)return a.assistantError&&!a.assistantLoading?`<div class="fda-section">
    ...
    <div class="fda-section">
      ...
      <div class="fda-field" style="margin-top:13px;">
        ...
        <select id="assistant-approval-window" ${g} onchange="updateAssistantPolicySummary()">
          ${[5,10,15,20,30,45,60].map(k=>`<option value="${k}" ${Number(i.windowMinutes||20)===k?"selected":""}>${k} minutes</option>`).join("")}
        </select>
      </div>
    ...
    <div class="fda-section">
      ...
      ${n.length?n.map(_).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label><input id="assistant-check-time" type="time" value="${o(e.dailyCheckTime||"18:00")}" ${p}></div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${o(I)}" ${p}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label><input id="assistant-quiet-start" type="time" value="${o(e.quietHoursStart||"")}" ${p}></div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label><input id="assistant-quiet-end" type="time" value="${o(e.quietHoursEnd||"")}" ${p}></div>
      </div>
  ...
  `}function h(){const t=document.getElementById("frontDeskAssistantSheet");t&&(t.innerHTML=`<div class="fda-sheet-head">
  ...
  <div class="fda-sheet-body">${V()}</div>`,T())}function U(){document.getElementById("frontDeskAssistantSheet")&&h()}function T(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,i=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${i} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${i} minutes keeps the booking and emails the guest a confirmation.`}function Y(){w(!1),D();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&y()}),document.body.appendChild(t)),document.body.style.overflow="hidden",h(),requestAnimationFrame(()=>w(!1)),a.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function y(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",w(!0)}function W(){a.assistantError="",h(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(a.assistantData=t.data),m(),h(),e&&c(e,"success"),t}async function Z(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),i=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!i)return c("Enter a name and mobile number.","error");try{const n=await l("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:i});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function G(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(i){c(i.message||"That code could not be verified.","error")}}async function J(t){try{b(await l("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function Q(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await l("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function K(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await l("PUT","/api/crm/frontdesk-assistant",t)),await l("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(i){c(i.message||"Could not save the assistant.","error")}}async function X(){try{return b(await l("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function tt(){try{b(await l("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function et(){const t=a.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:Text this contact when a walk-in or outside booking changes your availability.","END:VCARD"].join(`\r
`),i=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=i,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(i),2e3)}async function it(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){y(),c("Front Desk app access is managed with your Marketel account.","info");return}y();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const at={activateFromAssistant:it,addAssistantRecipient:Z,closeFrontDeskAssistant:y,loadFrontDeskAssistant:u,openFrontDeskAssistant:Y,removeAssistantRecipient:Q,refreshFrontDeskAssistantSheet:U,renderFrontDeskAssistantCard:m,retryFrontDeskAssistant:W,resendAssistantCode:J,runAssistantCheckNow:tt,saveAssistantContact:et,saveAssistantSettings:K,sendAssistantTest:X,updateAssistantPolicySummary:T,verifyAssistantRecipient:G};function st(){F||(F=!0,D(),R(at))}export{it as activateFromAssistant,Z as addAssistantRecipient,y as closeFrontDeskAssistant,st as install,u as loadFrontDeskAssistant,Y as openFrontDeskAssistant,U as refreshFrontDeskAssistantSheet,Q as removeAssistantRecipient,m as renderFrontDeskAssistantCard,J as resendAssistantCode,W as retryFrontDeskAssistant,tt as runAssistantCheckNow,et as saveAssistantContact,K as saveAssistantSettings,X as sendAssistantTest,T as updateAssistantPolicySummary,G as verifyAssistantRecipient};
