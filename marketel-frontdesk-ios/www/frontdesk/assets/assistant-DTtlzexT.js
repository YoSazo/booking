import{e as q,c as a}from"./settings-6g3gADOR.js";let I=!1,m=null;const H=12e3;function p(t,e,i){return typeof window.api!="function"?Promise.reject(new Error("Front Desk is not ready.")):window.api(t,e,i)}function c(t,e="info"){typeof window.toast=="function"&&window.toast(t,e)}function F(t){if(typeof window.setNativeShellVisible=="function"){window.setNativeShellVisible(t);return}try{window.webkit?.messageHandlers?.marketelShell?.postMessage({type:"visibility",visible:!!t})}catch{}}function O(t,e,i){let n=null;const s=new Promise((r,d)=>{n=setTimeout(()=>d(new Error(i)),e)});return Promise.race([t,s]).finally(()=>clearTimeout(n))}function o(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function D(t){return JSON.stringify(String(t??"")).replace(/</g,"\\u003c")}function E(){return(a.assistantData?.recipients||[]).filter(t=>t.active)}function S(){return E().filter(t=>t.verified)}function N(){return!!a.assistantData?.hotel?.subscribed||!!a.isMasterPin}function x(){return typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()}function _(){const t=[a.editRooms,a.manualAvailability?.rooms];for(const e of t){if(!Array.isArray(e))continue;const i=String(e[0]?.name||"").trim();if(i)return i}return"Queen Room"}function C(){if(a.assistantData?.latestResult)return a.assistantData.latestResult;const t=new Set(["availability_update","booking_decision","availability_warning"]);return(a.assistantData?.activities||[]).find(e=>t.has(String(e?.type||"")))||null}function j(t){const e=new Date(t||0).getTime();if(!Number.isFinite(e)||e<=0)return"Saved in Assistant activity";const i=Math.max(0,Math.floor((Date.now()-e)/6e4));if(i<1)return"Just now";if(i<60)return`${i} min ago`;const n=Math.floor(i/60);if(n<24)return`${n} hr${n===1?"":"s"} ago`;const s=Math.floor(n/24);return s<7?`${s} day${s===1?"":"s"} ago`:T(t)}function V(){if(!(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()))return"";const e=String(a.nativeNotificationState||""),i=e==="registered",n=e==="authorized",s=e==="unavailable";return`<div class="fda-section">
  ...
  `,document.head.appendChild(t)}async function u({force:t=!1}={}){if(a.assistantData&&!t)return a.assistantData;if(m&&!t)return m;const e=a.activeHotelId;return a.assistantLoading=!0,a.assistantError="",k(),m=O(p("GET","/api/crm/frontdesk-assistant"),H,"Front Desk Assistant took too long to respond. Tap retry.").then(i=>{if(!i?.success)throw new Error(i?.message||"Could not load Front Desk Assistant.");return a.activeHotelId!==e?null:(a.assistantData=i.data,i.data)}).catch(i=>{throw a.activeHotelId===e&&(a.assistantError=i?.message||"Could not load Front Desk Assistant."),i}).finally(()=>{a.assistantLoading=!1,m=null,k(),document.getElementById("frontDeskAssistantOverlay")&&g()}),m}function Y(){A();let t=document.getElementById("frontDeskAssistantPill");if(!(x()&&a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive)){t?.classList.remove("is-visible");return}t||(t=document.createElement("button"),t.id="frontDeskAssistantPill",t.type="button",t.className="fda-pill",t.addEventListener("click",()=>M()),document.body.appendChild(t));const i=a.assistantData,n=E(),s=!!i?.config?.enabled&&n.length>0,r=C(),d=!!r&&(r.type==="availability_warning"||r.status==="attention"),l=s?d?"Front Desk needs your review":"Front Desk Assistant":"Set up Front Desk Assistant";t.innerHTML=`<span class="fda-pill-mark" aria-hidden="true"><i data-lucide="phone" style="width:14px;height:14px;"></i></span>${d?'<span class="fda-pill-dot" aria-hidden="true"></span>':""}<span>${o(l)}</span>`,t.setAttribute("aria-label",l),t.classList.add("is-visible");try{window.lucide?.createIcons()}catch{}!i&&!a.assistantLoading&&!a.assistantError&&u().catch(()=>{})}function W(){document.getElementById("frontDeskAssistantPill")?.classList.remove("is-visible")}function k(){A();const t=document.getElementById("frontDeskAssistantPanel");if(!t)return;const e=a.currentFilter==="bookings"&&a.bookingsSubview==="bookings"&&!a.settingsTourActive;if(t.style.display=e?"block":"none",!e)return;if(!x()){t.innerHTML=`<div class="fda-card is-off">
      ...
      <span class="fda-native-result-copy">
        ...
        <span class="fda-native-result-time">${o(j(d.createdAt))} · View activity</span>
      </span>
      <span class="fda-native-result-arrow" aria-hidden="true">›</span>
    </button>`;return}if(!a.assistantData){const d=!!a.assistantError&&!a.assistantLoading;t.innerHTML=`<div class="fda-card is-off">
      <div class="fda-card-row">
        ...
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${d?o(a.assistantError):"Tell it when a walk-in or another channel takes a room. It updates Availability and your direct booking page."}</div>
        </div>
        <button type="button" class="fda-card-btn" onclick="openFrontDeskAssistant()">${d?"Retry":"Open"}</button>
      </div>
    </div>`,a.assistantLoading||u().catch(()=>{});return}const i=S(),n=a.assistantData.config||{},s=a.assistantData.bookingApproval||{};if(n.enabled){const d=n.nextCheckAt?` · next ${T(n.nextCheckAt,{relative:!0})}`:"",l=s.enabled?` · no reply ${s.noResponseAction==="release"?"releases request":"keeps booking"}`:"";t.innerHTML=`<div class="fda-card">
      <div class="fda-card-row">
        <div class="fda-card-icon"><i data-lucide="message-circle" style="width:16px;height:16px;"></i></div>
        <div class="fda-card-copy">
          ...
          <div class="fda-card-sub">${o(U(n.checkFrequency))}${o(d)}${o(l)}</div>
        </div>
  ...
  </div>`}function Z(t){const e=o((t.name||"?").charAt(0).toUpperCase()),i=[t.role,t.maskedPhone].filter(Boolean).join(" · ");return`<div>
  ...
  </div>`}function G(){const t=(a.assistantData?.activities||[]).slice(0,10);return t.length?t.map(e=>`<div class="fda-activity">
    ...
    <div class="fda-activity-copy">
  ...
  </div>`).join(""):'<div class="fda-section-sub" style="margin:4px 0 0;">Booking alerts and availability updates will appear here.</div>'}function J(){const t=a.assistantData;if(!t)return a.assistantError&&!a.assistantLoading?`<div class="fda-section">
      ...
      </div>`:'<div class="fda-section"><div class="loading"><div class="logo-sprite-bounce"></div> Opening assistant…</div></div>';const e=t.config||{},i=t.bookingApproval||{},n=E(),s=t.capabilities||{},r=N(),d=Number(s.maxRecipients||3),l=n.length<d,f=r?"":"disabled",v=r&&s.manualAvailability?"":"disabled",L=e.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",P=typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp(),y=[];s.smsConfigured||y.push("Marketel's texting number still needs its server credentials."),s.manualAvailability||y.push("The Assistant works with properties whose Availability is managed in Marketel.");const B=y.length?`<div class="fda-section fda-lock">
    ...
    ${!!e.enabled&&n.length>0?"":`<div class="fda-section fda-story">
      ...
      <div class="fda-bubble assistant"><strong>Front Desk</strong><br>New booking: ${o(_())}, tonight. Is it still free?</div>
    ...
    ${!r&&!P?`<div class="fda-section fda-lock">
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Turn Front Desk Assistant on">
          <input id="assistant-enabled" type="checkbox" ${e.enabled?"checked":""} ${f} aria-label="Turn Front Desk Assistant on">
    ...
    <div class="fda-section">
      <div class="fda-row fda-between">
        ...
        <label class="fda-switch" aria-label="Review new bookings before confirmation">
          <input id="assistant-approval-enabled" type="checkbox" ${i.enabled?"checked":""} ${v} aria-label="Review new bookings before confirmation" onchange="updateAssistantPolicySummary()">
          <span class="fda-switch-track" aria-hidden="true"></span>
        </label>
      </div>
      ...
      <div class="fda-policy-options">
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="confirm" ${i.noResponseAction!=="release"?"checked":""} ${v} onchange="updateAssistantPolicySummary()">
        ...
        <label class="fda-policy-option">
          <input type="radio" name="assistant-no-response" value="release" ${i.noResponseAction==="release"?"checked":""} ${v} onchange="updateAssistantPolicySummary()">
    ...
    <div class="fda-section">
      ...
      ${n.length?n.map(Z).join(""):'<div class="fda-section-sub" style="padding:4px 0 10px;">No phones connected yet.</div>'}
    ...
    <div class="fda-section">
      ...
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-check-time">Daily check time</label>${$("assistant-check-time",e.dailyCheckTime||"18:00",f)}</div>
        <div class="fda-field"><label for="assistant-time-zone">Time zone</label><input id="assistant-time-zone" type="text" value="${o(L)}" ${f}></div>
      </div>
      <div class="fda-grid">
        <div class="fda-field"><label for="assistant-quiet-start">Quiet hours start</label>${$("assistant-quiet-start",e.quietHoursStart||"",f)}</div>
        <div class="fda-field"><label for="assistant-quiet-end">Quiet hours end</label>${$("assistant-quiet-end",e.quietHoursEnd||"",f)}</div>
      </div>
      <label class="fda-row" style="font-size:12px;color:#40574b;margin-top:2px;cursor:pointer;">
        <input id="assistant-booking-alerts" type="checkbox" ${e.notifyNewBookings!==!1?"checked":""} ${f}>
  ...
  <div class="fda-sheet-body">${J()}</div>`,z())}function Q(){document.getElementById("frontDeskAssistantSheet")&&g()}function z(){const t=document.getElementById("assistant-policy-result");if(!t)return;const e=!!document.getElementById("assistant-approval-enabled")?.checked,i=Number(document.getElementById("assistant-approval-window")?.value||20),n=document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm";if(!e){t.innerHTML="<strong>Off.</strong> New direct bookings confirm immediately.";return}t.innerHTML=n==="release"?`<strong>Your rule:</strong> no answer after ${i} minutes releases the request, voids the $1 hold and emails the guest.`:`<strong>Your rule:</strong> no answer after ${i} minutes keeps the booking and emails the guest a confirmation.`}function K(t){const e=t?.closest?.(".fda-time-control")?.querySelector(".fda-time-value");e&&(e.textContent=R(t.value))}function M(){if(!x()){window.openFrontdeskAppDownload?.();return}F(!1),A();let t=document.getElementById("frontDeskAssistantOverlay");t||(t=document.createElement("div"),t.id="frontDeskAssistantOverlay",t.className="fda-overlay",t.innerHTML='<div class="fda-sheet" id="frontDeskAssistantSheet" role="dialog" aria-modal="true" aria-label="Front Desk Assistant"></div>',t.addEventListener("click",e=>{e.target===t&&w()}),document.body.appendChild(t)),document.body.style.overflow="hidden",g(),requestAnimationFrame(()=>F(!1)),a.assistantData?u({force:!0}).catch(()=>{}):u().catch(()=>{})}function w(){document.getElementById("frontDeskAssistantOverlay")?.remove(),document.body.style.overflow="",F(!0)}function X(){a.assistantError="",g(),u({force:!0}).catch(()=>{})}function b(t,e){if(!t?.success)throw new Error(t?.message||"That did not save.");return t.data&&(a.assistantData=t.data),k(),g(),e&&c(e,"success"),t}async function tt(){const t=document.getElementById("assistant-person-name")?.value.trim(),e=document.getElementById("assistant-person-role")?.value.trim(),i=document.getElementById("assistant-person-phone")?.value.trim();if(!t||!i)return c("Enter a name and mobile number.","error");try{const n=await p("POST","/api/crm/frontdesk-assistant/recipients",{name:t,role:e,phone:i});b(n,n.verificationSent?"Verification code sent.":"Phone connected.")}catch(n){c(n.message||"Could not add that phone.","error")}}async function et(t){const e=document.getElementById(`assistant-code-${t}`)?.value.trim();if(!e||e.length!==6)return c("Enter the 6-digit code.","error");try{b(await p("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/verify`,{code:e}),"Phone connected to Front Desk.")}catch(i){c(i.message||"That code could not be verified.","error")}}async function it(t){try{b(await p("POST",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}/resend`,{}),"A new code was sent.")}catch(e){c(e.message||"Could not resend the code.","error")}}async function at(t){if(window.confirm("Remove this phone from Front Desk Assistant?"))try{b(await p("DELETE",`/api/crm/frontdesk-assistant/recipients/${encodeURIComponent(t)}`),"Phone removed.")}catch(e){c(e.message||"Could not remove that phone.","error")}}async function nt(){const t={enabled:!!document.getElementById("assistant-enabled")?.checked,checkFrequency:document.getElementById("assistant-frequency")?.value||"smart",dailyCheckTime:document.getElementById("assistant-check-time")?.value||"18:00",quietHoursStart:document.getElementById("assistant-quiet-start")?.value||"",quietHoursEnd:document.getElementById("assistant-quiet-end")?.value||"",timeZone:document.getElementById("assistant-time-zone")?.value||Intl.DateTimeFormat().resolvedOptions().timeZone||"America/Chicago",notifyNewBookings:!!document.getElementById("assistant-booking-alerts")?.checked},e={enabled:!!document.getElementById("assistant-approval-enabled")?.checked,windowMinutes:Number(document.getElementById("assistant-approval-window")?.value||20),noResponseAction:document.querySelector('input[name="assistant-no-response"]:checked')?.value||"confirm"};try{b(await p("PUT","/api/crm/frontdesk-assistant",t)),await p("POST","/api/crm/booking-approval",e),await u({force:!0}),c("Front Desk Assistant and booking rule saved.","success")}catch(i){c(i.message||"Could not save the assistant.","error")}}async function st(){try{return b(await p("POST","/api/crm/frontdesk-assistant/test",{}),"Test text sent.")}catch(t){c(t.message||"Could not send a test text.","error")}}async function ot(){try{b(await p("POST","/api/crm/frontdesk-assistant/check-now",{}),"Front Desk asked for an availability update.")}catch(t){c(t.message||"Could not send that check.","error")}}function rt(){const t=a.assistantData?.capabilities?.assistantPhone||"";if(!t)return c("The Front Desk number is not available yet.","error");try{const s=window.webkit?.messageHandlers?.marketelShell;if(s&&typeof s.postMessage=="function"){s.postMessage({type:"saveContact",name:"Marketel Front Desk",phone:t});return}}catch{}const e=["BEGIN:VCARD","VERSION:3.0","FN:Marketel Front Desk","ORG:Marketel",`TEL;TYPE=CELL:${t}`,"NOTE:If a walk-in or another channel takes a room, text what happened. Marketel Front Desk updates Availability and your direct booking page.","END:VCARD"].join(`\r
`),i=URL.createObjectURL(new Blob([e],{type:"text/vcard;charset=utf-8"})),n=document.createElement("a");n.href=i,n.download="Marketel-Front-Desk.vcf",document.body.appendChild(n),n.click(),n.remove(),setTimeout(()=>URL.revokeObjectURL(i),2e3)}async function dt(){if(typeof window.isNativeFrontdeskApp=="function"&&window.isNativeFrontdeskApp()){w(),c("Front Desk app access is managed with your Marketel account.","info");return}w();try{const t=await window.loadSettingsModule?.(),e=window.goLive||t?.goLive;if(typeof e=="function"){e();return}}catch{}c("Open Your page to activate Marketel.","info")}const ct={activateFromAssistant:dt,addAssistantRecipient:tt,closeFrontDeskAssistant:w,loadFrontDeskAssistant:u,openFrontDeskAssistant:M,removeAssistantRecipient:at,refreshFrontDeskAssistantSheet:Q,renderFrontDeskAssistantCard:k,renderAssistantPill:Y,hideAssistantPill:W,retryFrontDeskAssistant:X,resendAssistantCode:it,runAssistantCheckNow:ot,saveAssistantContact:rt,saveAssistantSettings:nt,sendAssistantTest:st,updateAssistantPolicySummary:z,updateAssistantTimeDisplay:K,verifyAssistantRecipient:et};function ut(){I||(I=!0,A(),q(ct))}export{dt as activateFromAssistant,tt as addAssistantRecipient,w as closeFrontDeskAssistant,W as hideAssistantPill,ut as install,u as loadFrontDeskAssistant,M as openFrontDeskAssistant,Q as refreshFrontDeskAssistantSheet,at as removeAssistantRecipient,Y as renderAssistantPill,k as renderFrontDeskAssistantCard,it as resendAssistantCode,X as retryFrontDeskAssistant,ot as runAssistantCheckNow,rt as saveAssistantContact,nt as saveAssistantSettings,st as sendAssistantTest,z as updateAssistantPolicySummary,K as updateAssistantTimeDisplay,et as verifyAssistantRecipient};
