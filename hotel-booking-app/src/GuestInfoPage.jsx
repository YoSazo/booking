...
const GuestInfoPage = ({ bookingDetails, onComplete }) => { // Simplified props
  const stripe = useStripe();
  const elements = useElements();
  const navigate = useNavigate();

  // This check for bookingDetails is important
  if (!bookingDetails) {
    return (
  ...
  // Destructuring from props, using your original variable names
  const { room, nights, guests, checkin, checkout, total: totalPrice } = bookingDetails;
  ...
  const handleSubmit = async (event) => {
    ...
    if (!stripe || !elements || !clientSecret || !isFormComplete()) {
      setError('Please fill out all required information.');
      return;
    }
    setProcessing(true);
    setError(null);

    // --- UPDATED: Logic branches based on selected payment method ---
    if (selectedPaymentMethod === 'card') {
      ...
      const { error: paymentMethodError, paymentMethod } = await stripe.createPaymentMethod({
          type: 'card',
          card: cardElement,
          billing_details: { name: formData.fullName, email: formData.email, phone: formData.phone, address: { line1: formData.address, city: formData.city, state: formData.state, postal_code: formData.zip, country: 'US' } },
      });
      ...
      if (confirmError) {
          ...
          navigate('/confirmation', { state: { bookingDetails, guestInfo: formData, paymentMethod: 'Card' } });
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="card bg-base-100 shadow-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="label">Full Name</label><input type="text" name="fullName" value={formData.fullName} onChange={handleChange} className="input input-bordered w-full" required /></div>
                <div><label className="label">Email Address</label><input type="email" name="email" value={formData.email} onChange={handleChange} className="input input-bordered w-full" required /></div>
                <div><label className="label">Phone Number</label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="input input-bordered w-full" required /></div>
                <div><label className="label">Country</label><select name="country" value={formData.country} onChange={handleChange} className="select select-bordered w-full">{countries.map((c) => (<option key={c.code} value={c.name}>{c.name}</option>))}</select></div>
              </div>
            </div>
            ...
            <div className="card bg-base-100 shadow-xl p-6">
              ...
              <div className="flex gap-2 mb-6">
                <div style={paymentMethodButtonStyle('card')} onClick={() => setSelectedPaymentMethod('card')}>Card</div>
                {paymentRequest && (<div style={paymentMethodButtonStyle('wallet')} onClick={() => setSelectedPaymentMethod('wallet')}>Apple Pay / Google Pay</div>)}
                <div style={paymentMethodButtonStyle('amazon')} onClick={() => setSelectedPaymentMethod('amazon')}>Amazon Pay</div>
              </div>
              <div>
                {selectedPaymentMethod === 'card' && (
                  <div>
                    <label className="label font-semibold">Card Details</label>
                    <div className="p-4 border rounded-lg" style={{ borderColor: '#ddd' }}><CardElement options={cardElementOptions} /></div>
                  </div>
                )}
                {selectedPaymentMethod === 'wallet' && (<div className="text-center p-4 bg-gray-100 rounded-lg"><p>The <strong>Apple Pay / Google Pay</strong> modal will appear after you click "Complete Payment".</p></div>)}
                {selectedPaymentMethod === 'amazon' && (<div className="text-center p-4 bg-gray-100 rounded-lg"><p>The <strong>Amazon Pay</strong> modal will appear after you click "Complete Payment".</p></div>)}
              </div>
              <div className="mt-6">
            ...
            <button type="submit" className="btn btn-primary w-full text-lg" disabled={processing || !stripe || !clientSecret}>
        ...
        <div className="lg:col-span-1">
          <div className="card bg-base-100 shadow-xl p-6 sticky top-8">
            ...
            <div className="space-y-2">
              ...
              {/* Using original lowercase variable names */}
